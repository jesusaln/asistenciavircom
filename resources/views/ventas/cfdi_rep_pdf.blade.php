...
<head>
    ...
    <style>
        ...
        .header-table { margin-bottom: 20px; border-bottom: 2px solid {{ $color_principal ?? '#7c3aed' }}; }
        .company-name { font-size: 18px; color: {{ $color_principal ?? '#7c3aed' }}; margin: 0 0 5px 0; }
        .company-details { font-size: 9px; color: #555; line-height: 1.2; }
        
        .invoice-box { text-align: right; }
        .invoice-box h2 { margin: 0; font-size: 12px; color: {{ $color_principal ?? '#7c3aed' }}; background-color: {{ $color_principal ?? '#7c3aed' }}15; padding: 6px 12px; border-radius: 4px; display: inline-block; }
        ...
        .label { font-weight: bold; color: #6b7280; width: 130px; }
        
        .payment-box { background-color: #f0fdf4; border: 2px solid #22c55e; border-radius: 8px; padding: 15px; margin: 20px 0; }
        .payment-title { font-size: 14px; font-weight: bold; color: #15803d; margin-bottom: 10px; text-transform: uppercase; }
        .payment-amount { font-size: 28px; font-weight: bold; color: #15803d; }
        
        .related-doc { background-color: #eff6ff; border: 1px solid #3b82f6; border-radius: 6px; padding: 12px; margin-top: 15px; }
        .related-title { font-size: 10px; font-weight: bold; color: #1d4ed8; margin-bottom: 8px; text-transform: uppercase; }
        ...
        .sello-title { font-weight: bold; font-size: 7px; color: #374151; margin-top: 4px; border-bottom: 1px solid #d1d5db; text-transform: uppercase; letter-spacing: 0.5px; }
        .sello-text { font-family: monospace; font-size: 6px; color: #4b5563; word-wrap: break-word; word-break: break-all; white-space: normal; line-height: 1.2; display: block; margin-bottom: 5px; }
...
<body>
    ...
    <div style="background-color: #fef2f2; border: 2px solid #dc2626; border-radius: 8px; padding: 12px; margin-bottom: 15px;">
        <div style="font-weight: bold; color: #dc2626; font-size: 12px; margin-bottom: 5px;">[!] DOCUMENTO CANCELADO</div>
        @php
            $motivos = [
        ...
        <div style="font-size: 10px; color: #991b1b;">
            ...
            <strong>Fecha de Cancelación:</strong> {{ $cfdi->fecha_cancelacion ? \Carbon\Carbon::parse($cfdi->fecha_cancelacion)->format('d/m/Y H:i') : 'N/A' }}
        ...
        <tr>
            <td style="width: 65%; vertical-align: bottom; padding-bottom: 10px;">
                ...
                <div class="company-details">
                    ...
                    <strong>Régimen:</strong> {{ $emisor['RegimenFiscal'] }} - {{ $emisor['RegimenFiscalNombre'] ?? '' }}<br>
    ...
    <div class="section-content">
        <table>
            ...
            <tr>
                ...
                <td>{{ $receptor['RegimenFiscalReceptor'] ?? '' }} - {{ $receptor['RegimenFiscalReceptorNombre'] ?? '' }}</td>
            </tr>
            <tr>
    ...
    <div class="related-doc">
        ...
        <table style="font-size: 10px;">
            <tr>
                ...
                <td colspan="3" style="font-family: monospace; font-size: 9px;">{{ $doctoRelacionado['IdDocumento'] ?? '' }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="3"><strong>${{ number_format($doctoRelacionado['ImpSaldoInsoluto'] ?? 0, 2) }}</strong></td>
            </tr>
        </table>
    </div>
    @endif
