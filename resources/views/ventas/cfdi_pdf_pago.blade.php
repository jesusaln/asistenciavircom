    ...
    @if(isset($comprobante['ComplementoPago']['Pago']))
        @foreach($comprobante['ComplementoPago']['Pago'] as $pago)
            ...
            @php
                ...
                $docs = [];
                if (isset($pago['pago10:DoctoRelacionado']))
                    $docs = $pago['pago10:DoctoRelacionado'];
                elseif (isset($pago['pago20:DoctoRelacionado']))
                    $docs = $pago['pago20:DoctoRelacionado'];
                elseif (isset($pago['DoctoRelacionado']))
                    $docs = $pago['DoctoRelacionado'];

                // Normalizar a array si es uno solo
                if (isset($docs['@attributes']))
                    $docs = [$docs];
            @endphp
