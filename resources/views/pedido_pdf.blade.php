...
<body>
    <div class="header">
        ...
        <div class="invoice-info">
            <h2 style="font-size: 14px; padding: 5px 10px;">PEDIDO</h2>
            <p class="folio" style="margin: 5px 0;">{{ $pedido->numero_pedido }}</p>
            <p style="font-size: 10px;">
                <strong>Fecha:</strong> {{ $pedido->created_at ? $pedido->created_at->format('d/m/Y') : ($pedido->fecha ?? date('d/m/Y')) }}
            </p>
            <p style="font-size: 10px;"><strong>Moneda:</strong> {{ $moneda }}</p>
            @if(isset($pedido->estado))
                <p style="font-size: 10px; margin-top:2px;">
                    <strong>Estado:</strong> {{ ucfirst($pedido->estado->label()) }}
                </p>
            @endif
        </div>
        <div class="clear"></div>
    ...
    <div class="client-info" style="margin: 15px 0; padding: 10px;">
        ...
        <p class="name" style="font-size: 11px; margin-bottom: 2px;">{{ $pedido->cliente ? $pedido->cliente->nombre_razon_social : 'Público General' }}</p>
        <div style="font-size: 10px;">
            @if($pedido->cliente && $pedido->cliente->rfc && $pedido->cliente->rfc !== 'XAXX010101000')
                <span style="margin-right: 15px;"><strong>RFC:</strong> {{ $pedido->cliente->rfc }}</span>
            @endif
            @if($pedido->cliente && $pedido->cliente->telefono)
                <span style="margin-right: 15px;"><strong>Tel:</strong> {{ $pedido->cliente->telefono }}</span>
            @endif
            @if($pedido->cliente && $pedido->cliente->email)
                <span><strong>Email:</strong> {{ $pedido->cliente->email }}</span>
            @endif
            @if($pedido->cliente && ($pedido->cliente->direccion ?? $pedido->cliente->direccion_completa ?? false))
                <br><strong>Dir:</strong> {{ $pedido->cliente->direccion ?? $pedido->cliente->direccion_completa }}
            @endif
        </div>
    </div>
    ...
    <table style="margin-bottom: 15px;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    {{ $item->pedible ? $item->pedible->nombre : 'Producto no encontrado' }}
                </td>
                <td class="text-right">${{ number_format($item->precio, 2) }}</td>
                <td class="text-right">{{ $item->descuento > 0 ? number_format($item->descuento, 0) . '%' : '-' }}</td>
                <td class="text-right">${{ number_format($item->cantidad * $item->precio * (1 - ($item->descuento ?? 0 / 100)), 2) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
        ...
        <div class="totals">
            <table>
                <tr>
                    ...
                    <td class="text-right" style="padding: 2px;">${{ number_format($pedido->subtotal, 2) }} {{ $moneda }}</td>
                </tr>
                @if($pedido->descuento_general > 0)
                <tr>
                    <td class="text-right" style="padding: 2px;"><strong>Descuento:</strong></td>
                    <td class="text-right" style="padding: 2px;">-${{ number_format($pedido->descuento_general, 2) }} {{ $moneda }}</td>
                </tr>
                @endif
                <tr>
                    <td class="text-right" style="padding: 2px;"><strong>IVA (16%):</strong></td>
                    <td class="text-right" style="padding: 2px;">${{ number_format($pedido->iva, 2) }} {{ $moneda }}</td>
                </tr>
                @if(($pedido->isr ?? 0) > 0)
                <tr>
                    <td class="text-right" style="padding: 2px;"><strong>Retención ISR:</strong></td>
                    <td class="text-right" style="padding: 2px; color: #dc3545;">-${{ number_format($pedido->isr, 2) }} {{ $moneda }}</td>
                </tr>
                @endif
                <tr class="total-row">
                    <td class="text-right" style="padding: 5px !important;"><strong>TOTAL:</strong></td>
                    <td class="text-right" style="padding: 5px !important;"><strong>${{ number_format($pedido->total, 2) }} {{ $moneda }}</strong></td>
                </tr>
            </table>
        </div>
        <div class="clear"></div>
    ...
    <div class="footer">
        <p><strong>{{ $empresa['nombre'] ?? 'Empresa' }}</strong> - Gracias por su compra.</p>
        ...
        @endif
        <p style="font-size: 7px;">Este documento es una representación impresa de un pedido.</p>
    </div>
</body>
</html>
