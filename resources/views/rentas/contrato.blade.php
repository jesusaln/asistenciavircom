...
<body>
  <div class="header">
    ...
    <div class="doc-info">
      ...
      <div><strong>Fecha de emisión:</strong> {{ \App\Services\EmpresaConfiguracionService::formatearFecha(now()) }}
      </div>
    </div>
  </div>
    ...
    <div class="grid-2">
      <div class="box">
        ...
        <div>
          ...
          @if($renta->cliente->direccion_completa ?? false) <strong>Dirección:</strong>
          {{ $renta->cliente->direccion_completa }} @endif
        </div>
      </div>
      <div class="box">
        ...
        <table>
          <tbody>
            <tr>
              ...
              <td class="text-right">
                {{ \App\Services\EmpresaConfiguracionService::formatearFecha($renta->fecha_inicio) }}</td>
            </tr>
            <tr>
              <td class="muted">Fin:</td>
              <td class="text-right">{{ \App\Services\EmpresaConfiguracionService::formatearFecha($renta->fecha_fin) }}
              </td>
            </tr>
            <tr>
              <td class="muted">Día de pago:</td>
              <td class="text-right">{{ $renta->dia_pago }}</td>
            </tr>
            <tr>
              <td class="muted">Duración:</td>
              <td class="text-right">{{ $renta->meses_duracion }} meses</td>
            </tr>
            <tr>
              <td class="muted">Mensualidad:</td>
              <td class="text-right">
    ...
    <div class="grid-2">
      <div class="box">
        ...
        <div>
          ...
          {{ $configuracion['empresa']['direccion'] }}<br>
          <span class="muted">Tel:</span> {{ $configuracion['empresa']['telefono'] }} | <span
            class="muted">Email:</span> {{ $configuracion['empresa']['email'] }}<br>
      ...
      <div class="box">
        ...
        <table>
          <tbody>
            <tr>
              <td class="muted">Banco:</td>
              <td class="text-right">{{ $configuracion['empresa']['banco'] ?? '—' }}</td>
            </tr>
            <tr>
              <td class="muted">Sucursal:</td>
              <td class="text-right">{{ $configuracion['empresa']['sucursal'] ?? '—' }}</td>
            </tr>
            <tr>
              <td class="muted">Cuenta:</td>
              <td class="text-right" style="font-family: monospace;">{{ $configuracion['empresa']['cuenta'] ?? '—' }}
              </td>
            </tr>
            <tr>
              <td class="muted">CLABE:</td>
              <td class="text-right" style="font-family: monospace;">{{ $configuracion['empresa']['clabe'] ?? '—' }}
              </td>
            </tr>
            <tr>
              <td class="muted">Titular:</td>
              <td class="text-right">{{ $configuracion['empresa']['titular'] ?? '—' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  <div class="section">
    ...
    <div class="box">
      <p style="margin:0 0 6px 0;">
        ...
        <strong>{{ $configuracion['empresa']['direccion'] }}</strong>, RFC:
        <strong>{{ $configuracion['empresa']['rfc'] }}</strong>,
        representado para efectos de este contrato por su trabajador debidamente designado, y por la otra parte,
        <strong>{{ $renta->cliente->nombre_razon_social ?? 'EL CLIENTE' }}</strong>, quien actúa a nombre propio como
        persona
      ...
      <p class="muted" style="margin:0 0 6px 0;">Ambas partes se reconocen capaces legalmente para contratar y
        obligarse, y se sujetan al tenor de las siguientes declaraciones:</p>
      ...
      <ol style="margin:0 0 6px 14px; padding:0;">
        <li>a) Que es persona física con capacidad legal para obligarse.</li>
        <li>b) Que cuenta con la experiencia y los medios para otorgar en renta equipos de punto de venta y brindar los
          servicios asociados de instalación y asesoría.</li>
        <li>c) Que ha designado a un trabajador de su confianza para celebrar y firmar este contrato en su
          representación, quien tendrá facultades para recibir pagos en efectivo y cumplir con las obligaciones
          derivadas del presente contrato: <strong>{{ optional(auth()->user())->name ?? '________________' }}</strong>.
        </li>
      </ol>
      <p style="margin:0;"><strong>III. Declaran ambas partes:</strong> Que existe la voluntad de celebrar el presente
        contrato de renta de punto de venta, sujetándose a las siguientes cláusulas.</p>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="box">
      <p style="margin:0 0 4px 0;"><strong>PRIMERA. OBJETO.</strong> “EL PROVEEDOR” da en renta a “EL CLIENTE” equipo(s)
        de punto de venta (en lo sucesivo, “EL EQUIPO”), incluyendo instalación del sistema y asesoría para su uso.</p>
      <p style="margin:0 0 4px 0;"><strong>SEGUNDA. OBLIGACIONES DEL PROVEEDOR.</strong> “EL PROVEEDOR” se obliga a: (a)
        Entregar a “EL CLIENTE” el equipo en condiciones de funcionamiento; (b) Realizar la instalación y puesta en
        marcha del sistema de punto de venta; (c) Brindar asesoría básica para la operación del mismo.</p>
      <p style="margin:0 0 4px 0;"><strong>TERCERA. OBLIGACIONES DEL CLIENTE.</strong> “EL CLIENTE” se obliga a: (a)
        Pagar puntualmente la renta en los términos establecidos en este contrato; (b) Cuidar, conservar y utilizar el
        equipo de manera adecuada; (c) Responder por cualquier daño, extravío o robo del equipo, obligándose a cubrir el
        costo de reparación o reposición, salvo el desgaste natural por uso normal.</p>
      <p style="margin:0 0 4px 0;"><strong>CUARTA. PAGO.</strong> “EL CLIENTE” pagará a “EL PROVEEDOR” la cantidad de
        <strong>{{ \App\Services\EmpresaConfiguracionService::formatearMoneda($renta->monto_mensual) }}</strong> por
        concepto de renta mensual del(los) equipo(s). El pago será por adelantado, dentro de los primeros
      ...
      <p style="margin:0 0 4px 0;"><strong>SEXTA. VIGENCIA.</strong> El presente contrato tendrá vigencia indefinida,
        ...
        período, el contrato podrá darse por terminado mediante aviso por escrito de cualquiera de las partes con al
        menos 30 (treinta) días naturales de anticipación.</p>
      <p style="margin:0 0 4px 0;"><strong>SÉPTIMA. TERMINACIÓN ANTICIPADA.</strong> El contrato podrá darse por
        terminado de manera anticipada en los siguientes casos: (a) Por incumplimiento de cualquiera de las obligaciones
        establecidas en el presente contrato; (b) Por mutuo acuerdo entre las partes.</p>
      <p style="margin:0 0 4px 0;"><strong>OCTAVA. NO CESIÓN.</strong> Ninguna de las partes podrá ceder los derechos y
        obligaciones derivados del presente contrato a un tercero sin el consentimiento previo y por escrito de la otra
        parte.</p>
      <p style="margin:0 0 4px 0;"><strong>NOVENA. INVENTARIO.</strong> Se anexa al presente contrato el inventario del
        equipo entregado a “EL CLIENTE” (<em>Anexo 1: Inventario de Equipo Entregado</em>). Dicho inventario forma parte
        integral del contrato y deberá devolverse en su totalidad al término de la relación contractual.</p>
  ...
  <div class="firma">
    <div class="linea">
      EL PROVEEDOR (por trabajador designado)<br>
      <strong>{{ optional($renta->vendedor)->name ?? '________________' }}</strong>
    </div>
    <div class="linea">
      EL CLIENTE<br>
