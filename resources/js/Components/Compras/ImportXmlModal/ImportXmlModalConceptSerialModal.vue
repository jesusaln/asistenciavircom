    ...
    <div class="flex min-h-full items-center justify-center p-4">
      <div class="relative w-full max-w-lg bg-white rounded-xl shadow-2xl">
        <div class="bg-blue-600 px-6 py-4 rounded-t-xl flex justify-between items-center">
          ...
          <button @click="$emit('close', false)" class="text-white hover:text-slate-200">
        ...
        <div class="p-6">
          ...
          <div class="flex space-x-2 mb-4">
            <input
              ...
              type="text"
              class="flex-1 rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm"
          ...
          <div v-if="currentSerials.length > 0" class="mb-4">
            <p class="text-sm font-medium text-slate-700 mb-2">Series capturadas:</p>
            <ul class="space-y-1 max-h-40 overflow-y-auto custom-scrollbar">
              <li v-for="(serial, idx) in currentSerials" :key="idx" class="flex justify-between items-center p-2 bg-slate-100 rounded-xl text-sm">
                <span class="font-mono">{{ serial }}</span>
                <button @click="removeConceptSerial(idx)" class="text-rose-500 hover:text-rose-700">
