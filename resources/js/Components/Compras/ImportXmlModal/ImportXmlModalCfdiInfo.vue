<template>
  <div>
    <div v-if="!cfdiData.es_factura_valida" class="mb-4 bg-brand-50 border border-brand-200 rounded-xl p-4">
      <div class="flex items-start">
        <svg class="w-5 h-5 text-brand-500 mr-3 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      <div class="bg-slate-50 rounded-xl p-3">
        <p class="text-xs text-slate-500 uppercase tracking-wide">Folio</p>
        <p class="font-semibold text-slate-900">{{ cfdiData.serie }}{{ cfdiData.folio }}</p>
      </div>
      <div class="bg-slate-50 rounded-xl p-3">
        <p class="text-xs text-slate-500 uppercase tracking-wide">Fecha</p>
        <p class="font-semibold text-slate-900">{{ formatDate(cfdiData.fecha) }}</p>
      </div>
      <div class="bg-slate-50 rounded-xl p-3">
        <p class="text-xs text-slate-500 uppercase tracking-wide">Subtotal</p>
        <p class="font-semibold text-slate-900">${{ formatMoney(cfdiData.subtotal) }}</p>
      </div>
      <div class="bg-slate-50 rounded-xl p-3">
        <p class="text-xs text-slate-500 uppercase tracking-wide">IVA (16%)</p>
    ...
    <div v-if="cfdiData.descuento > 0" class="mb-4 bg-rose-50 border border-rose-200 rounded-xl p-3">
      <div class="flex items-center justify-between">
        <span class="text-sm font-medium text-rose-700">Descuento aplicado:</span>
        <span class="font-semibold text-rose-600">-${{ formatMoney(cfdiData.descuento) }}</span>
      </div>
    </div>
    ...
    <div class="mb-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
          ...
          <p class="font-semibold text-slate-900 text-lg">{{ cfdiData.emisor?.nombre }}</p>
          <div class="mt-2 space-y-1">
            <p class="text-sm text-slate-600">
              <span class="font-medium">RFC:</span> {{ cfdiData.emisor?.rfc }}
              <span v-if="cfdiData.emisor?.rfc?.length === 12" class="ml-2 px-1.5 py-0.5 text-xs bg-sky-100 text-sky-800 rounded-xl">Persona Moral</span>
              <span v-else-if="cfdiData.emisor?.rfc?.length === 13" class="ml-2 px-1.5 py-0.5 text-xs bg-purple-100 text-purple-700 rounded-xl">Persona Física</span>
            </p>
            <p v-if="cfdiData.emisor?.regimen_fiscal" class="text-sm text-slate-600">
            ...
            <p v-if="cfdiData.lugar_expedicion" class="text-sm text-slate-600">
              <span class="font-medium">CP Expedición:</span> {{ cfdiData.lugar_expedicion }}
            </p>
          </div>
        </div>
        <div v-if="cfdiData.proveedor_encontrado" class="flex items-center text-emerald-600 bg-emerald-50 px-3 py-2 rounded-xl">
        ...
        <div v-else class="flex flex-col items-end space-y-3 w-full max-w-sm">
          <div class="flex items-center text-brand-600 mb-1">
            <svg class="w-5 h-5 mr-1" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
            </svg>
          ...
          <div class="w-full space-y-2 bg-white p-3 rounded-xl border border-slate-200">
            <p class="text-xs text-slate-500 mb-2">Datos adicionales (opcionales):</p>
            <input
              :value="newProviderEmail"
              type="email"
              placeholder="Email de contacto"
              class="w-full text-sm rounded-xl border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
          ...
          <button
            ...
            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed w-full justify-center"
          >
          ...
          <p class="text-xs text-slate-400 text-center">Se usará: RFC, Nombre y Régimen Fiscal del XML</p>
        </div>
      </div>
    </div>
    ...
    <div class="mb-4 bg-indigo-50 border border-indigo-200 rounded-xl p-4">
      <div class="flex items-center justify-between">
        ...
        <select
          ...
          class="w-64 text-sm rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500"
          required
    ...
    <div v-if="cfdiData?.metodo_pago === 'PUE'" class="mb-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
      <div class="flex items-center justify-between">
        <label class="flex items-center cursor-pointer">
          <input
            type="checkbox"
            :checked="puePagado"
            class="h-4 w-4 text-emerald-600 focus:ring-emerald-500 border-slate-300 rounded-xl"
      ...
      <div v-if="puePagado" class="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-emerald-100">
        <div>
          ...
          <select
            ...
            class="w-full text-sm rounded-xl border-emerald-300 focus:border-emerald-500 focus:ring-emerald-500"
            required
    ...
    <div class="mb-4 flex items-center justify-between bg-slate-100 rounded-xl p-3">
      <span class="text-sm font-medium text-slate-700">Productos del CFDI</span>
      <div class="flex items-center space-x-4 text-sm">
        ...
        <span v-if="cfdiData.mapeo_stats?.similares" class="flex items-center text-amber-600">
          <span class="w-2 h-2 bg-brand-500 rounded-full mr-1"></span>
          {{ cfdiData.mapeo_stats.similares }} similares
        </span>
        <span v-if="cfdiData.mapeo_stats?.no_encontrados" class="flex items-center text-rose-600">
