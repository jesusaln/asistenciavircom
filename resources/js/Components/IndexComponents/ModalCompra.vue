            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-200 dark:border-slate-800">
              ...
              <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-800 bg-gray-50">
                <div class="grid grid-cols-2 gap-6">
                  <!-- Proveedor -->
                  <div>
                    <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Proveedor</h3>
                    <p class="text-base font-bold text-gray-900 dark:text-white">{{ selected?.proveedor?.nombre_razon_social || selected?.proveedor?.nombre || 'Desconocido' }}</p>
                  ...
                  <div>
                    ...
                    <div class="space-y-2 text-sm">
                      <div v-if="selected?.usuario || selected?.comprador" class="flex items-center">
                        ...
                        <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ selected.usuario?.name || selected.comprador?.nombre || 'N/A' }}</span>
                      </div>
            ...
            <div v-if="selected?.cfdi_uuid" class="mx-6 mb-6 p-4 bg-blue-50 border border-blue-100 rounded-lg">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex flex-col">
                  ...
                  <dd class="text-[11px] font-mono font-bold text-gray-900 dark:text-white break-all">{{ selected.cfdi_uuid }}</dd>
                </div>
                <div class="grid grid-cols-2 gap-4">
                  <div class="flex flex-col">
                    <dt class="text-[10px] font-semibold text-blue-600 uppercase">Folio / Serie</dt>
                    <dd class="text-xs font-bold text-gray-900 dark:text-white">{{ selected.cfdi_serie || '' }}{{ selected.cfdi_folio || '---' }}</dd>
                  </div>
                  <div class="flex flex-col">
                    <dt class="text-[10px] font-semibold text-blue-600 uppercase">RFC Emisor</dt>
                    <dd class="text-xs font-bold text-gray-900 dark:text-white">{{ selected.cfdi_emisor_rfc || '---' }}</dd>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-200 dark:border-slate-800">
              <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-800 bg-gray-50">
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-sm text-gray-500 dark:text-gray-400">{{ itemsCalculados.length }} items</span>
                </div>
              </div>
                      ...
                      <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                          <div class="flex items-center">
                            ...
                            <div class="ml-4">
                              <div class="text-sm font-medium text-gray-900 dark:text-white">{{ producto.nombre || producto.producto_nombre }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                          <span class="inline-flex items-center justify-center h-8 w-8 rounded-full bg-gray-100 text-sm font-semibold text-gray-900 dark:text-white">{{ producto.cantidad }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900 dark:text-white">{{ formatCurrency(producto.precio) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                          <span v-if="producto.descuento > 0" class="text-red-600 font-medium">{{ producto.descuento }}%</span>
                          <span v-else class="text-gray-400">-</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-900 dark:text-white">{{ formatCurrency(producto.subtotal) }}</td>
                      </tr>
                        ...
                        <td colspan="5" class="px-6 py-3">
                          ...
                          <div class="flex flex-wrap gap-2">
                            <div v-for="(serie, idx) in producto.series" :key="idx" class="inline-flex items-center bg-white dark:bg-slate-900 border border-blue-200 rounded-md px-3 py-1.5 shadow-sm">
                              <span class="text-sm font-mono font-semibold text-gray-900 dark:text-white">{{ serie.numero_serie || serie }}</span>
                              <span v-if="serie.almacen" class="ml-2 text-xs text-gray-500 dark:text-gray-400">Almacén {{ serie.almacen || 'N/D' }}</span>
                            </div>
                          </div>
                        </td>
                      </tr>
              ...
              <div v-else class="px-6 py-12 text-center">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Esta compra no tiene productos registrados.</p>
              </div>
            </div>
          </div>
...
const estadoClaseBg = computed(() => {
  const map = {
    ...
    borrador: 'bg-gray-50 border-gray-200 dark:border-slate-800'
  }
