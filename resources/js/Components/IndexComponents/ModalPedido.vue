      ...
      <div class="relative w-full max-w-6xl bg-white dark:bg-slate-950 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col transition-all duration-500 animate-in fade-in zoom-in-95 max-h-[95vh]">
        ...
        <div class="px-8 py-8 border-b border-slate-100 dark:border-slate-800 bg-transparent/30 dark:bg-slate-900/50 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
            ...
            <div class="flex items-center gap-6 relative z-10">
                <div class="w-16 h-16 rounded-[1.5rem] bg-indigo-600 flex items-center justify-center text-white shadow-xl shadow-indigo-600/20">
                ...
                <div>
                    <div class="flex items-center gap-3 mb-1">
                        <h2 class="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-wide">Análisis de Pedido</h2>
                        <span class="text-[10px] font-mono font-black text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-sky-900/20 px-3 py-1 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                            {{ numeroPedido }}
                        </span>
                    </div>
                    <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Expediente Logístico • {{ formatearFechaFull(selected.created_at) }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="p-8 bg-transparent dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 relative group overflow-hidden">
                            ...
                            <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] block mb-6">Información del Cliente</span>
                            <div class="flex items-start gap-5">
                                ...
                                <div>
                                    <p class="text-base font-black text-slate-900 dark:text-white uppercase leading-tight mb-2">{{ selected.cliente?.nombre_razon_social || 'Desconocido' }}</p>
                        ...
                        <div class="p-8 bg-transparent dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 relative group overflow-hidden">
                            <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] block mb-6">Detalles de Almacén</span>
                            <div class="space-y-4">
                                <div class="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:border-indigo-600">
                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-wide">Almacén</span>
                                    <span class="text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase">{{ selected.almacen?.nombre || 'General' }}</span>
                                </div>
                                <div class="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:border-indigo-600">
                    ...
                    <div v-if="selected.notas" class="p-8 bg-slate-900 dark:bg-white rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                        ...
                        <div class="flex items-start gap-6 relative z-10">
                            ...
                            <div>
                                <h3 class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.3em] mb-2">Instrucciones Operativas</h3>
                    ...
                    <div class="bg-white dark:bg-slate-950 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
                        <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-transparent/30 dark:bg-slate-900/50">
                            <h3 class="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-[0.3em]">Conceptos del Pedido</h3>
                            <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide">{{ itemsCalculados.length }} Items Registrados</span>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                                <thead class="bg-transparent dark:bg-slate-900/50">
                                    <tr>
                                        <th class="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-wide">Descripción</th>
                                    ...
                                    <tr v-for="item in itemsCalculados" :key="item.id" class="group hover:bg-transparent dark:hover:bg-slate-900/50 transition-all duration-300">
                                        <td class="px-8 py-6">
                                            <div class="flex items-center gap-4">
                                                <div :class="item.tipo === 'producto' ? 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'" class="w-12 h-12 rounded-xl border flex items-center justify-center font-black text-[10px] shadow-sm shrink-0">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="p-10 bg-slate-950 dark:bg-white rounded-[3rem] shadow-2xl relative overflow-hidden group border border-slate-800 dark:border-slate-100">
                        <div class="absolute bottom-0 right-0 w-48 h-48 bg-indigo-500/10 rounded-full -mr-24 -mb-24 transition-transform group-hover:scale-110 blur-2xl"></div>
                        
                        <h3 class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.4em] mb-10">Valoración de Orden</h3>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="pt-8 mt-8 border-t border-white/10 dark:border-slate-100">
                                <div class="flex items-center gap-2 mb-3">
                                    ...
                                    <p class="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em]">Total Operativo</p>
                                </div>
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <button v-if="selected.estado?.toLowerCase() !== 'cancelado'" @click="$emit('enviar-a-venta', selected)" class="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] shadow-xl shadow-indigo-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H7" /></svg>
                            Convertir a Venta
                        </button>
...
const emit = defineEmits(['close', 'editar', 'enviar-a-venta'])

const onClose = () => emit('close')

const numeroPedido = computed(() => props.selected?.numero_pedido || `#${props.selected?.id || '---'}`)
