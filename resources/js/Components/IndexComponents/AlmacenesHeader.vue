<template>
  <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden index-header-root">
    <!-- Header con estadísticas -->
    <div class="px-6 py-6 border-b border-slate-200/60">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Almacenes</h1>
          <p class="text-sm text-slate-600 mt-1">Gestiona todos tus almacenes en un solo lugar</p>
        </div>
        <button
          @click="onCrearNueva"
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Total Artículos</p>
              <p class="text-2xl font-bold text-blue-600">{{ formatNumber(totalArticulosGlobal) }}</p>
            </div>
            <div class="w-10 h-10 bg-sky-100 rounded-full flex items-center justify-center">
              <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Valor Total</p>
              <p class="text-2xl font-bold text-emerald-600">{{ formatearMoneda(valorTotalGlobal) }}</p>
            </div>
            <div class="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center">
              <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const props = defineProps({
  ...
  totalArticulosGlobal: { type: Number, default: 0 },
  valorTotalGlobal: { type: Number, default: 0 },
})
...
const onFiltroTipoChange = () => emit('filtro-tipo-change', filtroTipo.value)
...
watch([searchTerm, sortBy, filtroEstado, filtroTipo], () => {
  // Emitir cambios automáticamente
}, { immediate: true })
</script>

<style scoped>
/* Estilos específicos para el header de almacenes */
.almacenes-header {
  background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%);
}

@media (max-width: 768px) {
  .almacenes-header .grid {
