<template>
  <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors index-header-root">
    ...
    <div class="px-6 py-6 border-b border-slate-200/60 dark:border-slate-700" :style="{ background: `linear-gradient(135deg, ${colors.principal}15 0%, ${colors.secundario}10 100%)` }">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Cotizaciones</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-0.5">Gestiona todas tus cotizaciones en un solo lugar</p>
          </div>
        </div>
        <button
          @click="onCrearNueva"
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-600/50 shadow-sm">
        ...
        <div class="bg-white/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-600/50 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Enviado a Pedido</p>
        ...
        <div class="bg-white/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-600/50 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Cancelado</p>
              <p class="text-2xl font-bold text-rose-600 dark:text-rose-400">{{ cancelado }}</p>
            </div>
    ...
    <!-- Filtros y búsqueda -->
    <div class="px-6 py-4 bg-transparent dark:bg-slate-900/50 border-b border-slate-200/60 dark:border-slate-700">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        ...
        <div class="flex-1 max-w-md">
          <div class="relative">
            ...
            <input
              ...
              class="block w-full pl-10 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded-xl leading-5 bg-white dark:bg-slate-800 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:placeholder-slate-400 focus:ring-1 focus:ring-brand-500 focus:border-brand-500 text-sm transition-colors"
        ...
        <div class="flex items-center space-x-3">
          ...
          <select
            ...
            class="block w-48 pl-3 pr-10 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 bg-white dark:bg-slate-800 dark:text-white transition-colors"
          >
          ...
          <button
            ...
            class="inline-flex items-center px-3 py-2 border border-slate-300 dark:border-slate-600 text-sm leading-4 font-medium rounded-xl text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-700 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500 transition-colors duration-200"
          >
