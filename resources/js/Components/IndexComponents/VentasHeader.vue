    ...
    <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800" :style="{ background: `linear-gradient(135deg, ${colors.principal}08 0%, ${colors.secundario}05 100%)` }">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center space-x-5">
          ...
          <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-wider">Ventas</h1>
            <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Panel de Control Financiero</p>
          </div>
        </div>
    ...
    <div class="px-8 py-6 bg-transparent/30 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-md p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm transition-all hover:bg-white dark:hover:bg-slate-800/50">
          <div class="flex flex-col">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide">Total Global</span>
        ...
        <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-md p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm transition-all hover:bg-white dark:hover:bg-slate-800/50">
          <div class="flex flex-col">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide">Borrador</span>
                <div class="w-7 h-7 rounded-xl flex items-center justify-center bg-slate-100 dark:bg-slate-800">
                    <svg class="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </div>
            </div>
            <p class="text-2xl font-black text-slate-600 dark:text-slate-400">{{ borrador }}</p>
          </div>
        </div>
        ...
        <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-md p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm transition-all hover:bg-white dark:hover:bg-slate-800/50">
          <div class="flex flex-col">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[9px] font-black text-brand-600 dark:text-brand-500 uppercase tracking-wide">Pendientes</span>
            ...
            <p class="text-2xl font-black text-brand-600 dark:text-brand-500">{{ pendientes }}</p>
          </div>
        </div>
        ...
        <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-md p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm transition-all hover:bg-white dark:hover:bg-slate-800/50">
          <div class="flex flex-col">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[9px] font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-wide">Aprobadas</span>
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div class="flex-1 max-w-xl">
          <div class="relative group">
            ...
            <input
              v-model="searchTerm"
              type="text"
              aria-label="Buscar ventas por cliente, folio o estado"
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative min-w-[180px]">
            <select
              v-model="filtroCfdi"
              aria-label="Filtrar por estado CFDI"
