  ...
  <div class="bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden index-header-root">
    ...
    <div class="px-6 py-6 border-b border-slate-200/60">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Rentas</h1>
          <p class="text-sm text-slate-600 mt-1">Gestiona todas tus rentas de equipos en un solo lugar</p>
        </div>
        <button
          @click="onCrearNueva"
          class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-colors duration-200"
        >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-emerald-600">{{ activas }}</p>
            </div>
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Próx. 7 días</p>
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Vencidas</p>
              <p class="text-2xl font-bold text-rose-600">{{ vencidas }}</p>
            </div>
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold" :class="contratos_en_mora > 0 ? 'text-rose-600' : 'text-slate-600'">{{ contratos_en_mora }}</p>
            </div>
            <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="contratos_en_mora > 0 ? 'bg-rose-100' : 'bg-slate-100'">
              <svg class="w-5 h-5" :class="contratos_en_mora > 0 ? 'text-rose-600' : 'text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Monto Vencido</p>
              <p class="text-xl font-bold" :class="monto_vencido > 0 ? 'text-rose-600' : 'text-slate-600'">${{ formatearMoneda(monto_vencido) }}</p>
            </div>
            <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="monto_vencido > 0 ? 'bg-rose-100' : 'bg-slate-100'">
              <svg class="w-5 h-5" :class="monto_vencido > 0 ? 'text-rose-600' : 'text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white/70 rounded-xl p-4 border border-slate-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-slate-600">Ingresos Esperados</p>
              <p class="text-xl font-bold text-emerald-600">${{ formatearMoneda(ingresos_mensuales) }}</p>
            </div>
            <div class="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center">
              <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
