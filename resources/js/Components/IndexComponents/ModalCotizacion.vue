              ...
              <div class="px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-700 text-white rounded-t-lg">
                <div class="flex justify-between items-start">
                  <div>
                    ...
                    <p class="text-purple-100 text-sm mt-1">{{ numeroCotizacion }}</p>
                  </div>
                  <div class="text-right">
                    <p class="text-sm text-purple-100">Fecha</p>
          ...
          <div class="space-y-6">
            ...
            <div class="rounded-lg border" :class="estadoClaseBg">
              ...
              <div class="px-4 pb-4 text-sm text-gray-700">
                ...
                <p v-else>Revisa la cotización antes de enviarla a pedido.</p>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-200 dark:border-slate-800 p-4 space-y-2">
              ...
              <button v-if="selected?.estado === 'aprobada' || selected?.estado === 'aprobado'" @click="$emit('enviar-a-pedido', selected)" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const emit = defineEmits(['close', 'enviar-a-pedido', 'cancelar', 'eliminar', 'editar', 'duplicar', 'imprimir'])
...
const numeroCotizacion = computed(() => props.selected?.numero_cotizacion || `C${String(props.selected?.id || '').padStart(3, '0')}`)
...
const estadoClase = computed(() => {
  const map = {
    ...
    enviado_pedido: 'bg-orange-100 text-orange-800',
    convertida_pedido: 'bg-green-100 text-green-800',
    convertida: 'bg-green-100 text-green-800',
    borrador: 'bg-yellow-100 text-yellow-800',
    cancelado: 'bg-red-100 text-red-800',
    sin_estado: 'bg-gray-100 text-gray-800'
  }
...
const estadoPunto = computed(() => {
  const map = {
    ...
    convertida_pedido: 'bg-green-500',
    convertida: 'bg-green-500',
    aprobada: 'bg-blue-500',
    pendiente: 'bg-gray-500',
    enviada: 'bg-purple-500',
    enviado_pedido: 'bg-orange-500',
...
const estadoTexto = computed(() => {
  const map = {
    ...
    convertida_pedido: 'text-green-800',
    convertida: 'text-green-800',
    aprobada: 'text-blue-800',
    pendiente: 'text-gray-800',
    enviada: 'text-purple-800',
    enviado_pedido: 'text-orange-800',
    borrador: 'text-yellow-800'
  }
...
const verPdfEnNavegador = async (cotizacionId) => {
  try {
    const response = await fetch(route('cotizaciones.pdf', cotizacionId), {
      method: 'GET',
      headers: {
...
const descargarPdf = async (cotizacionId, numeroCotizacion) => {
  try {
    // Usamos la misma ruta pero tal vez enviando parametro extra si el controlador lo soporta?
    // De momento, si el controlador no soporta ?modo=download, esto solo descargará lo que retorne.
    // Asumiremos que el controlador PDF retorna stream o download.
    // Como el usuario pidió botones separados, forzamos descarga via JS con el blob.
    ...
    link.href = url
    link.download = `cotizacion-${numeroCotizacion || cotizacionId}.pdf`
