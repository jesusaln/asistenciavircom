...
const isProductosActive = computed(() => {
    return props.activeTab === 'tienda' || props.activeTab === 'life12plus';
});
...
<template>
    <nav 
        class="bg-white/90 dark:bg-slate-950/90 backdrop-blur-md border-b border-gray-100 dark:border-white/5 sticky top-0 z-50 transition-all duration-500 ease-in-out h-auto overflow-visible"
        :class="[
    ...
    >
        ...
        >
            ...
            <div class="flex items-center gap-2 sm:gap-4">
                ...
                <!-- CAS Badge -->
                <div class="hidden sm:flex flex-col items-start leading-none border-l border-gray-200 dark:border-white/10 pl-4">
                    <span class="text-[var(--color-primary)] font-black text-[10px] uppercase tracking-tighter">Centro Autorizado</span>
                    <span class="text-gray-900 dark:text-white font-black text-[12px] uppercase tracking-widest flex items-center gap-1">
                        Mirage
            ...
            <div class="hidden md:flex items-center gap-8">
                <template v-for="link in navLinks" :key="link.id">
                    ...
                    <div v-if="link.id === 'servicios'" class="relative" v-click-outside="() => showServiciosMenu = false">
                        ...
                        >
                            <div v-if="showServiciosMenu" class="absolute left-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-white/5 py-2 z-50">
                                <Link 
                                    v-for="sLink in serviciosLinks" 
                                    :key="sLink.id"
                                    :href="route(sLink.route, sLink.params || {})" 
                                    class="block px-4 py-3 text-sm font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-[var(--color-primary)] transition-colors"
                    ...
                    <div v-else-if="link.id === 'tienda'" class="relative" v-click-outside="() => showProductosMenu = false">
                        ...
                        <Transition
                            ...
                            leave-to-class="opacity-0 -translate-y-2"
                        >
                            <div v-if="showProductosMenu" class="absolute left-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-white/5 py-2 z-50">
                                <Link 
                                    ...
                                    :class="pLink.highlighted ? 'text-[var(--color-primary)] bg-[var(--color-primary)]/5 hover:bg-[var(--color-primary)]/10' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-[var(--color-primary)]'"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button 
                        ...
                        class="relative z-20 p-2.5 mr-2 rounded-xl text-gray-400 hover:text-[var(--color-primary)] hover:bg-gray-50 dark:hover:bg-white/5 transition-all focus:outline-none active:scale-90 cursor-pointer"
                    ...
                    <!-- Cart Icon -->
                    <Link :href="route('tienda.carrito')" class="relative p-2.5 bg-gray-50 dark:bg-white/5 rounded-xl text-gray-500 dark:text-gray-300 hover:text-[var(--color-primary)] hover:bg-white dark:hover:bg-white/10 hover:shadow-sm transition-all group/cart">
                    ...
                    <div v-if="currentUser" class="flex items-center gap-4">
                        <div class="relative" v-click-outside="() => showUserMenu = false">
                            ...
                            <Transition
                                ...
                                leave-to-class="opacity-0 -translate-y-2"
                            >
                                <div v-if="showUserMenu" class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-white/5 py-2 z-50">
                                    <Link 
                                        :href="route(currentUser.tipo === 'cliente' ? 'portal.dashboard' : 'dashboard')" 
                                        class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                    ...
                    <div v-else class="flex items-center gap-4">
                        <Link :href="route('portal.login')" class="px-5 py-2.5 bg-[var(--color-primary)] text-white rounded-xl font-black text-xs uppercase tracking-widest hover:shadow-lg hover:shadow-[var(--color-primary)]/30 transition-all">
                            Ingresar
                        </Link>
            ...
            <div class="md:hidden flex items-center gap-3">
                ...
                <button 
                    ...
                    class="w-12 h-12 flex items-center justify-center rounded-2xl bg-gray-50 dark:bg-white/5 text-gray-900 dark:text-white transition-all active:scale-90 relative z-[40] border border-gray-100 dark:border-white/5 shadow-sm"
            ...
            <Transition name="fade">
                <div v-if="showMobileMenu" class="fixed inset-0 z-[9999] md:hidden overflow-hidden">
                    ...
                    <Transition name="drawer-slide" appear>
                        <div v-if="showMobileMenu" class="absolute right-0 top-0 bottom-0 w-[85%] max-w-[360px] bg-white dark:bg-slate-950 shadow-[0_0_80px_rgba(0,0,0,0.5)] flex flex-col border-l border-gray-100 dark:border-white/5">
                            <div class="p-6 flex justify-between items-center border-b border-gray-50 dark:border-white/5">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-[var(--color-primary)] flex items-center justify-center text-white font-black text-sm">C</div>
                                    <span class="text-xs font-black uppercase tracking-[0.2em] text-gray-900 dark:text-white">Menú</span>
                                </div>
                                <button @click="showMobileMenu = false" class="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-100 dark:bg-white/5 text-gray-900 dark:text-white transition-all active:scale-90">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <template v-for="link in navLinks" :key="link.id">
                                    <div v-if="link.dropdown" class="mb-2">
                                        ...
                                        <Transition
                                            ...
                                            leave-to-class="max-h-0 opacity-0 overflow-hidden"
                                        >
                                            <div v-if="showMobileProductos" class="mt-4 bg-gray-50 dark:bg-white/5 rounded-3xl overflow-hidden border border-gray-100 dark:border-white/5">
                                                <Link 
                                                    v-for="pLink in productosLinks" 
                                                    :key="pLink.id"
                                                    :href="route(pLink.route, pLink.params || {})"
                                                    class="block px-6 py-4 text-base font-bold border-b border-white dark:border-white/5 last:border-0 active:bg-[var(--color-primary)] active:text-white"
                                                    :class="pLink.highlighted ? 'text-[var(--color-primary)] bg-[var(--color-primary)]/5' : 'text-gray-600 dark:text-gray-400'"
                                                    @click="showMobileMenu = false"
                                                >
                                                    {{ pLink.name }}
                                                </Link>
                            ...
                            <div class="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-t-[3rem] border-t border-gray-100 dark:border-white/5">
                                <div v-if="!currentUser" class="grid grid-cols-1 gap-4">
                                    <Link :href="route('portal.login')" class="py-4 text-center bg-[var(--color-primary)] text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-[var(--color-primary)]/30" @click="showMobileMenu = false">Ingresar</Link>
                                </div>
    ...
    <Teleport to="body">
        <Transition name="fade">
            <div v-if="showAuthModal" class="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                <div class="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" @click="showAuthModal = false"></div>
                <div class="relative bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl max-w-md w-full p-10 text-center border border-white/10 overflow-hidden animate-scale-in">
                    ...
                    <div class="text-gray-500 dark:text-gray-400 font-medium mb-8 leading-relaxed text-sm space-y-4">
                        <p>Tu cuenta ha sido creada y <strong class="text-green-600">puedes realizar compras ahora mismo.</strong></p>
                    ...
                    <button @click="showAuthModal = false" class="w-full py-5 bg-[var(--color-primary)] text-white rounded-2xl font-black uppercase tracking-widest text-xs">Continuar Comprando</button>
                </div>
            </div>
        </Transition>
    </Teleport>
