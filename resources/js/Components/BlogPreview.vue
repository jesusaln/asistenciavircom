...
<template>
    <section id="blog" class="py-24 bg-white dark:bg-slate-900 relative overflow-hidden transition-colors duration-300" :style="cssVars">
        <!-- Decorative Pattern -->
        <div class="absolute inset-0 opacity-[0.02] dark:opacity-[0.05] pointer-events-none transition-opacity">
            <svg width="100%" height="100%">
                <pattern id="blogGrid" width="60" height="60" patternUnits="userSpaceOnUse">
                    <circle cx="30" cy="30" r="1" class="text-slate-900 dark:text-white" fill="currentColor" />
                </pattern>
        ...
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            ...
            <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-16">
                <div>
                    ...
                    <h2 class="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight transition-colors">
                        Aprende sobre <span class="text-[var(--color-primary)]">Tecnología y Seguridad</span>
                    </h2>
                    <p class="mt-4 text-slate-500 dark:text-slate-400 max-w-xl transition-colors">
            ...
            <div class="grid lg:grid-cols-2 gap-8">
                <!-- Featured Article -->
                <article v-if="articuloDestacado" class="group relative bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl overflow-hidden min-h-[500px] flex flex-col justify-end">
                    ...
                    <div class="relative z-10 p-8 lg:p-10">
                        <div class="flex items-center gap-4 mb-6">
                            <span class="px-3 py-1 bg-[var(--color-primary)] text-white text-[10px] font-black uppercase tracking-wide rounded-full">
                                {{ articuloDestacado.categoria }}
                            </span>
                            <span class="text-slate-400 text-sm">{{ articuloDestacado.fecha }}</span>
                        ...
                        <p class="text-slate-300 mb-6 leading-relaxed line-clamp-2">
                            {{ articuloDestacado.extracto }}
                        </p>
                        
                        <Link 
                            :href="'/blog/' + articuloDestacado.slug" 
                            class="inline-flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl font-bold text-sm hover:bg-[var(--color-primary)] hover:text-white transition-all duration-300"
                        >
                            Leer artículo
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-col gap-6">
                    <article 
                        ...
                        :key="articulo.id"
                        class="group bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-xl hover:border-[var(--color-primary)]/20 transition-all duration-300"
                    >
                        <div class="flex">
                            <!-- Thumbnail -->
                            <div class="w-32 md:w-48 flex-shrink-0 bg-slate-100 dark:bg-slate-700 flex items-center justify-center transition-colors">
                                <img 
                            ...
                            <div class="flex-1 p-6">
                                <div class="flex items-center gap-3 mb-3">
                                    <span class="text-[var(--color-primary)] text-[10px] font-black uppercase tracking-wide">
                                        {{ articulo.categoria }}
                                    </span>
                                    <span class="text-slate-400 text-xs">{{ articulo.tiempo_lectura }}</span>
                                </div>
                                
                                <h3 class="font-bold text-slate-900 dark:text-white mb-2 group-hover:text-[var(--color-primary)] transition-colors line-clamp-2">
                                    {{ articulo.titulo }}
                                </h3>
                                
                                <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-4 transition-colors">
                                    {{ articulo.extracto }}
                                </p>
                    ...
                    <div class="bg-[var(--color-primary-soft)] rounded-2xl p-6 border border-[var(--color-primary)]/10">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-slate-900 mb-1">Recibe consejos cada semana</h4>
                                <p class="text-sm text-slate-600 mb-4">Tips exclusivos sobre seguridad, redes y mantenimiento de equipo.</p>
                                <div class="flex gap-2">
                                    <input 
                                        type="email"
                                        placeholder="tu@email.com"
                                        class="flex-1 px-4 py-2 rounded-xl border border-slate-200 focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all outline-none text-sm"
                                    >
                                    <button class="px-4 py-2 bg-[var(--color-primary)] text-white rounded-xl font-bold text-sm hover:shadow-lg transition-all">
                                        Suscribir
                                    </button>
                                </div>
                            </div>
