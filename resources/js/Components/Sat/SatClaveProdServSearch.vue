    ...
    <div class="relative w-full">
        ...
        <div class="relative">
            ...
            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                <svg v-if="loading" class="animate-spin h-5 w-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div v-if="showResults && (results.length > 0 || search.length >= 3)" class="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar">
            <ul v-if="results.length > 0" class="py-1">
                ...
                >
                    ...
                    <div class="text-slate-600 truncate">{{ result.descripcion }}</div>
                </li>
            </ul>
            <div v-else-if="search.length >= 3 && !loading" class="px-4 py-3 text-sm text-slate-500 italic">
                No se encontraron resultados para "{{ search }}"
            </div>
            <div v-else-if="search.length < 3" class="px-4 py-3 text-sm text-slate-400 italic">
                Escriba al menos 3 caracteres para buscar...
            </div>
        </div>

        <div v-if="selectedDescription" class="mt-2 p-2 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-800 flex items-start gap-2">
