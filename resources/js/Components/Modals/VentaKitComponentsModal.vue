    ...
    <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
        <div>
          ...
          <p class="text-[10px] text-slate-500 mt-1 uppercase tracking-wide font-black">{{ kit?.nombre }}</p>
        </div>
        ...
        <div v-for="component in components" :key="component.id" 
          ...
          <div class="flex items-start justify-between gap-4 mb-3">
            <div class="flex-1 min-w-0">
               <h4 class="text-xs font-black text-slate-800 dark:text-slate-200 uppercase tracking-wider">{{ component.item?.nombre }}</h4>
               <p class="text-[10px] text-slate-500 mt-0.5 font-bold uppercase tracking-wide">Código: {{ component.item?.codigo || '---' }}</p>
            </div>
          ...
          <div v-if="getSerials(component).length > 0" class="mb-3 flex flex-wrap gap-1.5">
             <span v-for="s in getSerials(component)" :key="s" 
                   class="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-xl text-[9px] font-bold font-mono">
               {{ s }}
             </span>
          </div>
          <div v-else class="mb-3">
             <span class="text-[10px] text-rose-500 font-bold uppercase tracking-wide flex items-center gap-1.5">
          ...
          <button 
            ...
            class="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-wide rounded-xl transition-all shadow-lg shadow-indigo-500/20 active:scale-95">
      ...
      <div class="p-6 border-t border-slate-100 dark:border-slate-800 flex justify-end">
        <button @click="$emit('close')" class="px-8 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-wide rounded-xl hover:bg-slate-200 dark:hover:bg-slate-750 transition-all">
          Listo
        </button>
      </div>
    </div>
