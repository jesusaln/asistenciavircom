...
const getTipoBadge = (tipo) => {
    ...
    return tipos[tipo] || 'bg-gray-50 text-gray-600 border-gray-100';
};
    ...
    <ClientLayout :empresa="empresa">
        <div class="px-2 sm:px-0">
            ...
            <div class="mb-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100">
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">Historial de Consumo</h1>
                        <p class="text-gray-500 font-medium text-sm mt-1">
                            Póliza: <strong class="text-gray-900">{{ poliza.nombre }}</strong> ({{ poliza.folio }})
                        </p>
                    </div>
                    ...
                    <div class="flex items-end gap-2 w-full md:w-auto">
                        <div class="w-full md:w-64">
                            ...
                            <select v-model="formMes" class="w-full bg-gray-50 border-gray-200 rounded-xl font-bold text-gray-700 focus:ring-[var(--color-primary)] focus:border-[var(--color-primary)] transition-all">
                                <option v-for="m in mesesDisponibles()" :key="m.val" :value="m.val">
                                    {{ m.label.charAt(0).toUpperCase() + m.label.slice(1) }}
                                </option>
                            </select>
                        </div>
                        <a :href="route('portal.polizas.reporte.pdf', { poliza: poliza.id, mes: formMes.split('-')[1], anio: formMes.split('-')[0] })" target="_blank" class="px-6 py-3 bg-white border-2 border-slate-100 text-slate-600 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2 h-[42px]">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-lg shadow-gray-200/30">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="consumo in consumos" :key="consumo.id" class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-6">
                                ...
                                <td class="px-8 py-6">
                                    <p class="text-sm font-medium text-gray-600 leading-relaxed max-w-md">
                                        {{ consumo.descripcion }}
                                    </p>
                                </td>
                            ...
                            <tr v-if="consumos.length === 0">
                                <td colspan="4" class="px-8 py-20 text-center">
                                    <div class="max-w-xs mx-auto">
                                        ...
                                        <h3 class="text-gray-900 font-black text-lg">Sin consumos</h3>
                                        <p class="text-gray-500 font-medium text-sm mt-2">No se registraron consumos de servicios para el periodo seleccionado.</p>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
