        ...
        <div class="px-2 sm:px-0">
            <div class="mb-10">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Pedido #{{ pedido.numero_pedido || pedido.id }}</h1>
                        <p class="text-gray-500 dark:text-gray-400 font-medium">Realizado el {{ formatDate(pedido.fecha_pedido || pedido.created_at) }}</p>
                    </div>
                    <span 
            ...
            <div class="grid lg:grid-cols-3 gap-10">
                ...
                <div class="lg:col-span-2 space-y-8">
                    <!-- Productos -->
                    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                        <div class="px-8 py-6 border-b border-gray-50 bg-white dark:bg-slate-900/50">
                            <h3 class="font-black text-gray-900 dark:text-white uppercase tracking-tight text-sm">Resumen de Productos</h3>
                        </div>
                        <div class="divide-y divide-gray-50">
                            <div v-for="item in pedido.items" :key="item.id" class="p-8 flex items-center gap-6">
                                <div class="w-20 h-20 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center text-gray-300 text-2xl border border-gray-100 overflow-hidden">
                                ...
                                <div class="flex-1">
                                    <h4 class="font-bold text-gray-900 dark:text-white">{{ item.pedible?.nombre || item.pedible?.descripcion || 'Producto' }}</h4>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium mt-1">Cantidad: <span class="text-gray-900 dark:text-white">{{ item.cantidad }}</span></p>
                                </div>
                                <div class="text-right">
                                    <p class="font-black text-gray-900 dark:text-white">${{ Number(item.precio * item.cantidad).toLocaleString('es-MX', {minimumFractionDigits: 2}) }}</p>
                    ...
                    <!-- Envíos / Tracking -->
                    <div v-if="pedido.numero_guia || pedido.direccion_entrega" class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 p-8">
                        ...
                        <div class="grid md:grid-cols-2 gap-10">
                            <div v-if="pedido.direccion_entrega">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-300 font-medium leading-relaxed">{{ pedido.direccion_entrega }}</p>
                            </div>
                            <div v-if="pedido.numero_guia">
                                ...
                                <div class="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                                    ...
                                    <p class="text-lg font-black text-gray-900 dark:text-white font-mono mt-1">{{ pedido.numero_guia }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-8">
                    <!-- Totales -->
                    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 p-8">
                        <h3 class="font-black text-gray-900 dark:text-white uppercase tracking-tight mb-6 text-sm">Resumen de Pago</h3>
                        <div class="space-y-4">
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-500 dark:text-gray-400 font-medium">Subtotal</span>
                                <span class="font-bold text-gray-900 dark:text-white">${{ Number(pedido.subtotal).toLocaleString('es-MX', {minimumFractionDigits: 2}) }}</span>
                            </div>
                            <div v-if="pedido.iva > 0" class="flex justify-between text-sm">
                                <span class="text-gray-500 dark:text-gray-400 font-medium">IVA (16%)</span>
                                <span class="font-bold text-gray-900 dark:text-white">${{ Number(pedido.iva).toLocaleString('es-MX', {minimumFractionDigits: 2}) }}</span>
                            </div>
                            ...
                            <div class="pt-4 border-t border-gray-50 flex justify-between items-end">
                                <span class="font-black text-gray-900 dark:text-white uppercase tracking-tight">Total</span>
                        ...
                        <div class="mt-8 pt-8 border-t border-gray-50">
                             ...
                             <p class="text-sm font-bold text-gray-900 dark:text-white uppercase">{{ pedido.metodo_pago || 'Pendiente' }}</p>
                    ...
                    <div class="bg-gray-900 rounded-[2rem] p-8 text-white relative overflow-hidden group">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-white dark:bg-slate-900/5 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform"></div>
                        ...
                        <Link :href="route('portal.tickets.create', { titulo: 'Ayuda con pedido #' + (pedido.numero_pedido || pedido.id) })" class="block w-full text-center py-4 bg-white dark:bg-slate-900 text-gray-900 dark:text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-[var(--color-primary)] hover:text-white transition-all">
                            Contactar Soporte
                        </Link>
                    </div>
                </div>
