...
const creditStatus = computed(() => {
    ...
    const maps = {
        'sin_credito': { label: 'Sin Crédito', class: 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:text-gray-400', icon: 'info-circle' },
...
<template>
    ...
    <ClientLayout :empresa="empresa">
        <div class="w-full px-4 sm:px-0">
            <!-- Header -->
            <div class="mb-10">
                <Link :href="route('portal.dashboard')" class="text-sm font-bold text-gray-400 dark:text-gray-500 dark:text-gray-400 hover:text-[var(--color-primary)] transition-colors inline-flex items-center gap-2 mb-4">
                    <font-awesome-icon icon="arrow-left" /> Volver al Inicio
                </Link>
                <h1 class="text-3xl font-black text-gray-900 dark:text-white dark:text-white tracking-tight">Gestión de <span class="text-[var(--color-primary)]">Crédito</span></h1>
                <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 font-medium">Consulte su saldo disponible y administre su documentación para compras a crédito.</p>
            </div>
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <!-- Estado Card -->
                    <div class="bg-white dark:bg-slate-900 dark:bg-slate-900/60 rounded-[2rem] p-8 shadow-xl dark:shadow-2xl shadow-gray-200 dark:shadow-black/50 border border-gray-100 dark:border-white/10 dark:backdrop-blur-xl transition-all">
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <p class="text-xs font-black text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase tracking-widest">Estado Actual</p>
                                <h3 class="font-black text-gray-900 dark:text-white dark:text-white">{{ creditStatus.label }}</h3>
                            </div>
                        </div>

                        <div v-if="cliente.credito_activo" class="space-y-4">
                            <div>
                                <p class="text-xs font-bold text-gray-400 dark:text-gray-500 dark:text-gray-400 mb-1">Límite de Crédito</p>
                                <p class="text-2xl font-black text-gray-900 dark:text-white dark:text-white">
                            ...
                            <div>
                                <p class="text-xs font-bold text-gray-400 dark:text-gray-500 dark:text-gray-400 mb-1">Saldo en Uso</p>
                            ...
                            <div class="pt-4 border-t border-gray-100 dark:border-white/10">
                                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 italic">Días de crédito autorizados: {{ cliente.dias_credito }} días.</p>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-2 space-y-8">
                    <!-- Upload Box -->
                    <div class="bg-white dark:bg-slate-900 dark:bg-slate-900/60 rounded-[2rem] p-8 shadow-xl dark:shadow-2xl border border-gray-100 dark:border-white/10 dark:backdrop-blur-xl transition-colors">
                        <div class="flex flex-col sm:flex-row items-center gap-4 mb-8">
                            ...
                            <div v-if="!cliente.credito_firma" class="flex-1 w-full">
                                <Link :href="route('portal.credito.solicitud.firmar')" class="flex flex-col items-center justify-center p-6 rounded-[2rem] bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-xl shadow-emerald-500/20 hover:scale-[1.02] transition-all group">
                                    <div class="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900/20 flex items-center justify-center text-2xl mb-3 group-hover:rotate-12 transition-transform">
                                        🖋️
                                    </div>
                            ...
                            <div v-else class="flex-1 w-full">
                                <a :href="route('portal.credito.solicitud.descargar')" target="_blank" class="flex flex-col items-center justify-center p-6 rounded-[2rem] bg-white dark:bg-slate-900 dark:bg-slate-800 border-2 border-emerald-100 dark:border-emerald-900/40 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all group">
                                    ...
                                    <span class="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 mt-1 font-bold">Documento Legal Generado</span>
                                </a>
                            </div>
                            ...
                            <div class="flex-1 w-full">
                                <a :href="route('portal.credito.solicitud.descargar')" target="_blank" class="flex flex-col items-center justify-center p-6 rounded-[2rem] bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/30 transition-all group border border-orange-100 dark:border-orange-900/30">
                                    <div class="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 dark:bg-orange-900/30 flex items-center justify-center text-2xl mb-3 group-hover:-translate-y-1 transition-transform">
                                        📥
                                    </div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-4 mb-6">
                            <div v-for="type in documentTypes" :key="type.value">
                                ...
                                >
                                    <div>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white dark:text-white group-hover:text-[var(--color-primary)]">{{ type.label }}</p>
                                        <p class="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">PDF o Imagen (Máx 5MB)</p>
                                    </div>
                                    <font-awesome-icon icon="plus-circle" class="text-gray-300 dark:text-gray-600 dark:text-gray-300 group-hover:text-[var(--color-primary)]" />
                                </button>
                            </div>
                        </div>
                        ...
                        <div 
                            ...
                            :class="[
                                ...
                                isDragging 
                                    ...
                                    : 'border-gray-100 dark:border-white/10 bg-white dark:bg-slate-900 dark:bg-slate-800/30'
                            ]"
                        >
                            <font-awesome-icon icon="cloud-upload-alt" class="text-4xl text-gray-200 dark:text-gray-600 dark:text-gray-300 mb-4 transition-colors" />
                            <p class="text-gray-400 dark:text-gray-500 dark:text-gray-400 font-bold">O arrastre sus archivos aquí para subirlos rápidamente</p>
                    ...
                    <!-- Expediente Actual -->
                    <div v-if="cliente.documentos?.length > 0" class="bg-white dark:bg-slate-900 dark:bg-slate-900/60 rounded-[2rem] p-8 shadow-xl dark:shadow-2xl border border-gray-100 dark:border-white/10 dark:backdrop-blur-xl transition-colors">
                        <h2 class="text-xl font-black text-gray-900 dark:text-white dark:text-white mb-6">Documentos Enviados</h2>
                        <div class="grid gap-4">
                            <div 
                                v-for="doc in cliente.documentos" :key="doc.id"
                                class="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-slate-900 dark:bg-slate-800/50 border border-gray-100 dark:border-white/10 hover:border-blue-200 dark:hover:border-blue-500/30 transition-all"
                            >
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 bg-white dark:bg-slate-900 dark:bg-slate-700/50 rounded-xl flex items-center justify-center text-gray-400 dark:text-gray-300 shadow-sm dark:shadow-none">
                                        <font-awesome-icon icon="file-pdf" v-if="doc.extension === 'pdf'" />
                                        <font-awesome-icon icon="file-image" v-else />
                                    </div>
                                    <div>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white dark:text-white">{{ getDocLabel(doc.tipo) }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 font-medium">{{ doc.nombre_archivo }}</p>
                                    </div>
                                </div>
