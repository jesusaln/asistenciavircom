    ...
    <div class="space-y-8">
        <div>
            ...
            <div class="bg-white dark:bg-slate-900 dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-slate-800 dark:border-gray-700 mb-6">
                <h3 class="text-md font-medium text-gray-900 dark:text-white dark:text-gray-100 mb-4">Seleccionar Proveedor Cloud</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button
                        @click="form.cloud_provider = 'none'"
                        :class="[
                            'p-4 rounded-xl border-2 text-left transition-all',
                            form.cloud_provider === 'none' ? 'border-gray-500 dark:border-gray-400 bg-white dark:bg-slate-900 dark:bg-gray-700' : 'border-gray-200 dark:border-slate-800 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                        ]"
                    >
                        <FontAwesomeIcon icon="times-circle" class="text-2xl text-gray-400 dark:text-gray-500 dark:text-gray-400 mb-2" />
                        <div class="font-medium text-gray-900 dark:text-white dark:text-gray-100">Sin Cloud</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">Solo respaldos locales</div>
                    </button>
                    ...
                    <button
                        ...
                        :class="[
                            'p-4 rounded-xl border-2 text-left transition-all',
                            form.cloud_provider === 'gdrive' ? 'border-blue-500 dark:border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-slate-800 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600'
                        ]"
                    >
                        <img src="https://www.gstatic.com/images/branding/product/1x/drive_2020q4_48dp.png" class="w-8 h-8 mb-2" alt="Google Drive">
                        <div class="font-medium text-gray-900 dark:text-white dark:text-gray-100">Google Drive</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">15 GB gratis, API estable</div>
                    </button>
                </div>
            </div>

            <!-- Configuración Google Drive -->
            <div v-if="form.cloud_provider === 'gdrive'" class="bg-white dark:bg-slate-900 dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-slate-800 dark:border-gray-700 mb-6">
                ...
                <div v-if="!gdriveConnected" class="text-center py-8">
                    ...
                    <p class="text-gray-600 dark:text-gray-300 dark:text-gray-300 mb-6">Conecta tu cuenta de Google Drive para respaldar automáticamente</p>
                    
                    <button 
                        @click="connectGoogleDrive"
                        :disabled="connecting"
                        class="inline-flex items-center gap-3 px-6 py-3 bg-white dark:bg-slate-900 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm hover:bg-white dark:bg-slate-900 dark:hover:bg-gray-600 transition-all"
                    >
                        ...
                        <FontAwesomeIcon v-if="connecting" icon="spinner" spin class="text-gray-400 dark:text-gray-500 dark:text-gray-400" />
                    </button>
                </div>
                ...
                <div v-else class="space-y-6">
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button 
                                ...
                                class="px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 bg-white dark:bg-slate-900 dark:bg-gray-700 border border-red-200 dark:border-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all shadow-sm"
                            >
                    ...
                    <div v-if="gdriveSpace" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="bg-white dark:bg-slate-900 dark:bg-gray-700 p-3 rounded-lg border border-gray-200 dark:border-slate-800 dark:border-gray-600">
                            <div class="text-gray-500 dark:text-gray-400 dark:text-gray-400">Total</div>
                            <div class="font-semibold text-gray-900 dark:text-white dark:text-gray-100">{{ gdriveSpace.total }}</div>
                        </div>
                        <div class="bg-white dark:bg-slate-900 dark:bg-gray-700 p-3 rounded-lg border border-gray-200 dark:border-slate-800 dark:border-gray-600">
                            <div class="text-gray-500 dark:text-gray-400 dark:text-gray-400">Usado</div>
                            <div class="font-semibold text-gray-900 dark:text-white dark:text-gray-100">{{ gdriveSpace.used }}</div>
                        </div>
                        <div class="bg-white dark:bg-slate-900 dark:bg-gray-700 p-3 rounded-lg border border-gray-200 dark:border-slate-800 dark:border-gray-600">
                            <div class="text-gray-500 dark:text-gray-400 dark:text-gray-400">Disponible</div>
                            <div class="font-semibold text-green-600 dark:text-green-400">{{ gdriveSpace.free }}</div>
                        </div>
                    </div>

                    <!-- Opciones -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-slate-800 dark:border-gray-700">
                        <div>
                            ...
                            <input 
                                ...
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-slate-900 dark:bg-gray-700 text-gray-900 dark:text-white dark:text-gray-200"
                        ...
                        <div class="flex items-end">
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-900 dark:text-white dark:text-gray-100">Auto-subir respaldos</span>
                            </label>
                        </div>
                    </div>

                    <!-- Lista de respaldos -->
                    <div class="pt-4 border-t border-gray-200 dark:border-slate-800 dark:border-gray-700">
                        ...
                        <div v-if="loadingBackups" class="text-center py-4 text-gray-500 dark:text-gray-400 dark:text-gray-400">
                            <FontAwesomeIcon icon="spinner" spin /> Cargando...
                        </div>
                        <div v-else-if="gdriveFiles.length === 0" class="text-center py-4 text-gray-500 dark:text-gray-400 dark:text-gray-400">
                            No hay respaldos aún
                        </div>
                        <div v-else class="space-y-2 max-h-48 overflow-y-auto">
                            <div v-for="(file, index) in gdriveFiles" :key="file.id" 
                                class="flex items-center justify-between p-2 rounded-lg"
                                :class="index < 3 ? 'bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-700' : 'bg-white dark:bg-slate-900 dark:bg-gray-700'"
                            >
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">({{ formatSize(file.size) }})</span>
                                ...
                                <div class="flex gap-1">
                                    ...
                                    <button 
                                        ...
                                        :class="index < 3 ? 'text-gray-300 dark:text-gray-500 dark:text-gray-400 cursor-not-allowed' : 'text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/40'"
