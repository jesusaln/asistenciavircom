<template>
    <div class="space-y-6">
        <div>
            ...
            <div class="bg-sky-50 dark:bg-sky-900/20 dark:bg-sky-900/20 border-l-4 border-blue-400 p-4 mb-6">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <FontAwesomeIcon icon="info-circle" class="text-blue-400" />
                    </div>
                    <div class="ml-3">
                        <p class="text-sm text-sky-800 dark:text-sky-200 dark:text-blue-300">
            ...
            <div class="pt-6">
                <div class="flex flex-col gap-3 max-w-xl">
                    <div class="bg-white dark:bg-slate-950 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl overflow-hidden flex items-center justify-center relative w-full h-64 shadow-inner" style="touch-action: none;">
                        ...
                        <div v-if="!isDrawingFirma && !form.firma_digital" class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <FontAwesomeIcon icon="pen-fancy" class="text-slate-200 dark:text-slate-800 text-6xl mb-4" />
                            <span class="text-slate-400 dark:text-slate-500 text-sm font-bold uppercase tracking-wide opacity-50 border-b border-slate-300 dark:border-slate-700">Firmar Aquí</span>
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                         <button v-if="!isDrawingFirma" @click="startEditFirma" type="button" class="flex-1 lg:flex-none text-sm font-bold uppercase py-3 px-6 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-md shadow-sky-500/20 active:scale-95">
                             {{ form.firma_digital ? 'Dibujar Nueva Firma' : 'Iniciar Trazo' }}
                         </button>
                         <button v-if="isDrawingFirma" @click="clearFirma" type="button" class="flex-1 lg:flex-none text-sm font-bold uppercase py-3 px-6 rounded-xl bg-slate-200 text-slate-700 hover:bg-slate-300 dark:bg-slate-800 dark:text-slate-200 transition-all">
                             Limpiar
                         </button>
                         ...
                         <button v-if="form.firma_digital && !isDrawingFirma" @click="removeFirma" type="button" class="text-xs font-bold text-rose-600 hover:text-rose-800 dark:text-rose-200 dark:text-rose-200 px-4 py-2 ml-auto">
                             Descartar Firma
                         </button>
                    </div>
                </div>
                ...
                <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="p-4 bg-[var(--ui-surface)] dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                        <FontAwesomeIcon icon="shield-alt" class="text-blue-500 mb-2" />
                        <h4 class="text-sm font-bold text-slate-900 dark:text-slate-100">Seguro</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">La firma se guarda de forma segura en tu base de datos local.</p>
                    </div>
                    <div class="p-4 bg-[var(--ui-surface)] dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                        <FontAwesomeIcon icon="print" class="text-emerald-500 mb-2" />
                        <h4 class="text-sm font-bold text-slate-900 dark:text-slate-100">Automático</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Aparecerá automáticamente en todos los PDFs oficiales que generes.</p>
                    </div>
                    <div class="p-4 bg-[var(--ui-surface)] dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                        <FontAwesomeIcon icon="mouse-pointer" class="text-purple-500 mb-2" />
                        <h4 class="text-sm font-bold text-slate-900 dark:text-slate-100">Fácil</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Puedes usar tu mouse o una tableta digitalizadora para mayor precisión.</p>
                    </div>
                </div>
            </div>
        </div>
...
import { faPenNib, faInfoCircle, faPenFancy, faShieldAlt, faPrint, faMousePointer } from '@fortawesome/free-solid-svg-icons';
import { notyf } from '@/Utils/notyf.js';
import Swal from '@/Utils/Swal';

library.add(faPenNib, faInfoCircle, faPenFancy, faShieldAlt, faPrint, faMousePointer);
...
const removeFirma = async () => {
    const result = await Swal.fire({
        title: '¿Deseas borrar la firma actual?',
