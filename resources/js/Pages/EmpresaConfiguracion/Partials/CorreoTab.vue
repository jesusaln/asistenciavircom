    ...
    <div class="space-y-6">
        <div>
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-6 flex items-center gap-2">
                <FontAwesomeIcon icon="envelope" class="text-brand-600 dark:text-amber-400" />
                Configuración de Correo (SMTP)
            </h2>

            <!-- Selector de Proveedor -->
            <div class="mb-8">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Cargar configuración predefinida</label>
                <div class="flex flex-wrap gap-2">
                    <button v-for="(prov, key) in proveedoresSMTP" :key="key" type="button" @click="aplicarConfiguracionProveedor(key)"
                        class="px-3 py-2 text-xs font-medium rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500 transition-colors">
                        {{ prov.nombre }}
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Host -->
                <div>
                    <label for="smtp_host" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Host SMTP</label>
                ...
                <!-- Puerto -->
                <div>
                    <label for="smtp_port" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Puerto</label>
                ...
                <!-- Usuario -->
                <div>
                    <label for="smtp_username" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Usuario SMTP</label>
                ...
                <!-- Contraseña -->
                <div>
                    <label for="smtp_password" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Contraseña</label>
                ...
                <!-- Encriptación -->
                <div>
                    <label for="smtp_encryption" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Encriptación</label>
            ...
            <div class="mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
                <h3 class="text-md font-medium text-slate-900 dark:text-slate-100 mb-4">Configuración de Remitente</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="email_from_address" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Correo Remitente</label>
                        <input v-model="form.email_from_address" id="email_from_address" type="email"
                            class="w-full px-4 py-3 border border-slate-300 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200" />
                    </div>
                    <div>
                        <label for="email_from_name" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Nombre Remitente</label>
            ...
            <!-- Prueba de Correo -->
            <div class="mt-8 p-4 bg-indigo-50 dark:bg-sky-900/20 rounded-xl border border-indigo-100 dark:border-indigo-700">
                ...
                <div class="flex gap-2">
                    <input v-model="emailPrueba" type="email" placeholder="correo@prueba.com"
                        class="flex-1 px-4 py-2 border border-indigo-200 dark:border-indigo-600 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200" />
                    <button type="button" @click="probarCorreo"
                        class="px-6 py-2 bg-brand-500 text-white font-medium rounded-xl hover:bg-brand-600 transition-colors">
                        Enviar Prueba
                    </button>
                </div>
            </div>
