...
<template>
    <Head :title="`Agendar Cita Premium - ${empresaData?.nombre_empresa || 'Climas del Desierto'}`" />

    <div 
        class="premium-wrapper min-h-screen selection:bg-orange-500/30 selection:text-orange-200"
        :class="isDarkMode ? 'dark bg-[#020617] text-slate-100' : 'bg-slate-50 text-slate-900'"
        :style="cssVars"
    >
        ...
        <div class="relative max-w-7xl mx-auto px-4 py-12 md:py-24">
            ...
            <div class="grid lg:grid-cols-2 gap-16 items-center">
                ...
                >
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-[10px] font-black uppercase tracking-widest mb-6">
                    ...
                    <h2 class="text-5xl md:text-7xl font-black text-white leading-[1.05] tracking-tight mb-8">
                        ...
                        <span class="bg-gradient-to-r from-[var(--color-primary)] via-orange-400 to-amber-300 bg-clip-text text-transparent italic">hoy mismo.</span>
                    </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div v-for="feat in features" :key="feat.title" class="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.05] hover:border-orange-500/30 transition-colors">
                ...
                >
                    <div class="relative group">
                        <!-- Card Glow -->
                        <div class="absolute -inset-1 bg-gradient-to-r from-[var(--color-primary)] to-amber-600 rounded-[2.5rem] opacity-20 blur-xl group-hover:opacity-30 transition-opacity"></div>
                        
                        <div class="relative bg-[#020617]/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl">
                            
                            <div v-if="page.props.flash?.success" class="mb-8 p-6 rounded-3xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                                <p class="font-black uppercase tracking-widest text-xs mb-1">Solicitud Enviada</p>
                                <p class="text-sm opacity-90 font-medium">{{ page.props.flash.success }}</p>
                            </div>

                            <form @submit.prevent="submit" class="space-y-6">
                                <div class="space-y-2">
                                    <label class="text-xs font-black uppercase tracking-widest text-slate-500 ml-1">Nombre Completo</label>
                                    <input 
                                        v-model="form.nombre"
                                        type="text" 
                                        placeholder="Ej. JUAN PÉREZ"
                                        class="w-full bg-white/[0.03] border border-white/10 rounded-2xl px-6 py-4 text-white placeholder:text-slate-600 outline-none focus:border-[var(--color-primary)] focus:bg-white/[0.06] focus:ring-4 focus:ring-[var(--color-primary-soft)] transition-all antialiased h-16 text-lg font-medium uppercase"
                                        required
                                    >
                                    <p v-if="form.errors.nombre" class="text-red-400 text-xs mt-1 ml-1">{{ form.errors.nombre }}</p>
                                </div>

                                <div class="space-y-2">
                                    <label class="text-xs font-black uppercase tracking-widest text-slate-500 ml-1">WhatsApp / Teléfono</label>
                                    <div class="relative">
                                        ...
                                        <input 
                                            ...
                                            maxlength="10"
                                            class="w-full bg-white/[0.03] border border-white/10 rounded-2xl pl-20 pr-6 py-4 text-white placeholder:text-slate-600 outline-none focus:border-[var(--color-primary)] focus:bg-white/[0.06] focus:ring-4 focus:ring-[var(--color-primary-soft)] transition-all h-16 text-lg font-medium font-mono tracking-wider"
                                            required
                                        >
                                    </div>
                                    <p v-if="form.errors.telefono" class="text-red-400 text-xs mt-1 ml-1">{{ form.errors.telefono }}</p>
                                </div>

                                <div class="space-y-2">
                                    <label class="text-xs font-black uppercase tracking-widest text-slate-500 ml-1">Horario de Contacto</label>
                                    <div class="relative">
                                        <select 
                                ...
                                <button 
                                    ...
                                    class="group/btn w-full h-20 bg-gradient-to-r from-[var(--color-primary)] to-orange-600 rounded-3xl text-white font-black uppercase tracking-[0.15em] shadow-xl shadow-orange-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-4 text-lg"
                                >
                                ...
                                <div class="flex items-center justify-center gap-2 text-[10px] text-slate-600 font-bold uppercase tracking-[0.2em] mt-2">
                                    <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                                    Técnicos disponibles en este momento
                                </div>

                                <p class="text-[10px] text-center text-slate-600 uppercase font-black tracking-widest mt-6">
