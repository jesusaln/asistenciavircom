    ...
    <div class="w-full px-6 py-8 animate-fade-in">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-slate-500 mt-2">Configura campañas con scripts generados por IA</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="c in campanias" :key="c.id" 
                 class="bg-white rounded-2xl shadow-xl-sm border border-slate-100 overflow-hidden hover:shadow-xl transition-shadow">
                <!-- Header -->
                <div class="px-5 py-4 border-b border-slate-100" 
                     :class="c.vigente ? 'bg-gradient-to-r from-purple-50 to-white' : 'bg-white'">
                    <div class="flex items-center justify-between">
                        <h3 class="font-bold text-slate-900 truncate">{{ c.nombre }}</h3>
                        <span class="px-2.5 py-1 rounded-xl text-xs font-bold"
                              :class="c.vigente ? 'bg-emerald-100 text-emerald-800 dark:text-emerald-200 dark:text-emerald-200' : c.activa ? 'bg-brand-100 text-brand-800 dark:text-brand-200 dark:text-amber-200' : 'bg-slate-100 text-slate-500'">
                ...
                <div class="px-5 py-4 space-y-3">
                    <p v-if="c.objetivo" class="text-sm text-slate-500">{{ c.objetivo }}</p>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2 text-sm">
                            <span class="px-2 py-1 bg-sky-100 text-sky-800 dark:text-sky-200 rounded-xl">
                                <FontAwesomeIcon :icon="['fas', 'phone']" class="mr-1" />
                                {{ c.meta_actividades_dia }}/día
                            </span>
                        </div>
                        <div class="flex items-center gap-2 text-sm">
                            <span class="px-2 py-1 bg-purple-100 text-purple-700 rounded-xl">
                ...
                <!-- Actions -->
                <div class="px-5 py-3 bg-white border-t border-slate-100 flex items-center justify-between gap-2">
                    <div class="flex items-center gap-1">
                        <a :href="`/crm/campanias/${c.id}/exportar`" 
                           class="p-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 hover:bg-emerald-100" 
                           title="Exportar JSON para IA">
                            <FontAwesomeIcon :icon="['fas', 'download']" />
                        </a>
                        <button @click="abrirImportarScripts(c)" 
                                class="p-2 rounded-xl bg-sky-50 dark:bg-sky-900/20 text-blue-600 hover:bg-sky-100" 
                                title="Importar Scripts">
                            <FontAwesomeIcon :icon="['fas', 'upload']" />
                        </button>
                        <button @click="toggleCampania(c)" 
                                class="p-2 rounded-xl" 
                                :class="c.activa ? 'bg-brand-50 dark:bg-brand-900/20 text-brand-600 hover:bg-amber-100' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'"
                                :title="c.activa ? 'Desactivar' : 'Activar'">
                            <FontAwesomeIcon :icon="['fas', c.activa ? 'pause' : 'play']" />
                        </button>
                    </div>
                    <Link :href="`/crm/campanias/${c.id}`" 
                          class="px-3 py-1.5 bg-purple-500 text-white text-sm rounded-xl hover:bg-purple-600">
                        Ver Scripts →
                    </Link>
                </div>
            </div>
            ...
            <!-- Empty State -->
            <div v-if="!campanias.length" class="col-span-full py-16 text-center bg-white rounded-2xl border border-slate-100">
                <FontAwesomeIcon :icon="['fas', 'bullhorn']" class="h-12 w-12 text-slate-300 mb-4" />
                <p class="text-slate-500 font-medium mb-2">Sin campañas</p>
                <p class="text-sm text-slate-400 mb-4">Crea tu primera campaña para generar scripts con IA</p>
                <button @click="showModalNueva = true" class="px-4 py-2 bg-purple-500 text-white rounded-xl hover:bg-purple-600">
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 animate-scale-in">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-slate-900">Nueva Campaña</h3>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input v-model="form.nombre" type="text" required placeholder="Ej: Promo Minisplits Diciembre"
                                       class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand-500" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Producto Específico</label>
                                <BuscarProducto
                                    :productos="productos"
                                    :solo-productos="true"
                                    :validar-stock="false"
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Meta de Actividades/Día *</label>
                                <input v-model.number="form.meta_actividades_dia" type="number" min="1" max="50" required
                                       class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand-500" />
                                <p class="text-xs text-slate-500 mt-1">Cuántas llamadas/seguimientos debe hacer cada vendedor por día</p>
                            </div>
                        </div>
        ...
        <div v-if="showModalImport" class="fixed inset-0 z-50 overflow-y-auto custom-scrollbar" @click.self="showModalImport = false">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 animate-scale-in">
                    ...
                    <div class="bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800/30 rounded-xl p-4 mb-4">
                        <h4 class="font-semibold text-sky-800 dark:text-sky-200 mb-2">Flujo con IA:</h4>
                    ...
                    <div class="bg-white border border-slate-200 rounded-xl p-4 mb-4">
                        <h4 class="font-semibold text-slate-700 mb-2">Formato CSV esperado:</h4>
                        <pre class="text-xs bg-white p-2 rounded-xl border overflow-x-auto">tipo,nombre,contenido,tips
