  ...
  <div class="w-full p-6">
    ...
    <form @submit.prevent="submit" class="bg-white rounded-xl border p-6 space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div v-if="equipo.imagen" class="mt-2">
            <img :src="'/storage/' + equipo.imagen" alt="Imagen actual" class="w-16 h-16 object-cover rounded-xl" />
          </div>
        </div>
      ...
      <div class="pt-4 flex gap-3">
        ...
        <Link :href="route('equipos.index')" class="px-4 py-2 rounded-xl bg-slate-200 hover:bg-slate-300">
          Cancelar
        </Link>
      </div>
    </form>
