...
const estadoChip = (est) => {
  const map = {
    pendiente: 'bg-amber-500/15 text-amber-200 ring-amber-500/30',
    pendiente_asignacion: 'bg-orange-500/15 text-orange-200 ring-orange-500/30',
    programado: 'bg-sky-500/15 text-sky-200 ring-sky-500/30',
    en_proceso: 'bg-violet-500/15 text-violet-200 ring-violet-500/30',
    completado: 'bg-emerald-500/15 text-emerald-200 ring-emerald-500/30',
    cancelado: 'bg-rose-500/15 text-rose-200 ring-rose-500/30',
    reprogramado: 'bg-cyan-500/15 text-cyan-200 ring-cyan-500/30',
  }
  return map[est] || 'bg-slate-600/40 text-slate-200 ring-slate-500/30'
}
...
<template>
  ...
  <AppLayout title="Citas por técnico">
    <div
      class="report-citas-tecnico min-h-[calc(100vh-5rem)] w-full max-w-none min-w-0 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 px-2 sm:px-3 lg:px-4 pb-10 pt-2 sm:pt-4 border-t border-white/5"
    >
      <!-- Hero + acciones -->
      <div class="w-full min-w-0 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-6 sm:mb-8">
        <div>
          ...
          <h1 class="text-2xl sm:text-3xl font-black tracking-tight text-white">
            Citas por técnico
            <span class="block sm:inline sm:ml-2 text-base sm:text-lg font-bold text-slate-400">detalle ejecutivo</span>
          </h1>
          <p class="mt-2 max-w-3xl text-sm text-slate-400 leading-relaxed">
            ...
            <span class="text-slate-500">No incluye tickets de soporte.</span>
      ...
      <div class="w-full min-w-0 max-w-none space-y-5 sm:space-y-6">
          ...
          <div class="rounded-2xl sm:rounded-3xl bg-slate-900/80 ring-1 ring-white/10 p-4 sm:p-5 shadow-2xl shadow-black/40 w-full min-w-0">
            ...
            <div class="flex flex-wrap gap-3 sm:gap-4 items-end">
              ...
              <div class="min-w-[200px] flex-1 sm:flex-none">
                <label class="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1.5">Técnico</label>
                <select
                  v-model="tecnicoId"
                  class="w-full rounded-xl border-0 bg-slate-950/80 text-slate-100 text-sm px-3 py-2.5 ring-1 ring-white/10 focus:ring-2 focus:ring-cyan-500/50"
                  @change="aplicar"
              ...
              <div class="min-w-[200px] flex-1 sm:flex-none">
                <label class="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1.5">Estado</label>
                <select
                  v-model="estado"
                  class="w-full rounded-xl border-0 bg-slate-950/80 text-slate-100 text-sm px-3 py-2.5 ring-1 ring-white/10 focus:ring-2 focus:ring-cyan-500/50"
                  @change="aplicar"
              ...
              <label
                class="inline-flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-950/50 ring-1 ring-white/10 text-xs text-slate-300 cursor-pointer max-w-full"
                title="Desactívalo para incluir citas sin técnico asignado."
              >
                <input v-model="soloConTecnico" type="checkbox" class="rounded border-slate-600 text-cyan-500 focus:ring-cyan-500/40" @change="aplicar" />
        ...
        <!-- KPIs -->
        <div>
          <h2 class="text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 mb-3">Estadísticas del periodo</h2>
          <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-white/10 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-cyan-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">Citas en rango (BD)</p>
              <p class="mt-1 text-2xl sm:text-3xl font-black text-white tabular-nums">{{ estadisticas.total_en_rango ?? 0 }}</p>
              <p class="mt-2 text-[11px] text-slate-500">Total que cumple fecha, estado y técnico en servidor.</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-white/10 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-violet-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">Cargadas en tabla</p>
              <p class="mt-1 text-2xl sm:text-3xl font-black text-white tabular-nums">{{ estadisticas.mostrados ?? 0 }}</p>
              <p class="mt-2 text-[11px] text-slate-500">
                <span v-if="truncated" class="text-amber-400/90">Corte por límite.</span>
                <span v-else>Coinciden con el total en rango.</span>
              </p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-emerald-500/20 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-emerald-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-emerald-400/80">Completadas</p>
              <p class="mt-1 text-2xl sm:text-3xl font-black text-emerald-300 tabular-nums">{{ estadisticas.completadas ?? 0 }}</p>
              <p class="mt-2 text-[11px] text-slate-500">{{ pctCompletadas }}% de las filas cargadas</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-rose-500/20 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-rose-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-rose-300/90">Canceladas</p>
              <p class="mt-1 text-2xl sm:text-3xl font-black text-rose-200 tabular-nums">{{ statsFilas.canceladas }}</p>
              <p class="mt-2 text-[11px] text-slate-500">{{ pctCanceladasFilas }}% de las filas cargadas</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-amber-500/15 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-amber-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-amber-200/80">En pipeline</p>
              <p class="mt-1 text-2xl sm:text-3xl font-black text-amber-100 tabular-nums">{{ statsFilas.pipeline }}</p>
              <p class="mt-2 text-[11px] text-slate-500">Pendiente, programado, en proceso o reprogramado.</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-cyan-500/25 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-blue-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-cyan-400/90">Importe ∑ (tabla)</p>
              <p class="mt-1 text-lg sm:text-xl font-black text-cyan-200 tabular-nums break-all">
                {{ fmtMoneda(estadisticas.importe_total_mostrado) }}
              </p>
              <p class="mt-2 text-[11px] text-slate-500">Suma de totales en filas visibles.</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-white/10 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-slate-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">Ticket promedio</p>
              <p class="mt-1 text-lg sm:text-xl font-black text-slate-100 tabular-nums">
                {{ fmtMoneda(statsFilas.importe_promedio) }}
              </p>
              <p class="mt-2 text-[11px] text-slate-500">Importe medio por cita en pantalla.</p>
            </div>
            <div class="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-slate-900/60 ring-1 ring-indigo-500/20 shadow-xl">
              <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-indigo-500/10 blur-2xl" />
              <p class="text-[10px] font-black uppercase tracking-widest text-indigo-300/90">Tiempo medio servicio</p>
              <p class="mt-1 text-lg sm:text-xl font-black text-indigo-100 tabular-nums">
                {{ statsFilas.mins_promedio_completadas != null ? fmtMin(statsFilas.mins_promedio_completadas) : '—' }}
              </p>
              <p class="mt-2 text-[11px] text-slate-500">Solo citas completadas con minutos registrados.</p>
            </div>
          </div>
        </div>

        <!-- Resumen por técnico -->
        <div v-if="resumen_por_tecnico.length" class="rounded-2xl sm:rounded-3xl bg-slate-900/50 ring-1 ring-white/10 overflow-hidden shadow-2xl">
          <div class="px-4 sm:px-6 py-4 border-b border-white/5 flex items-center justify-between gap-3">
            <h2 class="text-sm font-black uppercase tracking-[0.2em] text-slate-300">Resumen por técnico</h2>
            <span class="text-[10px] font-bold text-slate-500">{{ resumen_por_tecnico.length }} técnicos</span>
          </div>
          <div class="hidden lg:block w-full min-w-0 overflow-x-auto">
            <table class="w-full min-w-0 table-fixed text-sm">
              ...
              <thead>
                <tr class="text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-white/5">
              ...
              <tbody>
                ...
                >
                  <td class="px-4 xl:px-6 py-3.5 font-semibold text-slate-100 align-top break-words min-w-0">{{ row.tecnico_nombre }}</td>
                  <td class="px-3 py-3.5 text-right tabular-nums text-slate-200 align-top">{{ row.cantidad_citas }}</td>
                  <td class="px-3 py-3.5 text-right tabular-nums text-emerald-300/90 align-top">{{ row.completadas ?? '—' }}</td>
                  <td class="px-3 py-3.5 text-right tabular-nums text-rose-300/90 align-top">{{ row.canceladas ?? '—' }}</td>
                  <td class="px-3 py-3.5 text-right tabular-nums text-slate-300 align-top">{{ row.minutos_servicio ?? 0 }}</td>
                  <td class="px-4 xl:px-6 py-3.5 text-right font-bold text-cyan-200 tabular-nums align-top whitespace-nowrap">{{ fmtMoneda(row.total_importe) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div class="lg:hidden divide-y divide-white/5">
            <div v-for="(row, idx) in resumen_por_tecnico" :key="'m-' + idx" class="p-4 space-y-2">
              <p class="font-bold text-white">{{ row.tecnico_nombre }}</p>
              <div class="grid grid-cols-2 gap-2 text-xs text-slate-400">
                <span>Citas: <strong class="text-slate-200">{{ row.cantidad_citas }}</strong></span>
                <span>Complet.: <strong class="text-emerald-300">{{ row.completadas ?? '—' }}</strong></span>
                <span>Cancel.: <strong class="text-rose-300">{{ row.canceladas ?? '—' }}</strong></span>
                <span>Min. ∑: <strong class="text-slate-200">{{ row.minutos_servicio ?? 0 }}</strong></span>
              </div>
              <p class="text-sm font-black text-cyan-200">{{ fmtMoneda(row.total_importe) }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 sm:items-stretch">
          <div class="relative flex-1">
            ...
            <input
              ...
              class="w-full pl-11 pr-4 py-3 rounded-2xl bg-slate-900/80 text-slate-100 text-sm ring-1 ring-white/10 border-0 placeholder:text-slate-600 focus:ring-2 focus:ring-cyan-500/40"
            />
          </div>
        </div>

        <!-- Tabla detalle -->
        <div class="rounded-2xl sm:rounded-3xl bg-slate-900/40 ring-1 ring-white/10 overflow-hidden shadow-2xl w-full min-w-0">
          <div class="w-full min-w-0 overflow-x-auto lg:overflow-x-visible">
            <table class="w-full min-w-[920px] lg:min-w-0 lg:table-fixed text-left text-xs sm:text-sm border-collapse">
              <thead>
                <tr class="text-[10px] font-black uppercase tracking-widest text-slate-500 bg-slate-950/80 border-b border-white/10">
              ...
              <tbody>
                <template v-for="r in registrosFiltrados" :key="r.id">
                  <tr
                    class="border-b border-white/5 hover:bg-white/[0.04] align-top transition-colors cursor-pointer group"
                    @click="toggleRow(r.id)"
                  >
                    <td class="px-3 py-2.5 text-slate-500">
                      <button
                        type="button"
                        class="p-1 rounded-lg hover:bg-white/10 text-slate-400 group-hover:text-cyan-300 transition-colors"
                    ...
                    <td class="px-2 py-2.5 whitespace-nowrap font-mono text-cyan-200/90">{{ r.folio || '—' }}</td>
                    <td class="px-2 py-2.5 min-w-0 align-top break-words">
                      <div class="font-semibold text-slate-100 leading-snug">{{ r.tecnico_nombre }}</div>
                      <div v-if="r.tecnico_email" class="text-[11px] text-slate-500 break-all">{{ r.tecnico_email }}</div>
                    </td>
                    <td class="px-2 py-2.5 min-w-0 align-top break-words">
                      <div class="font-medium text-slate-100 leading-snug">{{ r.cliente || '—' }}</div>
                      <div v-if="r.cliente_telefono" class="text-[11px] text-slate-500">{{ r.cliente_telefono }}</div>
                    </td>
                    <td class="px-2 py-2.5 min-w-0 text-slate-400 text-[11px] leading-snug break-words hidden lg:table-cell align-top">
                      {{ r.direccion || '—' }}
                    </td>
                    <td class="px-2 py-2.5">
                      <div class="text-slate-200">{{ tipoServicioLegible(r.tipo_servicio) }}</div>
                      <div v-if="r.equipo" class="text-[11px] text-slate-500 mt-0.5 line-clamp-2">{{ r.equipo }}</div>
                    </td>
                    <td class="px-2 py-2.5 whitespace-nowrap">
                      <span class="inline-flex px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase bg-white/5 text-slate-300 ring-1 ring-white/10">
                        {{ prioridadLabel(r.prioridad) }}
                      </span>
                    </td>
                    <td class="px-2 py-2.5 whitespace-nowrap">
                      <span
                        class="inline-flex px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wide ring-1"
                        :class="estadoChip(r.estado)"
                      >
                        {{ estadoLegible(r.estado) }}
                      </span>
                    </td>
                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-300 tabular-nums">{{ fmtFecha(r.fecha_hora) }}</td>
                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-300 tabular-nums">
                      <div>{{ fmtFecha(r.inicio_servicio) }}</div>
                      <div class="text-slate-500 text-[11px]">{{ fmtFecha(r.fin_servicio) }}</div>
                    </td>
                    <td class="px-2 py-2.5 whitespace-nowrap text-slate-300 tabular-nums">{{ fmtMin(r.tiempo_servicio_minutos) }}</td>
                    <td class="px-2 py-2.5 whitespace-nowrap text-right font-bold text-white tabular-nums">{{ fmtMoneda(r.total) }}</td>
                    <td class="px-2 py-2.5 text-right align-top" @click.stop>
                      <div class="inline-flex flex-col sm:flex-row sm:items-center sm:justify-end gap-1.5 sm:gap-2">
                        <Link
                          ...
                          class="inline-flex items-center justify-end gap-1 text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-cyan-400 hover:text-cyan-300"
                        >
                          Ver
                        ...
                        <button
                          ...
                          type="button"
                          class="inline-flex items-center justify-end gap-1 rounded-lg px-2 py-1 text-[10px] sm:text-[11px] font-black uppercase tracking-wider bg-violet-600/20 text-violet-200 ring-1 ring-violet-500/35 hover:bg-violet-600/30"
                        ...
                        <button
                          ...
                          type="button"
                          class="inline-flex items-center justify-end gap-1 rounded-lg px-2 py-1 text-[10px] sm:text-[11px] font-black uppercase tracking-wider bg-indigo-600/20 text-indigo-200 ring-1 ring-indigo-500/35 hover:bg-indigo-600/30"
                        ...
                        <Link
                          ...
                          class="inline-flex items-center justify-end gap-1 rounded-lg px-2 py-1 text-[10px] sm:text-[11px] font-black uppercase tracking-wider bg-emerald-600/20 text-emerald-300 ring-1 ring-emerald-500/35 hover:bg-emerald-600/30 hover:text-emerald-200"
                      ...
                      <div class="grid md:grid-cols-2 gap-4 text-xs sm:text-sm">
                        <div class="space-y-3">
                          <div v-if="r.direccion" class="lg:hidden">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Ubicación</p>
                            <p class="text-slate-300 leading-relaxed">{{ r.direccion }}</p>
                          </div>
                          <div v-if="r.descripcion && (r.descripcion || '').trim() !== (r.problema_reportado || '').trim()">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Descripción</p>
                            <p class="text-slate-300 leading-relaxed whitespace-pre-wrap">{{ r.descripcion }}</p>
                          </div>
                          <div v-if="r.problema_reportado">
                            <p class="text-[10px] font-black uppercase tracking-widest text-amber-500/90 mb-1">Problema reportado</p>
                            <p class="text-slate-300 leading-relaxed whitespace-pre-wrap">{{ r.problema_reportado }}</p>
                          </div>
                        </div>
                        <div class="space-y-3">
                          <div v-if="r.trabajo_realizado">
                            <p class="text-[10px] font-black uppercase tracking-widest text-emerald-400/90 mb-1">Trabajo realizado</p>
                            <p class="text-slate-300 leading-relaxed whitespace-pre-wrap">{{ r.trabajo_realizado }}</p>
                          </div>
                          <div v-if="r.notas">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Notas</p>
                            <p class="text-slate-400 leading-relaxed whitespace-pre-wrap">{{ r.notas }}</p>
                          </div>
                          <div class="flex flex-wrap gap-x-6 gap-y-1 text-[11px] text-slate-500">
                            <span v-if="r.fecha_hora_fin">Fin programado: <span class="text-slate-300">{{ fmtFecha(r.fecha_hora_fin) }}</span></span>
                            <span>Actualizado: <span class="text-slate-300">{{ fmtFecha(r.actualizado_at) }}</span></span>
                            <span v-if="r.creado_at">Creado: <span class="text-slate-300">{{ fmtFecha(r.creado_at) }}</span></span>
                          </div>
                          <div v-if="$can('view ventas') && r.venta_id" class="pt-3 border-t border-white/10 flex flex-wrap gap-2">
                            <button
                              type="button"
                              class="inline-flex items-center gap-2 rounded-xl bg-violet-600/25 px-3 py-2 text-[11px] font-black uppercase tracking-wider text-violet-200 ring-1 ring-violet-500/40 hover:bg-violet-600/35"
                              @click="abrirDetalleVenta(r.venta_id)"
                            >
                              Ver venta{{ r.venta_numero ? ` ${r.venta_numero}` : '' }}
                            </button>
                          </div>
                          <div v-else-if="$can('view ventas')" class="pt-3 border-t border-white/10 flex flex-wrap gap-2">
                            <button
                              type="button"
                              class="inline-flex items-center gap-2 rounded-xl bg-indigo-600/25 px-3 py-2 text-[11px] font-black uppercase tracking-wider text-indigo-200 ring-1 ring-indigo-500/40 hover:bg-indigo-600/35"
                              @click="abrirModalVentaCita(r)"
                            >
                              Ventas del cliente / vincular
                            </button>
                          </div>
                          <div v-else-if="$can('create ventas')" class="pt-3 border-t border-white/10 flex flex-wrap gap-2">
                            <Link
                              :href="route('ventas.create', { cita_id: r.id })"
                              class="inline-flex items-center gap-2 rounded-xl bg-emerald-600/25 px-3 py-2 text-[11px] font-black uppercase tracking-wider text-emerald-200 ring-1 ring-emerald-500/40 hover:bg-emerald-600/35"
                            >
        ...
        <Teleport to="body">
          ...
          >
            <div
              class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-3xl w-full max-h-[88vh] overflow-hidden flex flex-col ring-1 ring-black/5 dark:ring-white/15"
              @click.stop
            >
              <div class="px-5 py-4 border-b border-slate-200 dark:border-white/10 flex items-start justify-between gap-3 shrink-0">
                <div class="min-w-0">
                  ...
                  <p class="text-xs text-slate-400 dark:text-slate-500 mt-2 leading-relaxed max-w-xl">
                    Elige una venta existente para vincularla a esta cita. Si no aparece el cobro, usa <strong class="text-slate-600 dark:text-slate-300">Crear venta</strong>.
                  </p>
                </div>
                <button
                  type="button"
                  class="shrink-0 p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10"
                  aria-label="Cerrar"
              ...
              <div class="shrink-0 px-4 py-3 border-b border-slate-200 dark:border-white/10 bg-emerald-50 dark:bg-emerald-950/40 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <p class="text-xs text-emerald-800 dark:text-slate-300 font-medium">
                ...
                <Link
                  ...
                  class="shrink-0 inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-widest shadow-md"
                  @click="cerrarModalVenta"
                >
                  Crear venta
                </Link>
              </div>
              <p v-if="modalVentaError" class="mx-4 mt-3 text-sm text-rose-600 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/50 ring-1 ring-rose-200 dark:ring-rose-500/30 rounded-lg px-3 py-2">
                {{ modalVentaError }}
              </p>
              <div class="overflow-y-auto flex-1 min-h-0 p-4">
                ...
                <table v-else-if="modalVentas.length" class="w-full text-sm">
                  <thead>
                    <tr class="text-left text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 border-b border-slate-200 dark:border-white/10">
                  ...
                  <tbody>
                    <tr v-for="v in modalVentas" :key="v.id" class="border-b border-slate-100 dark:border-white/5">
                      <td class="py-2.5 pr-2 font-mono font-bold text-amber-600 dark:text-amber-300/95">{{ v.numero_venta }}</td>
                      <td class="py-2.5 pr-2 text-slate-600 dark:text-slate-300">{{ fmtFechaVentaModal(v.fecha) }}</td>
                      <td class="py-2.5 pr-2 text-right font-bold text-slate-900 dark:text-white tabular-nums">{{ fmtMoneda(v.total) }}</td>
                      <td class="py-2.5 pr-2">
                        <span class="text-xs text-slate-700 dark:text-slate-300">{{ v.pagado ? 'Pagado' : 'Pendiente' }}</span>
                      ...
                      <td class="py-2.5 text-right">
                        <span
                          ...
                          class="inline-flex px-2 py-1 rounded-lg text-[10px] font-black uppercase bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-200 dark:ring-emerald-500/30"
                        >
                          En esta cita
                        </span>
                        <span
                          v-else-if="v.cita_id && Number(v.cita_id) !== Number(modalCita.id)"
                          class="inline-block text-[10px] font-bold text-amber-600 dark:text-amber-300/90 text-right max-w-[9rem] leading-snug"
                        >
                          Cita #{{ v.cita_id }}
                        </span>
                        <button
                          v-else
                          type="button"
                          class="px-3 py-1.5 rounded-lg text-xs font-black uppercase bg-emerald-600 text-white hover:bg-emerald-500 disabled:opacity-50 transition-colors"
                ...
                <div v-else class="py-10 text-center space-y-4">
                  ...
                  <Link
                    :href="route('ventas.create', { cita_id: modalCita.id })"
                    class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-widest shadow-md"
                    @click="cerrarModalVenta"
                  >
                    Crear venta
                  </Link>
              ...
              <div class="shrink-0 px-4 py-3 border-t border-slate-200 dark:border-white/10 flex flex-wrap justify-end gap-2 bg-slate-50 dark:bg-slate-950/80">
                <button
                  type="button"
        ...
        <Teleport to="body">
          <div
            ...
            class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md transition-all"
            @click.self="cerrarDetalleVenta"
          >
            <div
              class="bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col ring-1 ring-black/5 dark:ring-white/20"
              @click.stop
            >
              <!-- Header Modal -->
              <div class="px-8 py-6 border-b border-slate-100 dark:border-white/10 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-950/40">
                <div class="flex items-center gap-4">
                  <div class="w-12 h-12 rounded-2xl bg-violet-600/10 dark:bg-violet-600/20 flex items-center justify-center text-2xl text-violet-600 dark:text-violet-400 ring-1 ring-violet-500/20 dark:ring-violet-500/30">
                    💰
                  </div>
                  <div>
                    <h2 class="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Detalle de Venta</h2>
                    <p v-if="detalleVenta" class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">
                      Folio: <span class="text-amber-600 dark:text-amber-400/90 font-mono">{{ detalleVenta.numero_venta }}</span>
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  class="p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
              ...
              <!-- Content -->
              <div class="flex-1 overflow-y-auto p-8 min-h-0 space-y-8 custom-scrollbar bg-white dark:bg-slate-900">
                <div v-if="detalleVentaCargando" class="py-20 flex flex-col items-center justify-center space-y-4">
                  ...
                  <p class="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Cargando información...</p>
                </div>

                <div v-else-if="detalleVentaError" class="py-10 text-center space-y-4">
                  <div class="inline-flex w-16 h-16 rounded-full bg-rose-500/10 items-center justify-center text-rose-500 text-3xl">⚠️</div>
                  <p class="text-rose-600 dark:text-rose-300 font-bold">{{ detalleVentaError }}</p>
                  <button @click="cerrarDetalleVenta" class="text-xs font-black uppercase text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">Cerrar</button>
                </div>

                <div v-else-if="detalleVenta" class="space-y-8 animate-in fade-in duration-300">
                  <!-- Info Grid -->
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-slate-50 dark:bg-white/5 rounded-3xl p-5 ring-1 ring-slate-100 dark:ring-white/10 shadow-sm">
                      <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Cliente</p>
                    ...
                    <div class="bg-slate-50 dark:bg-white/5 rounded-3xl p-5 ring-1 ring-slate-100 dark:ring-white/10 shadow-sm">
                      <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Operación</p>
                      <div class="space-y-2 text-xs">
                        ...
                        <div class="flex justify-between">
                          ...
                          <span :class="detalleVenta.pagado ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'" class="font-black uppercase tracking-tighter">
                  ...
                  <!-- Conceptos -->
                  <div>
                    <h3 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-2">Conceptos de Venta</h3>
                    <div class="bg-white dark:bg-slate-950/40 rounded-3xl overflow-hidden ring-1 ring-slate-200 dark:ring-white/10 shadow-sm">
                      <table class="w-full text-xs text-left">
                        <thead class="bg-slate-50 dark:bg-white/5 text-[9px] font-black uppercase tracking-widest text-slate-500">
                          <tr>
                          ...
                          <tr v-for="item in detalleVenta.items" :key="item.id" class="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-5 py-3.5">
                              ...
                              <div v-if="item.series?.length" class="mt-2 flex flex-wrap gap-1">
                                <span v-for="s in item.series" :key="s" class="px-1.5 py-0.5 rounded bg-violet-100 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 font-mono text-[9px] border border-violet-200 dark:border-violet-500/20">
                                  {{ s }}
                                </span>
                              </div>
                            </td>
                  ...
                  <div class="flex justify-end pt-4 border-t border-slate-100 dark:border-white/5">
                    <div class="w-full max-w-[240px] space-y-2.5">
                      <div class="flex justify-between text-[11px] font-bold">
                        <span class="text-slate-400 dark:text-slate-500 uppercase tracking-widest">Subtotal</span>
                        <span class="text-slate-600 dark:text-slate-300">{{ fmtMoneda(detalleVenta.subtotal) }}</span>
                      </div>
                      <div v-if="detalleVenta.iva > 0" class="flex justify-between text-[11px] font-bold">
                        <span class="text-slate-400 dark:text-slate-500 uppercase tracking-widest">IVA (16%)</span>
                        <span class="text-slate-600 dark:text-slate-300">{{ fmtMoneda(detalleVenta.iva) }}</span>
                      </div>
                      <div v-if="detalleVenta.descuento_general > 0" class="flex justify-between text-[11px] font-bold">
                        <span class="text-rose-500 dark:text-rose-400 uppercase tracking-widest">Descuento</span>
                  ...
                  <!-- Notas -->
                  <div v-if="detalleVenta.notas" class="bg-amber-50 dark:bg-amber-400/5 rounded-2xl p-4 border border-amber-200 dark:border-amber-400/10">
                    <p class="text-[9px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-2">Observaciones</p>
                    <p class="text-xs text-slate-600 dark:text-slate-400 italic leading-relaxed">{{ detalleVenta.notas }}</p>
                  </div>
                </div>
              </div>

              <!-- Footer Modal -->
              <div class="px-8 py-5 border-t border-slate-100 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-slate-950/60 shrink-0">
                <button
                  type="button"
                  class="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                  @click="cerrarDetalleVenta"
                >
                  Cerrar
                </button>
                <div class="flex gap-3">
                  <a
                    ...
                    target="_blank"
                    class="px-5 py-2.5 rounded-xl bg-slate-200 dark:bg-white/5 hover:bg-slate-300 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest border border-slate-300 dark:border-white/10 transition-all shadow-sm"
                  >
                    PDF
                  </a>
                  <a
                    ...
                    target="_blank"
                    class="px-5 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-violet-900/20 transition-all active:scale-95"
                  >
