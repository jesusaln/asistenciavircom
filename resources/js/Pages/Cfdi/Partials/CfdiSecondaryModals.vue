...
<!-- Modal Descarga Masiva -->
    <Teleport to="body">
        <div v-if="showDescargaModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl p-10 animate-fadeIn">
                <button @click="closeDescarga" class="absolute top-6 right-6 text-gray-400 hover:text-gray-600 dark:text-gray-300">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>

                <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2">Descarga masiva SAT</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Selecciona dirección y rango de fechas. La descarga se ejecuta en segundo plano.</p>

                <div class="space-y-4">
                    <div class="flex gap-2">
                        <button @click="descargaForm.direccion = 'emitido'"
                                :class="['flex-1 py-3 rounded-xl font-bold text-sm', descargaForm.direccion === 'emitido' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600 dark:text-gray-300']">
                            Emitidos
                        </button>
                        <button @click="descargaForm.direccion = 'recibido'"
                                :class="['flex-1 py-3 rounded-xl font-bold text-sm', descargaForm.direccion === 'recibido' ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-600 dark:text-gray-300']">
                            Recibidos
                        </button>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        <button @click="setQuickRange(1)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:text-gray-300 rounded-lg text-[11px] font-bold uppercase tracking-widest">
                            Hoy
                        </button>
                        <button @click="setQuickRange(7)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:text-gray-300 rounded-lg text-[11px] font-bold uppercase tracking-widest">
                            Semana
                        </button>
                        <button @click="setQuickRange(15)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:text-gray-300 rounded-lg text-[11px] font-bold uppercase tracking-widest">
                            Quince dias
                        </button>
                        <button @click="setCurrentMonthRange()" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:text-gray-300 rounded-lg text-[11px] font-bold uppercase tracking-widest">
                            Mes
                        </button>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="date" v-model="descargaForm.fecha_inicio" class="w-full h-12 px-3 bg-white dark:bg-slate-900 border-2 border-transparent rounded-xl text-sm font-bold focus:bg-white dark:bg-slate-900 focus:border-blue-100 focus:ring-0 transition-all" />
                        </div>
                        <div>
                            <label class="text-[14px] font-black text-gray-400 uppercase tracking-[0.2em] pl-1">Fin</label>
                            <input type="date" v-model="descargaForm.fecha_fin" class="w-full h-12 px-3 bg-white dark:bg-slate-900 border-2 border-transparent rounded-xl text-sm font-bold focus:bg-white dark:bg-slate-900 focus:border-blue-100 focus:ring-0 transition-all" />
                        </div>
                    </div>
                </div>
    ...
    <Teleport to="body">
        <div v-if="showUploadModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg p-8 animate-fadeIn">
                <button @click="closeUpload" class="absolute top-6 right-6 text-gray-400 hover:text-gray-600 dark:text-gray-300">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
                
                <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2">Cargar CFDI Recibido</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Sube un archivo XML de un proveedor para agregarlo a tu archivo.</p>
                ...
                <div v-if="!uploadPreview"
                     ...
                     :class="['border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer', 
                              isDragging ? 'border-violet-500 bg-violet-50' : 'border-gray-200 dark:border-slate-800 hover:border-violet-300 hover:bg-white dark:bg-slate-900']"
                    ...
                    <div v-if="isUploading" class="flex flex-col items-center">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Procesando XML...</p>
                    </div>
                ...
                <!-- Preview -->
                <div v-else class="bg-white dark:bg-slate-900 rounded-2xl p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <p class="font-bold text-gray-900 dark:text-white truncate" :title="uploadPreview.data?.emisor?.nombre">{{ uploadPreview.data?.emisor?.nombre || 'N/A' }}</p>
                            <div class="flex items-center gap-1">
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ uploadPreview.data?.emisor?.rfc }}</p>
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <p class="font-mono text-xs text-gray-600 dark:text-gray-300 truncate">{{ uploadPreview.data?.uuid || 'N/A' }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Fecha</p>
                            <p class="font-bold text-gray-900 dark:text-white">{{ uploadPreview.data?.fecha || 'N/A' }}</p>
                        </div>
                        <div>
    ...
    <Teleport to="body">
        <div v-if="showReviewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full w-full max-h-[90vh] flex flex-col overflow-hidden animate-fadeIn">
                <!-- Header -->
                <div class="p-8 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-1">Revisar Documentos Descargados</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest">SAT - Staging Area</p>
                    </div>
                    <button @click="closeReview" class="w-10 h-10 flex items-center justify-center bg-white dark:bg-slate-900 text-gray-400 hover:text-gray-600 dark:text-gray-300 rounded-full transition-all">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-8 relative min-h-[300px]">
                    <div v-if="isLoadingReview" class="absolute inset-0 bg-white dark:bg-slate-900/80 flex flex-col items-center justify-center gap-4 z-10">
                        ...
                        <p class="text-sm font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">Cargando documentos...</p>
                    </div>

                    <div v-else-if="documentosStaging.length === 0 && duplicadosStaging.length === 0" class="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-gray-200 dark:border-slate-800">
                        <div class="w-20 h-20 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm border border-gray-100">
                        ...
                        <p class="text-lg font-black text-gray-900 dark:text-white">¡Nada más por aquí!</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Todos los documentos ya han sido importados.</p>
                    </div>
                            ...
                            <div class="grid gap-3">
                                <div v-for="doc in documentosStaging" :key="doc.id" 
                                     :class="['p-4 rounded-2xl border-2 transition-all flex items-center gap-4 group', 
                                              doc.importado ? 'bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-800 opacity-60 grayscale-[0.5]' :
                                              selectedStagingIds.includes(doc.id) ? 'bg-emerald-50 border-emerald-500 shadow-lg shadow-emerald-500/10' : 'bg-white dark:bg-slate-900 border-gray-100 hover:border-gray-200 dark:border-slate-800 hover:bg-white dark:bg-slate-900']">
                                ...
                                <div v-else @click="toggleSeleccionStaging(doc.id)"
                                     :class="['w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all cursor-pointer flex-shrink-0', 
                                             selectedStagingIds.includes(doc.id) ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 bg-white dark:bg-slate-900 group-hover:border-gray-400']">
                                ...
                                <div class="flex-1 grid grid-cols-1 md:grid-cols-12 gap-4 items-center" @click="!doc.importado && toggleSeleccionStaging(doc.id)">
                                    ...
                                    <div class="flex flex-col md:col-span-3 cursor-pointer">
                                        ...
                                        <span class="text-xs font-black text-gray-900 dark:text-white tabular-nums italic">{{ formatDateShort(doc.fecha_emision) }}</span>
                                    </div>
                                    ...
                                    <div class="flex flex-col md:col-span-5 cursor-pointer">
                                        ...
                                        <span class="text-xs font-bold text-gray-800 dark:text-gray-100 truncate" :title="doc.direccion === 'recibido' ? doc.nombre_emisor : doc.nombre_receptor">
                                    ...
                                    <div class="flex-1 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                        <div class="flex flex-col md:col-span-3">
                                            <div class="flex items-center gap-2 mb-1">
                                                ...
                                                <span class="text-[9px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-tighter truncate" :title="doc.uuid">{{ doc.uuid ? doc.uuid.substring(0, 8) : '--------' }}...</span>
                                            </div>
                                            <span class="text-xs font-black text-gray-900 dark:text-white tabular-nums italic">{{ formatDateShort(doc.fecha_emision) }}</span>
                                        </div>

                                        <div class="flex flex-col md:col-span-5">
                                            <span class="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-[0.1em] mb-0.5">
                                                {{ doc.direccion === 'recibido' ? 'Emisor' : 'Receptor' }}
                                            </span>
                                            <span class="text-xs font-bold text-gray-800 dark:text-gray-100 truncate" :title="doc.direccion === 'recibido' ? doc.nombre_emisor : doc.nombre_receptor">
                                                {{ doc.direccion === 'recibido' ? (doc.nombre_emisor || 'Desconocido') : (doc.nombre_receptor || 'Público General') }}
                                            </span>
                                            <span class="text-[9px] font-mono text-gray-500 dark:text-gray-400 capitalize">
                                                {{ doc.direccion === 'recibido' ? doc.rfc_emisor : doc.rfc_receptor }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="md:col-span-4 flex items-center justify-end gap-4">
                                            ...
                                            <div class="flex items-center gap-1">
                                                <button @click="verPdf(doc.uuid)" 
                                                        class="p-1.5 text-gray-500 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                                                        title="Ver PDF">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                                </button>
                                                <button @click="abrirModalCrearCuenta(doc)" 
                                                        class="p-1.5 text-gray-500 dark:text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors"
                                                        title="Crear Cuenta">
                                                ...
                                                <button @click="verXml(doc.uuid)" 
                                                        class="p-1.5 text-gray-500 dark:text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                                                        title="Ver XML">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
                                                </button>
                                            </div>
                ...
                <!-- Footer -->
                <div class="p-8 border-t border-gray-100 bg-white dark:bg-slate-900 flex items-center justify-between">
                    <p class="text-xs font-black text-gray-500 dark:text-gray-400">
                        {{ selectedStagingIds.length }} de {{ documentosStaging.filter(d => !d.importado).length }} pendientes seleccionados
                    </p>
                    <div class="flex gap-4">
                        <button @click="closeReview" class="px-6 py-3 bg-white dark:bg-slate-900 hover:bg-gray-100 text-gray-600 dark:text-gray-300 rounded-2xl font-black text-xs uppercase tracking-widest border border-gray-200 dark:border-slate-800 transition-all">
                            Cerrar
                        </button>
    ...
    <Teleport to="body">
        <div v-if="showDeleteConfirmModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-md p-10 animate-fadeIn text-center">
                ...
                <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2 tracking-tight italic">¿Estás seguro?</h3>
                ...
                <div v-if="cfdiParaEliminar" class="bg-white dark:bg-slate-900 rounded-2xl p-4 mb-8 text-left border border-gray-100">
                    <p class="text-[13px] font-black text-gray-400 uppercase tracking-widest mb-1">CFDI seleccionado</p>
                    <p class="text-sm font-black text-gray-800 dark:text-gray-100 tracking-tight italic">{{ cfdiParaEliminar.serie }}{{ cfdiParaEliminar.folio }}</p>
                ...
                <div class="flex gap-4">
                    <button @click="closeDelete" :disabled="isDeletingCfdi"
                            class="flex-1 px-4 py-4 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:text-gray-300 rounded-2xl font-black text-xs uppercase tracking-widest transition-all">
                        Cancelar
                    </button>
