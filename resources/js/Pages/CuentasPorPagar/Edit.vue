            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div class="flex items-center justify-between mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
                    <div class="flex items-center space-x-5">
                        <div class="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shadow-lg">
                    ...
                    <Link :href="route('cuentas-por-pagar.show', cuenta.id)" class="text-slate-500 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest px-4 py-2 bg-white/5 rounded-xl border border-white/5">
                        Volver
                    </Link>
                </div>
                ...
                <div class="bg-indigo-600/10 border border-indigo-500/20 rounded-[2rem] p-8 mb-10 backdrop-blur-sm">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                             <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Monto Total</p>
                             <p class="text-2xl font-black text-white">{{ formatCurrency(cuenta.monto_total) }}</p>
                        </div>
                        <div>
                             <p class="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">Pagado</p>
                             <p class="text-2xl font-black text-emerald-400">{{ formatCurrency(cuenta.monto_pagado) }}</p>
                        </div>
                        <div>
                             <p class="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1">Pendiente</p>
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-slate-900/50 border border-white/5 rounded-[2.5rem] p-10 backdrop-blur-sm shadow-2xl">
                        <form @submit.prevent="submit" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <input v-model="form.fecha_vencimiento" type="date"
                                           class="w-full bg-slate-800/80 border border-white/5 rounded-2xl px-6 py-5 text-white font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-inner" />
                                ...
                                <div class="flex flex-col justify-end">
                                     <div class="p-5 rounded-2xl bg-white/5 border border-white/5">
                                        ...
                                        <div :class="estadoClases" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest inline-block">
                                            {{ cuenta.estado }}
                                        </div>
                                     </div>
                                </div>
                            ...
                            <div>
                                ...
                                <textarea v-model="form.notas" rows="4"
                                          class="w-full bg-slate-800/50 border border-white/5 rounded-2xl px-6 py-5 text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none shadow-inner"
                            ...
                            <div class="flex justify-end gap-4 border-t border-white/5 pt-8">
                                ...
                                <button type="submit" :disabled="form.processing"
                                        class="px-10 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-black transition-all shadow-xl shadow-indigo-600/20 active:scale-95 disabled:opacity-50">
                    ...
                    <div v-if="toNumber(cuenta.monto_pendiente) > 0" class="bg-emerald-500/5 border border-emerald-500/10 rounded-[2.5rem] p-10 backdrop-blur-sm relative overflow-hidden group">
                        ...
                        <form @submit.prevent="registrarPago" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                            <div class="md:col-span-1">
                                ...
                                <input v-model="pagoForm.monto" type="number" step="0.01" :max="cuenta.monto_pendiente"
                                       class="w-full bg-slate-900/80 border border-white/5 rounded-xl px-4 py-3 text-emerald-400 font-bold focus:ring-1 focus:ring-emerald-500 outline-none" />
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <input v-model="pagoForm.notas" type="text"
                                       class="w-full bg-slate-900/80 border border-white/5 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-emerald-500 outline-none" />
                            </div>
                            <button type="submit" :disabled="pagoForm.processing || !pagoForm.monto"
                                    class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl transition-all shadow-lg shadow-emerald-600/20 disabled:opacity-30">
                                Registrar
                            </button>
                        </form>
                    </div>
...
const estadoClases = computed(() => {
    ...
    if (s === 'pagado') return 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30';
    if (s === 'parcial') return 'bg-amber-500/20 text-amber-400 border border-amber-500/30';
    if (s === 'vencido') return 'bg-rose-500/20 text-rose-400 border border-rose-500/30';
