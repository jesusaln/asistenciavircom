              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="bg-gray-50 dark:bg-slate-950/50 p-6 rounded-2xl border border-gray-100 dark:border-slate-800/50">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-100 dark:border-slate-800/50 pb-2">
                      <span class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Forma de pago</span>
                      <span class="text-xs font-black text-gray-900 dark:text-white text-right max-w-[60%]">{{ resumenFormaPago }}</span>
                    </div>
                    <div v-if="venta.forma_pago_sat || venta.metodo_pago_sat" class="flex justify-between items-center border-b border-gray-100 dark:border-slate-800/50 pb-2">
                      <span class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">SAT</span>
                      <span class="text-xs font-bold text-gray-700 dark:text-slate-300 text-right">{{ etiquetaSat }}</span>
                    </div>
                    <div v-if="venta.cuenta_bancaria" class="flex justify-between items-center">
                      <span class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Cuenta</span>
                      <span class="text-xs font-bold text-gray-800 dark:text-slate-200 text-right">{{ venta.cuenta_bancaria.banco }} — {{ venta.cuenta_bancaria.nombre }}</span>
                    </div>
                    <div v-if="venta.cliente?.forma_pago_default && !venta.metodo_pago" class="flex justify-between items-start gap-2 pt-2 border-t border-dashed border-gray-200 dark:border-slate-800/50">
                      <span class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest shrink-0">Cat. cliente</span>
                      <span class="text-[10px] font-bold text-gray-600 dark:text-slate-400 text-right">{{ textoFormaPagoClienteCatalogo(venta.cliente.forma_pago_default) }}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div :class="venta.pagado ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-800/30' : 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-100 dark:border-amber-800/30'"
            ...
            <div v-if="venta.pagado" class="space-y-4">
              <div class="bg-white/80 dark:bg-slate-900/40 p-4 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/20">
                <span class="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-1">Forma de pago (liquidación)</span>
                <span class="text-sm font-black text-emerald-600 dark:text-emerald-400 tracking-tight">{{ resumenFormaPago }}</span>
                <p v-if="etiquetaSat" class="text-[10px] font-bold text-gray-500 dark:text-slate-400 mt-2">{{ etiquetaSat }}</p>
              </div>
            ...
            <div v-else class="space-y-3">
              <div class="bg-white/80 dark:bg-slate-900/40 p-4 rounded-2xl border border-amber-200/60 dark:border-amber-800/30">
                <span class="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-1">Forma de pago acordada</span>
                <span class="text-sm font-black text-amber-800 dark:text-amber-300 tracking-tight">{{ resumenFormaPago }}</span>
                <p v-if="etiquetaSat" class="text-[10px] font-bold text-gray-500 dark:text-slate-400 mt-2">{{ etiquetaSat }}</p>
              </div>
              <p class="text-xs font-bold text-amber-700 dark:text-amber-500 uppercase leading-relaxed text-center p-4 bg-amber-100/30 dark:bg-amber-900/10 rounded-2xl border border-amber-200/50 dark:border-amber-800/30">
        ...
        <template #content>
          <div class="space-y-6 pt-4">
             ...
             <div v-if="usuariosCobroLista.length">
                <label class="block text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">¿Quién recibió el dinero?</label>
                <p class="text-xs text-gray-500 dark:text-slate-400 mb-2">Afecta corte de caja y reportes.</p>
                <select v-model="pagadoPorUserId" class="w-full bg-white dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-gray-900 dark:text-white focus:border-blue-500 transition-all outline-none">
