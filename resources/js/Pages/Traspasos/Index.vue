...
<template>
              ...
              <tr v-for="traspaso in traspasos.data" :key="traspaso.id" class="hover:bg-gray-50">
                  ...
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-10 w-10">
                      <div class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                        <span v-if="traspaso.productos_count > 1" class="text-xs font-bold text-gray-600">{{ traspaso.productos_count }}</span>
                    ...
                    <div class="ml-4">
                      <div v-if="traspaso.productos && traspaso.productos.length > 1" class="text-sm font-medium text-gray-900">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <div class="flex items-center gap-2">
                    <button type="button" @click="verTraspaso(traspaso)" class="p-2 rounded-full text-blue-600 hover:bg-blue-50" title="Ver">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    </button>
                    <Link :href="route('traspasos.edit', traspaso.id)" class="p-2 rounded-full text-amber-600 hover:bg-amber-50" title="Editar">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    </Link>
                    <button @click="confirmarEliminar(traspaso)" class="p-2 rounded-full text-red-600 hover:bg-red-50" title="Eliminar">
  ...
  <div v-if="mostrarModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" @click.self="cerrarModal">
    <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 space-y-4">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-xl font-bold text-gray-900">Traspaso #{{ traspasoSeleccionado?.id }}</h2>
          <p class="text-sm text-gray-600">Fecha: {{ formatDate(traspasoSeleccionado?.created_at) }}</p>
        </div>
      ...
      <div class="border border-gray-200 rounded-lg overflow-hidden">
        <div class="bg-gray-100 px-4 py-2 border-b">
          <p class="text-sm font-semibold text-gray-700">Productos Traspasados ({{ traspasoSeleccionado?.productos?.length || 1 }})</p>
        </div>
        <div class="divide-y divide-gray-200">
          <div v-if="traspasoSeleccionado?.productos && traspasoSeleccionado.productos.length" v-for="(prod, idx) in traspasoSeleccionado.productos" :key="idx" class="px-4 py-3 flex justify-between items-center">
            ...
            <span class="text-sm font-semibold text-blue-600">{{ prod.cantidad }} unid.</span>
          </div>
          <div v-else class="px-4 py-3 flex justify-between items-center">
            <span class="text-sm text-gray-900">{{ traspasoSeleccionado?.producto?.nombre || 'N/D' }}</span>
            <span class="text-sm font-semibold text-blue-600">{{ traspasoSeleccionado?.cantidad || traspasoSeleccionado?.cantidad_total }} unid.</span>
          </div>
        </div>
      </div>
  ...
  <div v-if="mostrarModalEliminar" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" @click.self="cerrarModalEliminar">
    <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 space-y-4">
      <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-900">Eliminar traspaso</h2>
        <button @click="cerrarModalEliminar" class="text-gray-500 hover:text-gray-700">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
      </div>
