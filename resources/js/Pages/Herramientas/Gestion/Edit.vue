...
<template>
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
    ...
    <div class="flex flex-wrap gap-3">
      <Link class="px-4 py-2 bg-white/[0.05] text-slate-300 font-semibold rounded-xl hover:bg-white/[0.1] hover:text-white border border-white/10 transition-all duration-300 flex items-center shadow-lg text-sm" :href="route('herramientas.gestion.index')">
      ...
      <Link class="px-4 py-2 bg-white/[0.05] text-slate-300 font-semibold rounded-xl hover:bg-white/[0.1] hover:text-white border border-white/10 transition-all duration-300 flex items-center shadow-lg text-sm" :href="route('herramientas.dashboard')">
        <svg class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
        Dashboard
      </Link>
      <Link
        class="px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-semibold rounded-xl hover:from-emerald-500 hover:to-emerald-400 transition-all duration-300 shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:shadow-[0_0_20px_rgba(16,185,129,0.5)] flex items-center text-sm"
  ...
  <div class="bg-slate-900/60 backdrop-blur-md rounded-2xl shadow-xl border border-white/[0.08] p-6 mb-8 relative overflow-hidden group">
    <div class="absolute top-0 right-0 w-48 h-48 bg-blue-500/10 rounded-full blur-3xl -mr-20 -mt-20 group-hover:bg-blue-500/20 transition-all duration-700 pointer-events-none"></div>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-3 text-sm text-slate-400 mt-1 font-medium">
      ...
      <div class="text-right flex items-center gap-4 md:block">
        <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-left md:text-right">Herramientas asignadas</div>
  ...
  <form @submit.prevent="submit" class="grid lg:grid-cols-2 gap-6">
    ...
    <div class="bg-slate-900/60 backdrop-blur-md rounded-2xl shadow-xl border border-white/[0.08] flex flex-col h-full overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div class="relative w-full sm:w-auto">
            ...
            <input
              ...
              type="search"
              placeholder="Buscar..."
              class="w-full bg-slate-950/50 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-colors"
            />
          </div>
        </div>
      </div>
      <div class="p-6 overflow-y-auto max-h-[500px] custom-scrollbar space-y-3">
        ...
        <label v-for="herramienta in herramientasAsignadasFiltradas" :key="`a-${herramienta.id}`" class="flex items-center gap-4 p-3 border border-white/[0.05] rounded-xl transition-all duration-300 bg-slate-800/20 hover:bg-slate-800/80 hover:border-blue-500/30 cursor-pointer group/item">
          <input type="checkbox" :value="herramienta.id" v-model="form.asignadas" class="sr-only peer" />
          <div class="w-5 h-5 rounded border-2 border-slate-600 peer-checked:bg-blue-500 peer-checked:border-blue-500 transition-colors flex items-center justify-center">
          ...
          <img v-if="herramienta.foto" :src="`/storage/${herramienta.foto}`" alt="Foto" class="w-10 h-10 object-cover rounded shadow-inner" />
          <div v-else class="w-10 h-10 bg-slate-900 rounded border border-white/10 flex items-center justify-center">
          ...
          <div class="flex-1 truncate">
            ...
            <div class="flex items-center gap-2 mt-1">
              ...
              <span :class="['text-[9px] font-bold uppercase px-1.5 rounded border', getEstadoClasses(herramienta.estado)]">
                {{ herramienta.estado }}
              </span>
            </div>
          </div>
          <button @click.stop.prevent="abrirModalReasignacion(herramienta)" class="px-3 py-1.5 text-[10px] font-black bg-blue-600/10 hover:bg-blue-600 text-blue-400 hover:text-white rounded-lg transition-all border border-blue-500/20">
            MOVILIZAR
          </button>
        </label>
      </div>
    ...
    <div class="bg-slate-900/60 backdrop-blur-md rounded-2xl shadow-xl border border-white/[0.08] flex flex-col h-full overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div class="relative w-full sm:w-auto">
            ...
            <input
              ...
              type="search"
              placeholder="Buscar..."
              class="w-full bg-slate-950/50 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
            />
          </div>
        </div>
      </div>
      <div class="p-6 overflow-y-auto max-h-[500px] custom-scrollbar space-y-3">
        ...
        <label v-for="herramienta in herramientasDisponiblesFiltradas" :key="`d-${herramienta.id}`" class="flex items-center gap-4 p-3 border border-white/[0.05] rounded-xl transition-all duration-300 bg-slate-800/40 hover:bg-slate-800/80 hover:border-emerald-500/30 cursor-pointer group/item">
          <input type="checkbox" :value="herramienta.id" v-model="form.asignadas" class="sr-only peer" />
          <div class="w-5 h-5 rounded border-2 border-slate-600 peer-checked:bg-emerald-500 peer-checked:border-emerald-500 transition-colors flex items-center justify-center">
            <svg class="w-3 h-3 text-white opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg>
          </div>
          <img v-if="herramienta.foto" :src="`/storage/${herramienta.foto}`" alt="Foto" class="w-10 h-10 object-cover rounded shadow-inner" />
          <div v-else class="w-10 h-10 bg-slate-900 rounded border border-white/10 flex items-center justify-center">
          ...
          <span class="text-[10px] font-black text-emerald-500/80 bg-emerald-500/5 px-2 py-1 rounded-lg border border-emerald-500/10">STOCK</span>
        </label>
      </div>
    </div>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
        ...
        <div class="flex items-center gap-4 w-full sm:w-auto">
          ...
          <button
            ...
            type="submit"
            class="flex-1 sm:flex-none px-8 py-2.5 bg-gradient-to-r from-blue-600 to-blue-500 text-white font-black rounded-xl hover:from-blue-500 hover:to-blue-400 shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50"
          >
  ...
  <div v-if="showReasignarForm" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
    <div class="bg-slate-900 w-full max-w-md rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
      <div class="p-6 border-b border-white/[0.05] bg-slate-800/30 flex justify-between items-center">
        <h2 class="text-xl font-black text-white tracking-tight leading-none uppercase tracking-widest">Movilizar Equipo</h2>
      ...
      <div class="p-6">
        <div v-if="herramientaAReasignar" class="space-y-6">
          <div class="bg-slate-950 px-4 py-3 rounded-xl border border-white/5">
            <p class="text-[10px] font-black text-slate-600 uppercase mb-1">Equipo Seleccionado</p>
          ...
          <div>
            ...
            <select v-model="nuevoTecnicoId" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-blue-500">
          ...
          <div>
             ...
             <textarea v-model="observacionesReasignacion" rows="3" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-blue-500 resize-none" placeholder="Opcional..."></textarea>
          </div>
          ...
          <div class="flex gap-4 pt-2">
            ...
            <button @click="reasignarHerramienta" class="flex-1 py-3 bg-emerald-600 text-white font-black rounded-xl hover:bg-emerald-500 shadow-lg shadow-emerald-500/20 transition-all uppercase text-xs tracking-wider">EJECUTAR CAMBIO</button>
          </div>
        </div>
      </div>
    </div>
