...
<template>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-1">
      <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border p-6 sticky top-6">
    ...
    <div class="lg:col-span-3">
      <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border">
        ...
        <div class="p-6 border-b">
          ...
          <p class="text-gray-600 dark:text-gray-300">{{ reportes.find(r => r.id === reporteSeleccionado)?.descripcion }}</p>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Generado el: {{ formatDate(new Date()) }}</p>
        </div>
        ...
        <div class="p-6">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                  <thead class="bg-white dark:bg-slate-900">
                    <tr>
                      ...
                      <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Último Mant.</th>
          ...
          <div v-if="reporteSeleccionado === 'mantenimiento'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="text-center p-4 bg-red-50 rounded-lg">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-300">Requieren Mant. Urgente</div>
              </div>
              <div class="text-center p-4 bg-orange-50 rounded-lg">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-300">Próximo Mes</div>
              </div>
              <div class="text-center p-4 bg-green-50 rounded-lg">
                <div class="text-2xl font-bold text-green-600">{{ herramientas.filter(h => !h.requiere_mantenimiento).length }}</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">Al Día</div>
              </div>
            </div>
          ...
          <div v-if="reporteSeleccionado === 'uso'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="text-center p-4 bg-blue-50 rounded-lg">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-300">Actualmente Asignadas</div>
              </div>
              <div class="text-center p-4 bg-purple-50 rounded-lg">
                <div class="text-2xl font-bold text-purple-600">{{ estadisticas.total_asignaciones || 0 }}</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">Total Asignaciones</div>
              </div>
              <div class="text-center p-4 bg-green-50 rounded-lg">
                <div class="text-2xl font-bold text-green-600">{{ estadisticas.promedio_dias_uso || 0 }}</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">Promedio Días Uso</div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-3">
                <div v-for="herramienta in herramientas.slice(0, 10)" :key="herramienta.id" class="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-lg">
                  <div>
                  ...
                  <div class="text-right">
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-300">{{ herramienta.estado }}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="['categoria', 'vida_util'].includes(reporteSeleccionado)" class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Reporte en Desarrollo</h3>
            <p class="text-gray-600 dark:text-gray-300">Esta funcionalidad estará disponible próximamente.</p>
          </div>
        </div>
      </div>
    </div>
