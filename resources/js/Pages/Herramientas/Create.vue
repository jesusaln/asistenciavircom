    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Nueva Herramienta</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Registra un nuevo activo en el sistema</p>
          </div>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="p-8 -mt-4 relative">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                  ...
                  <input v-model="form.nombre" type="text" placeholder="Ej: Rotomartillo SDS Max 1500W" class="w-full px-4 py-3 rounded-2xl bg-[var(--ui-surface)]/50 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all" required />
                ...
                <div>
                  ...
                  <input v-model="form.numero_serie" type="text" placeholder="SN-123456789" class="w-full px-4 py-3 rounded-2xl bg-[var(--ui-surface)]/50 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-mono text-sm focus:ring-2 focus:ring-brand-500 transition-all" />
                </div>
                ...
                <div class="md:col-span-2">
                  ...
                  <textarea v-model="form.descripcion" rows="4" placeholder="Especificaciones técnicas, accesorios incluidos, etc." class="w-full px-4 py-3 rounded-2xl bg-[var(--ui-surface)]/50 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-500 transition-all"></textarea>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 p-8 transition-all duration-200">
            ...
            <div class="relative aspect-square rounded-3xl overflow-hidden bg-[var(--ui-surface)]/50 border-2 border-dashed border-slate-200 dark:border-slate-700 group transition-all">
              ...
              <div v-else class="w-full h-full flex flex-col items-center justify-center text-slate-400 gap-4">
                ...
                <p class="text-[10px] font-bold uppercase tracking-wide opacity-50">Sin Vista Previa</p>
              </div>
              <button v-if="fotoPreview" @click.prevent="removeImage" class="absolute top-4 right-4 w-10 h-10 bg-brand-500 text-white rounded-2xl shadow-xl flex items-center justify-center hover:bg-rose-600 transition-all">✕</button>
            </div>
