...
<template>
  <Head title="Usuarios" />
  <div class="usuarios-index min-h-screen bg-white dark:bg-slate-900">
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
            <thead class="bg-white dark:bg-slate-900">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Rol</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Estado</th>
                <th v-if="$can('edit usuarios') || $can('delete usuarios') || $can('view usuarios')" class="px-6 py-4 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Acciones</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-800">
              <tr v-for="usuario in usuariosDocumentos" :key="usuario.id" class="hover:bg-white dark:bg-slate-900 transition-colors duration-150">
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-900 dark:text-white">{{ formatearFecha(usuario.fecha) }}</div>
                </td>
                <td class="px-6 py-4">
                  ...
                  <div class="flex flex-wrap gap-1">
                    <span v-for="rol in usuario.raw.roles" :key="rol.id" 
                          ...
                          :class="{
                            ...
                            'bg-gray-100 text-gray-800 dark:text-gray-100': !['admin', 'vendedor', 'cobranza', 'tecnico', 'almacenista', 'compras'].includes(rol.name)
                          }">
                      {{ rol.name }}
                    </span>
                    <span v-if="!usuario.raw.roles || usuario.raw.roles.length === 0" class="text-xs text-gray-500 dark:text-gray-400 italic">
                      Sin rol
                    </span>
                  </div>
                </td>
                ...
                <td colspan="6" class="px-6 py-16 text-center">
                  <div class="flex flex-col items-center space-y-4">
                    ...
                    <div class="space-y-1">
                      ...
                      <p class="text-sm text-gray-500 dark:text-gray-400">Los usuarios aparecerán aquí cuando se creen</p>
                    </div>
                  </div>
                </td>
              </tr>
          ...
          <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-3">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900 dark:text-white bg-white dark:bg-slate-900 px-3 py-2 rounded-md">{{ selectedUsuario.name }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Email</label>
                      <p class="mt-1 text-sm text-gray-900 dark:text-white bg-white dark:bg-slate-900 px-3 py-2 rounded-md">{{ selectedUsuario.email || 'N/A' }}</p>
                    </div>
                    <div>
                  ...
                  <div class="space-y-3">
                    <div>
                      ...
                      <div class="mt-1 flex flex-wrap gap-1">
                        <span v-for="rol in selectedUsuario.roles" :key="rol.id" 
                              class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 dark:text-gray-100">
                          {{ rol.name }}
                        </span>
                        <span v-if="!selectedUsuario.roles || selectedUsuario.roles.length === 0" class="text-sm text-gray-500 dark:text-gray-400 italic px-3 py-2 bg-white dark:bg-slate-900 rounded-md block w-full">
                          Sin rol asignado
                        </span>
                      </div>
                    </div>
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900 dark:text-white bg-white dark:bg-slate-900 px-3 py-2 rounded-md">{{ formatearFecha(selectedUsuario.created_at) }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Última actualización</label>
                      <p class="mt-1 text-sm text-gray-900 dark:text-white bg-white dark:bg-slate-900 px-3 py-2 rounded-md">{{ formatearFecha(selectedUsuario.updated_at) }}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-if="modalMode === 'confirm'">
              <div class="text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">¿Eliminar Usuario?</h3>
