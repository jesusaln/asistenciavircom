        ...
        <div class="py-6">
            <div class="w-full px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Dashboard de Pólizas</h1>
                        <p class="text-gray-600 dark:text-gray-300 mt-1">Control financiero y alertas operativas</p>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
                    ...
                    <div :class="['rounded-xl p-4 shadow-lg', stats.cobros_pendientes > 0 ? 'bg-gradient-to-br from-red-500 to-rose-600 text-white animate-pulse' : 'bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-800 text-gray-600 dark:text-gray-300']">
                        <div :class="['text-3xl font-black', stats.cobros_pendientes > 0 ? 'text-white' : 'text-gray-400']">
                            {{ formatCurrency(stats.cobros_pendientes) }}
                        </div>
                        <div :class="['text-xs font-semibold uppercase tracking-wider', stats.cobros_pendientes > 0 ? 'text-red-100' : 'text-gray-500 dark:text-gray-400']">
                            Cobros Pendientes
                        </div>
                ...
                <div v-if="stats.polizas_con_deuda > 0 || stats.con_exceso_horas > 0 || stats.con_exceso_tickets > 0" class="mb-6 p-4 bg-red-50 border-2 border-red-200 rounded-xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div v-if="stats.polizas_con_deuda > 0" class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-red-200">
                        ...
                        <div v-if="stats.con_exceso_horas > 0" class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-purple-200">
                        ...
                        <div v-if="stats.con_exceso_tickets > 0" class="bg-white dark:bg-slate-900 rounded-lg p-3 border border-orange-200">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="divide-y max-h-96 overflow-y-auto">
                            <div v-for="p in proximasVencer" :key="p.id" class="px-6 py-4 hover:bg-white dark:bg-slate-900 transition">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <Link :href="route('polizas-servicio.show', p.id)" class="font-bold text-gray-900 dark:text-white hover:text-blue-600">
                                            {{ p.nombre }}
                                        </Link>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ p.cliente }}</div>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="divide-y max-h-96 overflow-y-auto">
                            <div v-for="p in excesoHoras" :key="p.id" class="px-6 py-4 hover:bg-white dark:bg-slate-900 transition">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="divide-y max-h-96 overflow-y-auto">
                            <div v-for="p in excesoTickets" :key="p.id" class="px-6 py-4 hover:bg-white dark:bg-slate-900 transition">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <Link :href="route('polizas-servicio.show', p.id)" class="font-bold text-gray-900 dark:text-white hover:text-blue-600">
                                            {{ p.nombre }}
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b bg-gradient-to-r from-blue-50 to-cyan-50">
                            <h2 class="font-bold text-gray-900 dark:text-white">📊 Top Consumo Mensual</h2>
                        </div>
                        <div class="divide-y max-h-96 overflow-y-auto">
                            <div v-for="(p, index) in topConsumo" :key="p.id" class="px-6 py-3 hover:bg-white dark:bg-slate-900 transition">
                                <div class="flex justify-between items-center mb-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <Link :href="route('polizas-servicio.show', p.id)" class="font-semibold text-gray-900 dark:text-white hover:text-blue-600 text-sm">
                                            {{ p.nombre }}
                                        </Link>
                                    </div>
                                    <span class="text-sm font-bold text-gray-600 dark:text-gray-300">{{ p.horas_usadas }}h</span>
                                </div>
                                <div class="ml-9">
                ...
                <div class="mt-6 bg-white dark:bg-slate-900 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b bg-gradient-to-r from-green-50 to-emerald-50">
                        <h2 class="font-bold text-gray-900 dark:text-white">💰 Últimos Cobros Generados</h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-slate-800">
                                <tr v-for="c in ultimosCobros" :key="c.id" class="hover:bg-white dark:bg-slate-900">
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ c.fecha }}</td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ c.cliente }}</td>
                                    <td class="px-6 py-4 text-sm font-bold text-gray-900 dark:text-white text-right">{{ formatCurrency(c.monto) }}</td>
