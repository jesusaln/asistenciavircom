<template>
  <div class="citas-wizard min-h-screen bg-[var(--ui-surface)] transition-colors duration-500">
        ...
        <div class="flex flex-col py-4">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Editar <span class="text-brand-500 dark:text-amber-400">Cita #{{ cita.id }}</span></h1>
      ...
      <form @submit.prevent="submit" class="relative min-h-[500px]">
        ...
        >
            ...
            <div class="section-card glass-morphism p-8 md:p-12">
               ...
               <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                  ...
                  <div class="space-y-12">
                     ...
                     <!-- Estado de la Cita -->
                     <div class="space-y-6">
                        <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide pl-1">Estado de la Cita</label>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <button 
                                v-for="opt in estadoOptions" :key="opt.value"
                                type="button"
                                @click="form.estado = opt.value"
                                class="py-4 px-2 rounded-2xl border-2 font-bold text-[10px] sm:text-xs uppercase tracking-wider transition-all duration-300 active:scale-95 text-center"
                                :class="[
                                    form.estado === opt.value 
                                        ? getStatusClass(opt.value) + ' border-transparent shadow-xl shadow-sky-500/10' 
                                        : 'bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 text-slate-400 hover:border-brand-500'
                                ]"
                            >
                                {{ opt.text }}
                            </button>
                  ...
                  <div class="space-y-12">
                     ...
                     <div class="space-y-6">
                        ...
                        <p v-if="form.errors.tipo_servicio" class="text-xs text-rose-500 mt-1 font-bold">{{ form.errors.tipo_servicio }}</p>
                     </div>
               ...
               <div class="space-y-10">
                    ...
                    <div class="grid grid-cols-1 gap-8">
                        <FormField 
                            ...
                            :label="form.tipo_servicio === 'instalacion' ? 'Detalles del Servicio / Instalación (Visible para el Cliente)' : 'Descripción de la Falla (Visible para el Cliente)'" 
                            type="textarea" 
                            id="descripcion" 
                            :rows="15" 
                        ...
                        <div class="space-y-4">
                            ...
                            <!-- Imágenes existentes -->
                            <div v-if="cita.evidencias_previas?.length > 0" class="flex flex-wrap gap-4 mb-4">
                                <div v-for="(foto, idx) in cita.evidencias_previas" :key="idx" class="relative w-24 h-24 rounded-2xl overflow-hidden shadow-sm group cursor-pointer" @click="openGallery(['/storage/' + foto], 'Evidencia Previa #' + (idx+1))">
                            ...
                            <div class="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[32px] hover:border-brand-500 dark:hover:border-brand-500 transition-all group relative overflow-hidden">
                                <input 
                                    ...
                                    multiple 
                                    accept="image/*"
                                    @change="(e) => handleEvidenciasPrevias(e)"
                                ...
                                <div class="text-center">
                                    ...
                                    <p class="text-sm font-bold text-slate-600 dark:text-slate-300">Añadir más imágenes del cliente</p>
                            ...
                            <!-- Preview de nuevas imágenes del cliente -->
                            <div v-if="previewEvidenciasPrevias.length > 0" class="flex flex-wrap gap-3 mt-4">
                                <div v-for="(preview, idx) in previewEvidenciasPrevias" :key="idx" class="relative w-20 h-20 rounded-xl overflow-hidden shadow-md">
                                    <img :src="preview" class="w-full h-full object-cover" />
                                    <button @click.prevent="removeEvidenciaPrevia(idx)" class="absolute top-1 right-1 bg-rose-500 text-white rounded-full p-1 shadow-lg">
          ...
          <div v-if="['en_proceso', 'completado'].includes(form.estado)" class="section-card glass-morphism p-8 md:p-12 mt-8">
               <div class="flex items-center gap-5 mb-12">
                <div class="w-16 h-16 rounded-[24px] bg-brand-500 flex items-center justify-center text-white shadow-xl shadow-emerald-200 dark:shadow-none transition-transform hover:rotate-3">
              ...
              <div class="space-y-6">
                 ...
                 <div class="space-y-6">
                    <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide transition-colors">Evidencias Visuales Existentes</label>
                    <div class="grid grid-cols-3 sm:grid-cols-6 gap-4">
                       ...
                       <div v-for="(foto, idx) in cita.fotos_finales" :key="idx" class="aspect-square rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-700 border border-slate-100 dark:border-slate-700 relative group cursor-pointer" @click="openGallery(['/storage/' + foto], 'Evidencia #' + (idx+1))">
                          <img :src="'/storage/' + foto" class="w-full h-full object-cover group-hover:scale-105 transition-transform">
                          <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                       ...
                       <!-- Subida de Nuevas -->
                       <label for="new_photos" class="aspect-square rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 dark:hover:bg-black/50 transition-all group">
                          <svg class="w-10 h-10 text-slate-300 group-hover:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    ...
                    <div v-if="previewNewPhotos.length > 0" class="flex flex-wrap gap-3 p-4 bg-[var(--ui-surface)] dark:bg-slate-800/30 rounded-2xl animate-in fade-in">
                       <div v-for="(preview, idx) in previewNewPhotos" :key="idx" class="relative group w-16 h-16 rounded-xl overflow-hidden border-2 border-sky-200 dark:border-sky-800/30 dark:border-blue-900/30">
                          <img :src="preview" class="w-full h-full object-cover">
                          <button type="button" @click="removeNewPhoto(idx)" class="absolute top-1 right-1 bg-brand-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                       </div>
                    </div>
                 </div>
                 
                 <div class="flex flex-col justify-end" v-if="cita.ticket_id && form.estado === 'completado'">
                       <div class="p-4 bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800/30 rounded-xl flex items-center gap-4">
                         <input type="checkbox" v-model="form.cerrar_ticket" id="cerrar_ticket" class="w-4 h-4 rounded-xl border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-brand-500">
    ...
    <div v-if="showGalleryModal" class="fixed inset-0 bg-black/95 z-[60] flex items-center justify-center p-6" @click.self="showGalleryModal = false">
       ...
       <img :src="galleryImages[0]" class="max-h-[85vh] max-w-[90vw] object-contain rounded-2xl shadow-xl animate-in zoom-in-95" :key="galleryImages[0]">
    </div>
...
const form = useForm({
    ...
    ayudante_id: props.cita.ayudante_id || '',
    poliza_id: '',
    ...
    cerrar_ticket: false,
    notas_internas: props.cita.notas_internas || '',
    evidencias_previas: [],
    nuevas_fotos: [],
...
const selectedAyudanteName = computed(() => {
    const t = props.tecnicos.find((tecnico) => String(tecnico.id) === String(form.ayudante_id));
    return t ? getTecnicoDisplayName(t) : '';
});
...
const previewEvidenciasPrevias = ref([]);
const handleEvidenciasPrevias = (e) => {
    const files = Array.from(e.target.files);
    files.forEach(file => {
        form.evidencias_previas.push(file);
        const reader = new FileReader();
        reader.onload = (e) => previewEvidenciasPrevias.value.push(e.target.result);
        reader.readAsDataURL(file);
    });
};

const removeEvidenciaPrevia = (idx) => {
    form.evidencias_previas.splice(idx, 1);
    previewEvidenciasPrevias.value.splice(idx, 1);
};
...
const getPriorityClass = (p) => {
    switch(p) {
        ...
        case 'alta': return 'bg-brand-500 text-white';
...
const getStatusClass = (s) => {
    switch(s) {
        case 'pendiente': return 'bg-slate-600 text-white';
        case 'programado': return 'bg-blue-600 text-white';
        case 'en_proceso': return 'bg-brand-500 text-white';
