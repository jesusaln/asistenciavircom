    ...
    <div class="space-y-6">
        ...
        <div v-if="sessions.length > 0" class="space-y-6">
            <div v-for="(session, i) in sessions" :key="i" class="flex items-center p-4 bg-white/5 border border-white/5 rounded-2xl hover:border-white/10 transition-all">
                <div class="w-10 h-10 flex items-center justify-center bg-[#0b0f19] rounded-xl text-slate-500">
                    <svg v-if="session.agent.is_desktop" class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="ms-4 flex-1">
                    ...
                    <div class="text-xs text-slate-500 font-medium">
                        {{ session.ip_address }},
                        <span v-if="session.is_current_device" class="text-emerald-400 font-black uppercase tracking-wide">Este dispositivo</span>
        ...
        <div class="flex items-center pt-4 border-t border-white/5">
            <button @click="confirmLogout" class="h-10 px-6 bg-white hover:bg-[#ff6600] hover:text-white text-[#0b0f19] font-black uppercase tracking-wide text-xs rounded-2xl transition-all shadow-xl">
                Cerrar sesión en otros dispositivos
            </button>

            <ActionMessage :on="form.recentlySuccessful" class="ms-4 text-emerald-400 font-bold text-xs uppercase tracking-wide">
                ¡Listo!
            </ActionMessage>
        </div>
        ...
        <DialogModal :show="confirmingLogout" @close="closeModal">
            ...
            <template #footer>
                <div class="flex gap-3">
                    ...
                    <button
                        class="h-12 px-8 bg-[#ff6600] text-black font-black uppercase tracking-wide text-xs rounded-2xl hover:bg-white transition-all shadow-xl shadow-[#ff6600]/10"
