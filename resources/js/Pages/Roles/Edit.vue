      ...
      <div class="w-full sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded p-6">
          <div class="mb-8 border-b pb-4">
            <h2 class="text-2xl font-bold text-slate-900">Editar Rol</h2>
            <p class="text-sm text-slate-500">Modifica el nombre del rol y gestiona los niveles de acceso de forma detallada.</p>
          </div>

          <form @submit.prevent="submit">
            <!-- Nombre del Rol -->
            <div class="mb-8 max-w-md">
              <label for="name" class="block text-sm font-medium text-slate-700 mb-2">Nombre del Rol</label>
              <input type="text" id="name" v-model="form.name" class="block w-full border-slate-300 focus:border-brand-500 focus:ring-brand-500 rounded-2xl shadow-sm" placeholder="Ej: Gestor de Inventario">
            ...
            <div class="mb-8">
              <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-slate-900">Configuración de Accesos (Matriz)</h3>
              ...
              <div class="overflow-x-auto border border-slate-200 rounded-2xl shadow-sm">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                  <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                      <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider sticky left-0 bg-white z-10">
                        Módulo / Funcionalidad
                      </th>
                      <th v-for="action in actions" :key="action" scope="col" class="px-3 py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-200 transition-colors" @click="toggleColumn(action)">
                        <div class="flex flex-col items-center gap-1">
                            <span :class="{'text-purple-600': isColumnSelected(action)}">{{ actionLabels[action] }}</span>
                            <div class="w-2 h-2 rounded-full border border-slate-300 transition-colors" :class="isColumnSelected(action) ? 'bg-purple-600 border-purple-600' : 'bg-white'"></div>
                        </div>
                      </th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
                    <tr v-for="group in permissionGroups" :key="group.module" class="hover:bg-white transition-colors">
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 sticky left-0 bg-white group-hover:bg-white z-10 flex items-center justify-between cursor-pointer" @click="toggleRow(group)">
                        <span>{{ group.label }}</span>
                        <div class="w-3 h-3 rounded-xl border border-slate-300 transition-colors" :class="isRowSelected(group) ? 'bg-purple-600 border-purple-600' : 'bg-white'"></div>
                      </td>
                      <td v-for="action in actions" :key="action" class="px-3 py-4 whitespace-nowrap text-center">
                        <div v-if="group.permissions[action]" class="flex justify-center">
                          <input type="checkbox" 
                                 ...
                                 class="h-5 w-5 text-purple-600 focus:ring-brand-500 border-slate-300 rounded-xl transition-all cursor-pointer">
                        </div>
            ...
            <div class="flex items-center justify-end mt-8 pt-6 border-t border-slate-200">
              <Link :href="route('roles.index')" class="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 mr-4 transition-colors">
                Cancelar
              </Link>
              <button type="submit" 
                      class="inline-flex items-center px-6 py-3 bg-purple-600 border border-transparent rounded-xl font-bold text-sm text-white uppercase tracking-wide hover:bg-purple-700 active:bg-purple-800 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 disabled:opacity-25 transition-all shadow-xl hover:shadow-xl transform hover:shadow-xl hover:shadow-xl.5" 
