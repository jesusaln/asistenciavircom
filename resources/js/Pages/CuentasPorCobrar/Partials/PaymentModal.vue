<template>
    <DialogModal :show="show" @close="close" maxWidth="md">
        <template #content>
            <div class="bg-white rounded-3xl shadow-2xl w-full overflow-hidden border border-gray-100 transform transition-all">
                <div class="px-8 py-6 bg-white border-b border-gray-50 flex justify-between items-center">
                    ...
                    <button @click="close" class="text-gray-300 hover:text-gray-900 transition-colors">
                ...
                <div class="p-8 space-y-6">
                    <div v-if="cuenta" class="mb-4">
                        <p class="text-xs text-gray-500 font-bold uppercase tracking-widest">{{ cuenta.cobrable?.cliente?.nombre_razon_social || 'Cliente' }}</p>
                        <p class="text-lg font-black text-gray-800">Saldo Pendiente: {{ formatCurrency(cuenta.monto_pendiente) }}</p>
                    </div>
                    ...
                    <div class="p-6 bg-white rounded-2xl border border-gray-100">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="form.monto" type="number" step="0.01" class="w-full pl-6 py-2 bg-transparent border-0 rounded-none text-3xl font-black text-gray-900 focus:ring-0 placeholder:text-gray-100" placeholder="0.00" />
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Método de Pago</label>
                        <select v-model="form.metodo_pago" class="w-full py-4 px-5 bg-white border-2 border-gray-100 rounded-2xl font-bold text-gray-900 focus:border-gray-900 focus:ring-0 transition-all">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.notas" rows="2" class="w-full px-5 py-4 bg-white border-2 border-gray-100 rounded-2xl font-bold text-gray-900 focus:border-gray-900 focus:ring-0 transition-all" placeholder="Ej: Pago de factura..."></textarea>
                    </div>
                </div>
                ...
                <div class="px-8 py-6 bg-white/50 border-t border-gray-100 flex flex-col gap-3">
                    ...
                    <button @click="close" class="w-full py-3 font-black text-gray-400 hover:text-gray-900 uppercase text-[10px] tracking-widest transition-colors tracking-widest">Cancelar</button>
                </div>
            </div>
        </template>
    </DialogModal>
