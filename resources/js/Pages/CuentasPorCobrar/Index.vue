<template>
    <AppLayout title="Cuentas por Cobrar">
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <button
                        ...
                        class="bg-brand-500 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-xl flex items-center"
                    >
                    ...
                    <Link
                        ...
                        class="bg-brand-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl"
                    >
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6 font-black uppercase">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 dark:border-slate-800 transition-all">
        ...
        <!-- Filtros -->
        <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 dark:border-slate-800 mb-6 transition-all">
            <div class="p-6">
                <form @submit.prevent="applyFilters" class="flex flex-wrap gap-4 items-end">
                    <div class="w-full md:w-1/3 min-w-[200px]">
                        ...
                        <div class="mt-1 relative rounded-xl shadow-sm">
                            <input type="text" v-model="filters.search" placeholder="Cliente, Folio, RFC..." 
                                   class="block w-full border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-brand-500 focus:border-brand-500 sm:text-sm dark:bg-slate-700 dark:text-white" />
                    ...
                    <div>
                        ...
                        <select v-model="filters.estado" class="mt-1 block w-full border-slate-300 dark:border-slate-700 rounded-xl shadow-sm min-w-[150px] dark:bg-slate-700 dark:text-white">
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wide">Cliente</label>
                        <select v-model="filters.cliente_id" class="mt-1 block w-full border-slate-300 dark:border-slate-800 rounded-2xl shadow-sm dark:bg-slate-950 dark:text-white focus:ring-brand-500 focus:border-brand-500">
                            <option value="">Todos</option>
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="bg-brand-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl">
                            Filtrar
                        </button>
                    </div>
                </form>
        ...
        <!-- Tabla de cuentas -->
        <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-3xl border border-slate-100 dark:border-slate-800 transition-all">
            <div class="p-6">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
                            <tr v-for="cuenta in cuentas.data" :key="cuenta.id" class="hover:bg-slate-50 dark:hover:bg-slate-700 transaction-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 dark:text-slate-100">
                                    ...
                                    <template v-else-if="cuenta.cobrable_type && (cuenta.cobrable_type.includes('Renta') || cuenta.cobrable_type === 'renta')">
                                        ...
                                        <div class="text-xs text-slate-500">{{ cuenta.notas }}</div>
                                    </template>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span :class="{
                                        'bg-rose-100 text-rose-800 dark:text-rose-200': cuenta.estado === 'vencido',
                                        'bg-brand-100 text-brand-800 dark:text-amber-200': cuenta.estado === 'parcial',
                                        'bg-emerald-100 text-emerald-800 dark:text-emerald-200': cuenta.estado === 'pagado',
                                        'bg-slate-100 text-slate-800': cuenta.estado === 'pendiente'
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center justify-start space-x-2">
                                        ...
                                        <button @click="abrirModalDetalles(cuenta.id)"
                                              class="group/btn relative inline-flex items-center justify-center w-9 h-9 rounded-xl bg-sky-50 dark:bg-sky-900/20 dark:bg-sky-900/40 text-blue-600 dark:text-blue-400 hover:bg-sky-100 dark:hover:bg-blue-900/60 hover:shadow-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:ring-offset-1"
                                              title="Ver detalles">
                                        ...
                                        <button v-if="cuenta.estado !== 'pagado'"
                                                ...
                                                class="group/btn relative inline-flex items-center justify-center w-9 h-9 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 dark:bg-slate-800/50 text-emerald-600 dark:text-slate-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 hover:shadow-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:ring-offset-1"
                        ...
                        <div class="flex space-x-1">
                            <Link
                                ...
                                :class="[
                                    ...
                                    link.active ? 'bg-brand-500 text-white border-blue-500' : (link.url ? 'bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border-slate-300 dark:border-slate-600 cursor-not-allowed')
                                ]"
