        ...
        <div class="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 p-4 md:p-6 transition-colors">
            ...
            <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                     <Link :href="route('pedidos-online.index')" class="w-10 h-10 rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-200 dark:hover:border-blue-900 transition-all">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-white/50 dark:bg-gray-800/50">
                            ...
                            <table class="w-full">
                                <thead class="bg-gray-50/50 dark:bg-gray-700/30">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Cant.</th>
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                    <tr v-for="item in pedido.items" :key="item.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-400 dark:text-gray-500">
                                                ...
                                                <div class="ml-4">
                                                    ...
                                                    <div class="text-xs text-gray-500 dark:text-gray-400 font-mono">{{ item.codigo }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right text-sm text-gray-900 dark:text-white font-bold">
                                            {{ item.cantidad }}
                                        </td>
                                        <td class="px-6 py-4 text-right text-sm font-bold text-gray-900 dark:text-white font-mono">
                                            {{ formatCurrency(item.subtotal) }}
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot class="bg-gray-50/30 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700">
                                    <tr>
                                        <td colspan="3" class="px-6 py-3 text-right text-sm text-gray-500 dark:text-gray-400">Subtotal:</td>
                                        <td class="px-6 py-3 text-right text-sm font-medium text-gray-900 dark:text-white font-mono">{{ formatCurrency(pedido.subtotal) }}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="3" class="px-6 py-3 text-right text-sm text-gray-500 dark:text-gray-400">Envío:</td>
                                        <td class="px-6 py-3 text-right text-sm font-medium text-gray-900 dark:text-white font-mono">{{ formatCurrency(pedido.costo_envio) }}</td>
                                    </tr>
                                    <tr>
                    ...
                    <!-- Bitácora de Eventos -->
                    <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50">
                             <h3 class="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start group">
                                        <div class="space-y-1">
                                            ...
                                            <div v-if="evento.usuario" class="flex items-center gap-2 pt-1">
                                                 <span class="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full flex items-center gap-1">
                                                     <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                                     {{ evento.usuario.name }}
                                                 </span>
                                            </div>
                                        </div>
                                        <span class="text-xs text-gray-400 dark:text-gray-500 font-mono mt-1 sm:mt-0 bg-white dark:bg-gray-800 px-2 py-1 rounded border border-gray-100 dark:border-gray-700">
                                            {{ formatDate(evento.created_at) }}
                                        </span>
                                    </div>
                                </div>
                                <div v-if="!pedido.bitacora.length" class="pl-8 text-gray-500 dark:text-gray-400 italic font-sm">No hay eventos registrados aún.</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <!-- Panel de Gestión -->
                    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-blue-100 dark:border-blue-900/30 shadow-lg shadow-blue-500/5 relative overflow-hidden">
                        ...
                        <div class="p-6 space-y-5">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1.5">Estatus de Pago</label>
                                <div class="relative">
                                    <select v-model="form.estatus_pago" class="block w-full pl-3 pr-10 py-2 text-base border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white transition-colors">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1.5">Estatus del Pedido</label>
                                <div class="relative">
                                    <select v-model="form.estatus_pedido" class="block w-full pl-3 pr-10 py-2 text-base border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white transition-colors">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1.5">Guía de Envío</label>
                                <div class="relative rounded-md shadow-sm">
                                    ...
                                    <input v-model="form.guia_envio" type="text" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 sm:text-sm border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white transition-colors py-2" placeholder="Ej. FEDEX-123456">
                                </div>
                            </div>
                    ...
                    <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                         <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2 bg-white/50 dark:bg-gray-800/50">
                            <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                             <h3 class="text-lg font-bold text-gray-800 dark:text-white">Cliente</h3>
                        </div>
                        <div class="p-6 space-y-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-bold text-gray-900 dark:text-white">{{ pedido.nombre }}</div>
                    ...
                    <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2 bg-white/50 dark:bg-gray-800/50">
                             <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path></svg>
                             <h3 class="text-lg font-bold text-gray-800 dark:text-white">Envío</h3>
                        </div>
                        <div class="p-6">
                            <div class="mb-4">
