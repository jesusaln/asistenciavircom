            ...
            <div class="p-4 bg-slate-950/30 border-b border-white/5">
                <div class="relative">
                    <input 
                        ...
                        class="w-full bg-slate-900 border-2 border-white/5 rounded-2xl px-12 py-4 text-lg font-bold focus:border-purple-500 focus:ring-0 transition-all"
                        autofocus
                        @input="onSearchInput"
                    />
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 custom-scrollbar min-h-[400px]">
                <div class="space-y-2">
                    <div v-if="showPublico" 
                        ...
                        class="p-4 rounded-2xl border border-dashed border-white/10 hover:border-purple-500/50 hover:bg-purple-500/5 transition-all cursor-pointer flex items-center gap-4 group"
                    >
                        <div class="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-purple-500 group-hover:text-white transition-all">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 font-bold uppercase tracking-widest">RFC: XAXX010101000</p>
                        </div>
                        <div v-if="!selectedClienteId" class="w-3 h-3 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/50"></div>
                    </div>

                    <div v-for="c in filteredClientes" 
                        :key="c.id"
                        @click="emit('select', c)"
                        :class="selectedClienteId === c.id ? 'bg-purple-600 border-purple-400 text-white' : 'bg-slate-800/40 border-white/5 text-slate-400 hover:border-purple-500/30 hover:bg-slate-800'"
                        class="p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 group">
                        
                        <div class="w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg transition-all"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-3 text-[10px] font-bold uppercase tracking-tight opacity-50">
                                <span>RFC: {{ c.rfc }}</span>
                                <span v-if="c.email" class="truncate">• {{ c.email }}</span>
                            </div>
                        </div>

                        <div v-if="selectedClienteId === c.id" class="w-6 h-6 rounded-full bg-white flex items-center justify-center shadow-xl">
