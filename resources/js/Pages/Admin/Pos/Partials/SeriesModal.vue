        ...
        <div class="relative bg-slate-900 border border-white/10 rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div class="p-4 bg-slate-950/30 border-b border-white/5">
                <div class="relative">
                    <input 
                        ...
                        class="w-full bg-slate-900 border-2 border-white/5 rounded-2xl px-12 py-3 text-sm font-bold focus:border-brand-500 focus:ring-0 transition-all"
                        @input="onSearchInput"
                    />
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 custom-scrollbar min-h-[300px]">
                <div v-if="loading" class="h-64 flex flex-col items-center justify-center opacity-40">
                    ...
                    <p class="font-black text-xs uppercase tracking-wide">Cargando Series...</p>
                </div>

                <div v-else-if="error" class="h-64 flex flex-col items-center justify-center p-8 text-center">
                    <div class="w-16 h-16 rounded-full bg-brand-500/10 flex items-center justify-center text-rose-500 mb-4">
                ...
                <div v-else-if="filteredSeries.length > 0" class="grid grid-cols-2 gap-3">
                    <div v-for="serie in filteredSeries" 
                        ...
                        :class="isSelected(serie) ? 'bg-purple-600 border-purple-400 text-white' : 'bg-slate-800/50 border-white/5 text-slate-400 hover:border-brand-500/30'"
                        class="p-4 rounded-2xl border transition-all cursor-pointer group">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-black uppercase tracking-wide opacity-50">{{ serie.lote || 'Sin Lote' }}</span>
