            ...
            <div class="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-200">
                <div class="p-6 border-b border-white/5 flex justify-between items-center bg-slate-950/50 backdrop-blur-xl">
                    <div>
                        <h2 class="text-2xl font-black text-white flex items-center gap-3">
                            <span class="w-3 h-8 bg-gradient-to-b from-rose-500 to-rose-700 rounded-full shadow-lg shadow-rose-500/20"></span>
                            Cierre de Caja
                        </h2>
                        <p class="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1 ml-6">Arqueo de Efectivo y Corte</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 custom-scrollbar">
                    <div class="lg:col-span-7 space-y-8">
                        <div class="bg-slate-800/30 rounded-2xl p-6 border border-white/5">
                            <h3 class="flex items-center gap-2 text-emerald-400 font-bold uppercase text-xs tracking-widest mb-6">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                <div v-for="denom in bills" :key="denom" class="relative group">
                                    ...
                                    <input 
                                        ...
                                        class="w-full bg-slate-950 border border-slate-700 rounded-xl pl-16 pr-4 py-3 text-right text-white font-mono font-bold focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all group-hover:bg-slate-900" 
                                        min="0" 
                                        placeholder="0"
                                        @input="onInputDenom(denom, $event.target.value)"
                                        @focus="$event.target.select()"
                                    >
                                    <div class="absolute -top-2 right-2 bg-slate-800 text-[9px] text-slate-400 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        ...
                        <div class="bg-slate-800/30 rounded-2xl p-6 border border-white/5">
                            <h3 class="flex items-center gap-2 text-amber-400 font-bold uppercase text-xs tracking-widest mb-6">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                <div v-for="(label, key) in coins" :key="key" class="relative group">
                                    ...
                                    <input 
                                        ...
                                        class="w-full bg-slate-950 border border-slate-700 rounded-xl pl-16 pr-4 py-3 text-right text-white font-mono font-bold focus:border-amber-500 focus:ring-1 focus:ring-amber-500/50 transition-all group-hover:bg-slate-900" 
                                        min="0" 
                                        placeholder="0"
                    ...
                    <div class="lg:col-span-5 flex flex-col gap-6">
                        <div class="bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-xl relative overflow-hidden">
                            ...
                            <h3 class="text-purple-400 font-bold uppercase text-xs tracking-widest mb-6 relative z-10">Resumen Financiero</h3>

                            <div class="space-y-4 relative z-10" v-if="closingDetails && closingDetails.total_sistema !== null">
                                <div class="bg-slate-950/50 rounded-xl p-4 border border-white/5 flex justify-between items-center group hover:border-purple-500/30 transition-colors">
                                ...
                                <div class="bg-slate-950/50 rounded-xl p-4 border border-white/5 flex justify-between items-center group hover:border-green-500/30 transition-colors">
                                    <span class="text-slate-400 font-medium">Ventas Efectivo</span>
                                    <span class="font-mono text-green-400 font-bold">+ {{ formatCurrency(closingDetails.ventas_efectivo || 0) }}</span>
                                </div>
                                <div v-if="closingDetails.ingresos > 0" class="bg-slate-950/50 rounded-xl p-4 border border-white/5 flex justify-between items-center group hover:border-emerald-500/30 transition-colors">
                                ...
                                <div class="bg-slate-950/50 rounded-xl p-4 border border-white/5 flex justify-between items-center group hover:border-rose-500/30 transition-colors">
                        ...
                        <div class="bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-xl flex-1 flex flex-col justify-center">
                            <h3 class="text-slate-400 font-bold uppercase text-xs tracking-widest mb-2 text-center">Total Contado</h3>
                            ...
                            <div v-if="closingDetails && closingDetails.total_sistema !== null"
                                class="rounded-xl p-4 border-l-4 transition-all duration-300"
                ...
                <div class="p-6 bg-slate-950/80 backdrop-blur-xl border-t border-white/5 flex justify-end gap-4">
                    ...
                    <button 
                        @click="emit('confirm')" 
                        :disabled="loading"
                        class="px-10 py-4 bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white font-bold rounded-xl shadow-lg shadow-rose-600/20 disabled:opacity-50 transition-all flex items-center gap-3 transform active:scale-95 duration-200"
                    >
