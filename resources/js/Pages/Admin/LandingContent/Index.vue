                        ...
                        <div v-if="faqs?.length" class="space-y-3">
                            <div v-for="faq in faqs" :key="faq.id" class="bg-white dark:bg-gray-800 rounded-lg p-4 flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2 ml-4">
                                    ...
                                    <button @click="openFaqModal(faq)" class="p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400"><font-awesome-icon icon="pen" /></button>
                                    <button @click="deleteFaq(faq)" class="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400"><font-awesome-icon icon="trash-can" /></button>
                                </div>
                            </div>
                        </div>
                            ...
                            <div v-for="t in testimonios" :key="t.id" class="bg-white dark:bg-gray-800 rounded-lg p-4">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-3 mb-3">"{{ t.comentario }}"</p>
                                    <div class="flex items-center justify-between">
                                        <div class="flex">
                                        <span v-for="i in t.calificacion" :key="i" class="text-yellow-400 text-sm"><font-awesome-icon icon="star" /></span>
                                    </div>
                                    <div class="flex gap-1">
                                        <button @click="openTestimonioModal(t)" class="p-1 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 text-sm"><font-awesome-icon icon="pen" /></button>
                                        <button @click="deleteTestimonio(t)" class="p-1 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 text-sm"><font-awesome-icon icon="trash-can" /></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="logos?.length" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            <div v-for="logo in logos" :key="logo.id" class="bg-white dark:bg-gray-800 rounded-lg p-4 text-center relative group">
                                ...
                                <div class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                                    <button @click="openLogoModal(logo)" class="p-1 bg-white dark:bg-gray-700 rounded shadow text-xs dark:text-gray-200"><font-awesome-icon icon="pen" /></button>
                                    <button @click="deleteLogo(logo)" class="p-1 bg-white dark:bg-gray-700 rounded shadow text-xs dark:text-gray-200"><font-awesome-icon icon="trash-can" /></button>
                                </div>
                            </div>
                        </div>
                        <div v-else class="text-center py-12 text-gray-400 dark:text-gray-500">
                        ...
                        <div v-if="marcas?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div v-for="marca in marcas" :key="marca.id" class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 group relative">
                                ...
                                <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                                    <button @click="openMarcaModal(marca)" class="p-2 bg-white dark:bg-gray-700 rounded-lg shadow-md text-sm hover:text-blue-600 dark:hover:text-blue-400"><font-awesome-icon icon="pen" /></button>
                                    <button @click="deleteMarca(marca)" class="p-2 bg-white dark:bg-gray-700 rounded-lg shadow-md text-sm hover:text-red-600 dark:hover:text-red-400"><font-awesome-icon icon="trash-can" /></button>
                                </div>
                            </div>
                        </div>
                        <div v-else class="text-center py-12 text-gray-400 dark:text-gray-500">
                        ...
                        <div v-if="procesos?.length" class="space-y-4">
                            <div v-for="(p, index) in procesos" :key="p.id" class="flex gap-4 items-start bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 group">
                                ...
                                <div class="flex gap-2">
                                    <button @click="openProcesoModal(p)" class="p-2 bg-white dark:bg-gray-700 rounded-lg shadow-sm text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400"><font-awesome-icon icon="pen" /></button>
                                    <button @click="deleteProceso(p)" class="p-2 bg-white dark:bg-gray-700 rounded-lg shadow-sm text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400"><font-awesome-icon icon="trash-can" /></button>
                                </div>
                            </div>
                        </div>
                        <div v-else class="text-center py-12 text-gray-400 dark:text-gray-500">
                        ...
                        <div v-if="ofertas?.length" class="space-y-4">
                            <div v-for="oferta in ofertas" :key="oferta.id" class="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-6 text-white relative group">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex gap-2">
                                        <button @click="openOfertaModal(oferta)" class="p-3 bg-white/10 rounded-lg hover:bg-white/20 transition"><font-awesome-icon icon="pen" /></button>
                                        <button @click="deleteOferta(oferta)" class="p-3 bg-white/10 rounded-lg hover:bg-red-500/50 transition"><font-awesome-icon icon="trash-can" /></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <form @submit.prevent="saveProceso" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input v-model="procesoForm.icono" type="text" class="w-full rounded-lg border-gray-300 text-center" placeholder="cogs" />
                            </div>
                        </div>
                        <div>
