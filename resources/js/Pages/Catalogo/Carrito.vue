...
<template>
    ...
    <div class="min-h-screen bg-white dark:bg-slate-900 dark:bg-gray-900 flex flex-col font-sans transition-colors duration-300" :style="cssVars">
        ...
        <main class="flex-grow w-full px-4 sm:px-6 lg:px-8 py-8 lg:py-12 w-full">
            <h1 class="text-3xl font-black text-gray-900 dark:text-white dark:text-white mb-8 flex items-center gap-3 transition-colors">
                ...
                Carrito de Compras
                <span v-if="itemCount > 0" class="text-lg font-bold text-gray-400 dark:text-gray-500 dark:text-gray-400 transition-colors">({{ itemCount }})</span>
            </h1>

            <div v-if="isEmpty" class="text-center py-20 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-[2rem] shadow-xl shadow-gray-100 dark:shadow-none border border-gray-100 dark:border-gray-700 transition-colors">
                ...
                <h2 class="text-2xl font-black text-gray-900 dark:text-white dark:text-white mb-4 transition-colors">Tu carrito está vacío</h2>
                <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 font-medium mb-8 transition-colors">Parece que aún no has agregado nada.</p>
            ...
            <div v-else class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-4">
                    <div v-for="item in items" :key="item.producto_id"
                         class="bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-[1.5rem] p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex gap-6 hover:shadow-md transition-all">
                        <!-- Image -->
                        <div class="w-28 h-28 flex-shrink-0 bg-white dark:bg-slate-900 dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 transition-colors">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-between py-1">
                            <div>
                                <h3 class="font-bold text-gray-900 dark:text-white dark:text-white text-lg truncate transition-colors">{{ item.nombre }}</h3>
                        ...
                        <div class="flex flex-col items-end justify-between py-1">
                             <p class="text-lg font-black text-gray-900 dark:text-white dark:text-white transition-colors">
                                {{ formatCurrency(item.precio * item.cantidad) }}
                            </p>

                            <!-- Quantity Controls -->
                            <div class="flex items-center gap-1 bg-white dark:bg-slate-900 dark:bg-gray-700 rounded-xl p-1 border border-gray-100 dark:border-gray-600 transition-colors">
                                <button @click="decrementQuantity(item.producto_id)"
                                        class="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 dark:bg-gray-600 flex items-center justify-center text-gray-500 dark:text-gray-400 dark:text-gray-300 hover:text-[var(--color-primary)] hover:shadow-sm transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                                    </svg>
                                </button>
                                <span class="w-8 text-center font-bold text-gray-900 dark:text-white dark:text-white text-sm transition-colors">{{ item.cantidad }}</span>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-[2rem] p-8 shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 sticky top-28 transition-colors">
                        <h2 class="text-xl font-black text-gray-900 dark:text-white dark:text-white mb-6 uppercase tracking-tight transition-colors">Resumen</h2>
                        
                        <div class="space-y-4 mb-8">
                            <div class="flex justify-between text-gray-400 dark:text-gray-500 dark:text-gray-400 font-medium text-xs uppercase tracking-widest transition-colors">
                                <span>Subtotal (sin IVA)</span>
                                <span class="font-bold text-gray-600 dark:text-gray-300 dark:text-gray-300 transition-colors">{{ formatCurrency(subtotalSinIva) }}</span>
                            </div>
                            <div class="flex justify-between text-gray-400 dark:text-gray-500 dark:text-gray-400 font-medium text-xs uppercase tracking-widest transition-colors">
                                <span>IVA (16%)</span>
                                <span class="font-bold text-gray-600 dark:text-gray-300 dark:text-gray-300 transition-colors">{{ formatCurrency(iva) }}</span>
                            </div>
                            <div class="flex justify-between text-gray-500 dark:text-gray-400 dark:text-gray-400 font-medium text-sm transition-colors">
                                <span>Costo de Envío</span>
                                <span class="font-bold text-gray-900 dark:text-white dark:text-white transition-colors">
                            ...
                            <div class="flex justify-between text-lg">
                                <span class="font-black text-gray-900 dark:text-white dark:text-white transition-colors">Total</span>
                        ...
                        <div class="mt-4 text-center">
                             <Link :href="route('catalogo.index')" class="text-xs font-bold text-gray-400 dark:text-gray-500 dark:text-gray-400 hover:text-[var(--color-primary)] uppercase tracking-widest transition-colors">
                                Seguir comprando
                            </Link>
                        </div>
                    </div>
                    ...
                    <!-- Seguridad -->
                    <div class="mt-6 flex items-center justify-center gap-2 text-[10px] uppercase font-black tracking-widest text-gray-300 dark:text-gray-600 dark:text-gray-300">
            ...
            <div v-if="recomendados?.length" class="mt-20 border-t border-gray-100 dark:border-gray-800 pt-12 transition-colors">
                <h2 class="text-2xl font-black text-gray-900 dark:text-white dark:text-white mb-8 transition-colors">Te podría interesar</h2>
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                    <Link v-for="rel in recomendados" :key="rel.id"
                          :href="route('catalogo.show', rel.id)"
                          class="group bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-700 hover:shadow-2xl hover:border-white dark:hover:border-gray-600 transition-all duration-500">
                        <div class="aspect-square bg-white dark:bg-slate-900 dark:bg-gray-900 overflow-hidden relative">
                            ...
                            <div v-else class="w-full h-full flex items-center justify-center">
                                <svg class="w-12 h-12 text-gray-300 dark:text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-5">
                            <h3 class="font-bold text-gray-900 dark:text-white dark:text-white text-sm line-clamp-2 group-hover:text-[var(--color-primary)] transition-colors mb-2">
                                {{ rel.nombre }}
                            </h3>
