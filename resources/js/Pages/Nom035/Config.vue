...
const responsible_name = ref(props.config.responsible_name || '')
const responsible_position = ref(props.config.responsible_position || '')
...
const savePolicy = () => {
    ...
    router.put(route('nom035.config.policy.update'), {
        ...
        responsible_position: responsible_position.value,
    }, {
        onSuccess: () => {
            saving.value = false
            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-[var(--ui-surface)] border border-[var(--ui-border)] rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <!-- Responsable Legal Section -->
                        <div class="bg-indigo-50/50 border border-indigo-100 p-6 rounded-2xl space-y-4 dark:bg-indigo-900/10 dark:border-indigo-800/50">
                            <div class="flex items-center gap-3 mb-2">
                                <div class="h-8 w-8 bg-indigo-500/10 rounded-lg flex items-center justify-center text-indigo-600">
                                    <font-awesome-icon icon="user-tie" />
                                </div>
                                <h4 class="text-sm font-black text-indigo-900 uppercase dark:text-indigo-300">Responsable Legal de la NOM-035</h4>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-[var(--ui-text-soft)] uppercase tracking-wider">Nombre del Responsable</label>
                                    <input 
                                        v-model="responsible_name" 
                                        type="text" 
                                        class="w-full bg-[var(--ui-surface-soft)] border-[var(--ui-border)] rounded-xl px-4 py-2 text-[var(--ui-text-main)] focus:ring-2 focus:ring-indigo-500/20 transition-all text-sm"
                                        placeholder="Ej. Jesús Alberto López Noriega"
                                    />
                                </div>
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-[var(--ui-text-soft)] uppercase tracking-wider">Cargo / Puesto</label>
                                    <input 
                                        v-model="responsible_position" 
                                        type="text" 
                                        class="w-full bg-[var(--ui-surface-soft)] border-[var(--ui-border)] rounded-xl px-4 py-2 text-[var(--ui-text-main)] focus:ring-2 focus:ring-indigo-500/20 transition-all text-sm"
                            ...
                            <p class="text-[10px] text-indigo-800/60 dark:text-indigo-300/60 italic">
