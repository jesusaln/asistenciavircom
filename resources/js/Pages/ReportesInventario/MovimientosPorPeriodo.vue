    ...
    <div class="w-full">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-slate-900">Movimientos por Período</h1>
            <p class="text-slate-500 mt-1">Historial de movimientos de inventario filtrado por fechas</p>
          </div>
          <Link
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold text-emerald-600">{{ stats.entradas }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold text-rose-600">{{ stats.salidas }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm font-medium text-slate-500">Almacenes Afectados</p>
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        <form @submit.prevent="aplicarFiltros" class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label for="fecha_inicio" class="block text-sm font-medium text-slate-700 mb-2">Fecha Inicio</label>
            <input
              id="fecha_inicio"
          ...
          <div>
            <label for="fecha_fin" class="block text-sm font-medium text-slate-700 mb-2">Fecha Fin</label>
            <input
              id="fecha_fin"
          ...
          <div>
            <label for="tipo" class="block text-sm font-medium text-slate-700 mb-2">Tipo de Movimiento</label>
            <select
              id="tipo"
          ...
          <div class="flex items-end">
            <button
              type="submit"
              class="w-full px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 focus:ring-2 focus:ring-brand-500 transition-colors"
            >
              Aplicar Filtros
            </button>
          </div>
      ...
      <div v-if="movimientos.length > 0" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Almacén</th>
                ...
                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Motivo</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
              <tr v-for="movimiento in movimientos" :key="movimiento.id" class="hover:bg-white">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-slate-900">{{ formatFechaHoraLocal(movimiento.created_at) }}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-8 w-8">
                      <div class="h-8 w-8 rounded-xl bg-slate-200 flex items-center justify-center">
                    ...
                    <div class="ml-3">
                      <div class="text-sm font-medium text-slate-900">{{ movimiento.producto_nombre }}</div>
                      <div class="text-sm text-slate-500">{{ movimiento.producto_codigo }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-slate-900">{{ movimiento.cantidad }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-slate-900">{{ movimiento.almacen_nombre }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-slate-900">{{ movimiento.usuario_nombre || 'Sistema' }}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-slate-900">{{ movimiento.motivo || 'Sin especificar' }}</div>
                  <div v-if="movimiento.detalles" class="text-xs text-slate-500 mt-1">
      ...
      <div v-else class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
        ...
        <h3 class="mt-2 text-sm font-medium text-slate-900">No hay movimientos en el período seleccionado</h3>
        <p class="mt-1 text-sm text-slate-500">Intenta ajustar las fechas o filtros para ver más resultados.</p>
      </div>
    </div>
  </div>
</template>
