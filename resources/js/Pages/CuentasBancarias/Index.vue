<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          ...
          <p class="text-slate-600 dark:text-slate-400 mt-1">Gestiona tus cuentas bancarias y saldos</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <Link
            ...
            class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 transition-colors shadow-md shadow-blue-600/20"
          >
      ...
      <div
        ...
        class="mb-6 p-4 rounded-xl border-l-4 border-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200"
      >
        {{ $page.props.flash.success }}
      </div>
      <div
        v-if="$page.props.flash?.error"
        class="mb-6 p-4 rounded-xl border-l-4 border-red-500 bg-red-50 dark:bg-red-950/40 text-red-800 dark:text-red-200"
      >
      ...
      <div v-if="cuentas.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="cuenta.id"
          class="rounded-2xl border overflow-hidden transition-shadow hover:shadow-xl bg-white dark:bg-slate-900 border-slate-200/80 dark:border-slate-800 shadow-[0_10px_40px_rgba(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.25)]"
        ...
        >
          ...
          <div class="p-5">
            <div class="flex items-start justify-between mb-4 gap-2">
              ...
              <span
                v-if="!cuenta.activa"
                class="shrink-0 px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-full font-medium"
              >
                Inactiva
              </span>
            </div>

            <div class="rounded-xl p-4 mb-4 bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/80">
              <p class="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold tracking-wide">Saldo actual</p>
              <p
                class="text-2xl font-bold tabular-nums"
                :class="cuenta.saldo_actual >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'"
              >
            ...
            <div class="flex flex-wrap items-center justify-end gap-2 pt-4 border-t border-slate-200 dark:border-slate-700">
              <Link
                :href="route('cuentas-bancarias.show', { cuentas_bancaria: cuenta.id })"
                class="p-2 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 rounded-xl transition-colors"
              ...
              <Link
                :href="route('cuentas-bancarias.show', { cuentas_bancaria: cuenta.id })"
                class="px-3 py-2 bg-emerald-600 dark:bg-emerald-500 text-white text-sm rounded-xl hover:bg-emerald-700 dark:hover:bg-emerald-400 transition-colors font-medium"
                title="Ver registros"
              >
                Ver registros
              </Link>
              <button
                type="button"
                class="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/40 rounded-xl transition-colors"
              ...
              <button
                ...
                type="button"
                class="p-2 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-xl transition-colors"
                title="Eliminar"
      ...
      <div
        v-else
        class="rounded-2xl border p-12 text-center bg-white dark:bg-slate-900 border-slate-200/80 dark:border-slate-800 shadow-[0_10px_40px_rgba(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.25)]"
      >
        <FontAwesomeIcon :icon="['fas', 'piggy-bank']" class="h-16 w-16 text-slate-300 dark:text-slate-600 mb-4 mx-auto" />
        ...
        <Link
          :href="route('cuentas-bancarias.create')"
          class="inline-flex px-6 py-3 rounded-xl bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 font-semibold shadow-md"
        >
          Agregar cuenta
        </Link>
      </div>
    ...
    >
      <div
        class="rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 max-h-[90vh] overflow-y-auto"
      >
        ...
        <div class="p-6 border-b border-slate-200 dark:border-slate-700">
          <div class="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              class="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 shrink-0"
        ...
        <div class="p-6 space-y-4">
          ...
          >
            ...
            <div class="relative">
              ...
              <p class="text-3xl font-black tabular-nums">${{ formatMonto(cuentaSeleccionada?.saldo_actual) }}</p>
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-xl p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700">
          ...
          <div v-if="cuentaSeleccionada?.clabe" class="rounded-xl p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700">
          ...
          <div v-if="cuentaSeleccionada?.notas" class="rounded-xl p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700">
            <p class="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Notas</p>
            <p class="text-slate-700 dark:text-slate-300 text-sm mt-1 whitespace-pre-wrap">{{ cuentaSeleccionada?.notas }}</p>
          </div>
        </div>

        <div class="flex justify-end gap-3 p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/40">
          <button
            type="button"
            class="px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-800"
            @click="showModal = false"
          >
            Cerrar
          </button>
          <Link
            ...
            class="px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 font-semibold"
          >
            Editar
          </Link>
        </div>
