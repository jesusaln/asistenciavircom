...
<template>
    ...
    <div
        class="appointment-shell min-h-screen bg-[var(--color-app-bg-page)] text-[var(--color-app-text-primary)] transition-colors duration-500"
        :style="cssVars"
    >
        ...
        <button 
            ...
            class="fixed bottom-6 right-6 z-50 p-3 rounded-full shadow-xl transition-colors border border-[var(--ui-border)] bg-[var(--ui-surface)] text-[var(--ui-text-muted)] hover:bg-[var(--ui-surface-soft)]"
            title="Alternar tema"
        >
            <svg v-if="isDarkMode" class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
            <svg v-else class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
        </button>
        ...
        <main>
            ...
            <section class="relative overflow-hidden">
                ...
                <div class="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16 lg:py-24 xl:py-28">
                    ...
                    >
                        <!-- Badge -->
                        <div class="mb-8 inline-flex items-center gap-2.5 rounded-full border border-orange-200/80 bg-white/80 px-5 py-2.5 text-[10px] font-black uppercase tracking-[0.22em] text-brand-700 shadow-sm backdrop-blur-lg transition-colors dark:border-white/10 dark:bg-white/[0.06] dark:text-orange-300 dark:shadow-none">
                        ...
                        <h1 class="max-w-2xl text-[2.75rem] font-black leading-[1.08] tracking-tight text-slate-950 transition-colors dark:text-white sm:text-5xl lg:text-[3.5rem]">
                            ...
                            <span class="mt-2 block bg-gradient-to-r from-[var(--color-primary)] via-brand-500 to-brand-500 bg-clip-text text-transparent">Nosotros te llamamos.</span>
                        </h1>

                        <p class="mt-7 max-w-xl text-lg leading-8 text-slate-500 transition-colors dark:text-slate-400">
                        ...
                        <div class="mt-12 grid gap-6 sm:grid-cols-3">
                            <div
                                ...
                                :key="feat.title"
                                class="feature-card group relative rounded-[1.75rem] border bg-white/70 p-6 shadow-sm backdrop-blur-sm transition-all duration-500 hover:shadow-xl hover:shadow-xl hover:shadow-xl dark:bg-white/[0.04] dark:shadow-none dark:hover:bg-white/[0.07]"
                                :class="[
                                    feat.accent === 'orange' ? 'border-orange-200/60 hover:border-brand-300 dark:border-brand-500/15 dark:hover:border-brand-400/30' :
                                    feat.accent === 'blue' ? 'border-slate-200/60 hover:border-brand-300 dark:border-white/10 dark:hover:border-brand-500/30' :
                                    'border-brand-200 dark:border-brand-800/30/60 hover:border-brand-500 dark:border-brand-500/15 dark:hover:border-brand-500/30',
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :class="
                                        feat.accent === 'orange' ? 'bg-gradient-to-br from-orange-100/50 to-transparent dark:from-brand-500/[0.08]' :
                                        feat.accent === 'blue' ? 'bg-gradient-to-br from-blue-100/50 to-transparent dark:from-blue-500/[0.08]' :
                                        'bg-gradient-to-br from-brand-100/50 to-transparent dark:from-brand-500/[0.08]'
                                    "
                                ></div>
                                <div class="relative">
                                    ...
                                    <p
                                        ...
                                        :class="
                                            feat.accent === 'orange' ? 'text-brand-600 dark:text-orange-400' :
                                            feat.accent === 'blue' ? 'text-slate-900 dark:text-white' :
                                            'text-brand-800 dark:text-brand-200 dark:text-brand-200 dark:text-amber-400'
                                        "
                                    >{{ feat.title }}</p>
                                    <p class="mt-2 text-sm leading-relaxed text-slate-500 transition-colors dark:text-slate-400">{{ feat.desc }}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        <!-- Form Card -->
                        <div class="form-card relative overflow-hidden rounded-[2.5rem] border border-slate-200/80 bg-white/80 p-7 shadow-2xl shadow-orange-200/30 backdrop-blur-xl transition-all duration-500 dark:border-white/[0.08] dark:bg-slate-800/80 dark:shadow-black/40 sm:p-9">
                            ...
                            <div class="absolute -bottom-12 -left-12 h-28 w-28 rounded-full bg-gradient-to-tr from-brand-200/30 to-transparent blur-2xl dark:from-brand-500/10"></div>

                            <!-- Success message -->
                            <div v-if="page.props.flash?.success" class="relative mb-7 rounded-2xl border border-emerald-200 dark:border-emerald-800/30 bg-gradient-to-r from-emerald-50 to-teal-50 p-5 text-emerald-800 dark:text-emerald-200 transition-colors dark:border-emerald-500/20 dark:from-emerald-500/10 dark:to-teal-500/10 dark:text-emerald-200">
                                <div class="flex items-start gap-3">
                                    <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-lg dark:bg-brand-500/20">✅</div>
                                    <div>
                            ...
                            <div class="relative">
                                ...
                                <div class="mb-7">
                                    <div class="mb-4 flex items-center gap-2">
                                        <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--color-primary)] to-brand-500 text-lg text-white shadow-xl shadow-orange-300/30 dark:shadow-brand-500/20">
                                            📞
                                        </div>
                                        <div>
                                ...
                                <form class="space-y-5" @submit.prevent="submit">
                                    <div class="group">
                                        <label class="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 transition-colors dark:text-slate-200">
                                            <span class="flex h-6 w-6 items-center justify-center rounded-xl bg-slate-100 text-[10px] font-black text-slate-500 transition-colors dark:bg-white/10 dark:text-slate-400">1</span>
                                            Nombre
                                        </label>
                                        <input
                                            v-model="form.nombre"
                                            ...
                                            class="input-field w-full rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-slate-900 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-[var(--color-primary)] focus:bg-white focus:shadow-xl focus:shadow-[var(--color-primary-soft)] focus:ring-0 dark:border-white/10 dark:bg-slate-950/60 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-[var(--color-primary)] dark:focus:bg-slate-950 dark:focus:shadow-[var(--color-primary-glow)]"
                                        >
                                        <p v-if="form.errors.nombre" class="mt-2 text-sm font-medium text-rose-500">{{ form.errors.nombre }}</p>
                                    </div>

                                    <div class="group">
                                        <label class="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 transition-colors dark:text-slate-200">
                                            <span class="flex h-6 w-6 items-center justify-center rounded-xl bg-slate-100 text-[10px] font-black text-slate-500 transition-colors dark:bg-white/10 dark:text-slate-400">2</span>
                                            Teléfono
                                        </label>
                                        <input
                                            :value="form.telefono"
                                    ...
                                    <div class="group">
                                        ...
                                        ></textarea>
                                        <p v-if="form.errors.mensaje" class="mt-2 text-sm font-medium text-rose-500">{{ form.errors.mensaje }}</p>
                                    </div>

                                    <p v-if="form.errors.general" class="rounded-2xl border border-rose-200 dark:border-rose-800/30 bg-rose-50 dark:bg-rose-900/20 px-4 py-3 text-sm font-medium text-rose-800 dark:text-rose-200 dark:text-rose-200 transition-colors dark:border-rose-400/20 dark:bg-brand-500/10 dark:text-rose-300">
                                        {{ form.errors.general }}
                                    </p>
                                    ...
                                    <button
                                        ...
                                        class="submit-btn group relative flex w-full items-center justify-center gap-3 overflow-hidden rounded-2xl bg-gradient-to-r from-[var(--color-primary)] via-brand-600 to-brand-500 px-6 py-5 text-base font-black uppercase tracking-wider text-white shadow-xl shadow-orange-400/30 transition-all duration-200 hover:shadow-xl hover:shadow-xl.5 hover:shadow-2xl hover:shadow-orange-400/40 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60 dark:shadow-brand-500/20 dark:hover:shadow-brand-500/30"
                                    >
                                        <!-- Shine effect -->
                                        ...
                                        <svg v-if="!form.processing" class="relative h-5 w-5 transition-transform duration-200 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="contact-card mt-5 overflow-hidden rounded-[2rem] border border-slate-200/50 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 p-6 shadow-xl transition-colors dark:border-white/[0.06] dark:from-white/[0.04] dark:to-white/[0.02]">
                            <div class="flex items-center gap-5">
                                <div class="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-400 to-brand-500 text-xl shadow-xl shadow-brand-500/30">
                                    📱
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-[10px] font-black uppercase tracking-[0.22em] text-brand-400 dark:text-amber-300">Contacto directo</p>
