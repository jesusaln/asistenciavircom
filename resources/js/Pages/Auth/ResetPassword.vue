...
const passwordStrengthColor = computed(() => {
    const colors = ['text-rose-500', 'text-orange-500', 'text-brand-500', 'text-blue-500', 'text-emerald-500', 'text-emerald-500'];
        ...
        <div class="sm:mx-auto sm:w-full sm:max-w-md text-center relative z-10">
            <Link href="/" class="flex justify-center mb-6">
                ...
                <div v-else class="flex flex-col items-center">
                    ...
                    <span class="text-2xl font-black text-slate-900 dark:text-white mt-4 uppercase tracking-wide">{{ $page.props.empresa_config?.nombre_empresa || 'Climas' }}</span>
                </div>
            </Link>
            <h2 class="text-4xl font-black text-slate-900 dark:text-white tracking-tight uppercase">
                Nueva Contraseña
            </h2>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide">
                Restablecer Acceso
            </p>
        </div>

        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0 relative z-10">
            <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-2xl py-10 px-6 sm:px-12 shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] sm:rounded-[3rem] border border-white/30 dark:border-slate-800/50 relative overflow-hidden group transition-all duration-500 hover:shadow-brand-500/10">

                <!-- Instrucciones -->
                <div class="mb-6 p-4 rounded-2xl bg-brand-50 dark:bg-brand-900/20 text-brand-800 dark:text-brand-200 dark:text-brand-200 dark:text-brand-300 text-xs font-bold uppercase tracking-wide border border-brand-100 dark:border-brand-800 flex items-start gap-2">
                    ...
                    <span class="normal-case font-normal">Crea una nueva contraseña segura para tu cuenta.</span>
                </div>
                ...
                <form class="space-y-6 relative z-10" @submit.prevent="submit">
                    ...
                    <div>
                        ...
                        <div class="relative mt-2">
                            ...
                            <input
                                ...
                                v-model="form.email"
                                type="email"
                                class="block w-full bg-slate-100 dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 rounded-2xl pl-12 pr-5 py-4 text-slate-500 dark:text-slate-400 cursor-not-allowed"
                                readonly
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center">
                            <InputLabel for="password" value="Nueva Contraseña" class="dark:text-slate-400 font-bold uppercase text-[10px] tracking-wide" />
                            <span class="text-[10px] font-bold uppercase tracking-wide" :class="passwordStrengthColor">{{ passwordStrengthText }}</span>
                        </div>
                        <div class="relative mt-2">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <svg class="w-4 h-4 text-slate-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <TextInput
                                ...
                                class="block w-full bg-[var(--ui-surface)] dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 focus:ring-brand-500 focus:border-brand-500 rounded-2xl pl-12 pr-14 py-4 transition-all"
                                required
                        ...
                        <div v-if="form.password" class="mt-2">
                            <div class="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    ...
                                    :class="{
                                        'bg-brand-500 w-1/5': passwordStrength === 1,
                                        'bg-brand-500 w-2/5': passwordStrength === 2,
                                        'bg-brand-500 w-3/5': passwordStrength === 3,
                                        'bg-brand-500 w-4/5': passwordStrength === 4,
                                        'bg-brand-500 w-full': passwordStrength === 5
                                    }"
                                ></div>
                            </div>
                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center">
                            <InputLabel for="password_confirmation" value="Confirmar Contraseña" class="dark:text-slate-400 font-bold uppercase text-[10px] tracking-wide" />
                            <svg v-if="passwordsMatch" class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="relative mt-2">
                            ...
                            <TextInput
                                ...
                                class="block w-full bg-[var(--ui-surface)] dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 focus:ring-brand-500 focus:border-brand-500 rounded-2xl pl-12 pr-14 py-4 transition-all"
                                :class="{ 'border-emerald-500': passwordsMatch }"
                                required
                                autocomplete="new-password"
                                placeholder="••••••••"
                            />
