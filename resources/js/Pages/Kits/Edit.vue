...
const form = ref({
  ...
  destacado: props.kit.destacado || false,
  imagen: null,
  componentes: []
})
...
onMounted(() => {
  if (props.kit.imagen) {
    imagePreview.value = props.kit.imagen.startsWith('http') ? props.kit.imagen : `/storage/${props.kit.imagen}`
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    const response = await axios.post(`/kits/${props.kit.id}`, formData, {
      headers: {
    ...
    if (error.response && error.response.data) {
      if (error.response.data.errors) {
        ...
        notyf.error('Por favor corrige los errores');
      } else {
        notyf.error(error.response.data.message || 'Error al actualizar el kit');
      }
    } else {
