<template>
  <AppLayout title="Crear Nuevo Kit">
    <div class="kits-create">
      <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
        <!-- Header -->
        <div class="mb-8">
          <div class="flex justify-between items-center">
            <div>
              <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Crear Nuevo Kit</h1>
              <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Define un kit con productos y/o servicios</p>
            </div>
        ...
        <form @submit.prevent="submitForm" class="bg-white dark:bg-slate-900 shadow-lg rounded-lg overflow-hidden">
          ...
          <div class="px-6 py-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800">
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Define el nombre, precio y detalles básicos del kit</p>
          </div>
          ...
          <div class="px-6 py-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800">
            <div class="flex justify-between items-center">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Agrega productos y/o servicios que conforman este kit</p>
              </div>
          ...
          <div class="px-6 py-6">
            ...
            <div class="space-y-4">
              <div v-for="(componente, index) in form.componentes" :key="index"
                ...
                <!-- Nota: Las series se asignan al VENDER el kit, no al crearlo -->
                <div v-if="componente.item_type === 'producto' && componente.requiereSeries" class="mt-3">
                  <span class="inline-flex items-center px-2 py-1 text-xs text-blue-700 bg-blue-50 rounded border border-blue-200">
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    Este producto requiere serie (se asignará al vender)
                  </span>
                </div>
              </div>
            </div>
...
const updateItemInfo = (index) => {
  ...
  // Check if product requires series (solo para info, NO se seleccionan aquí)
  if (componente.item_type === 'producto') {
    ...
    // Store series requirement info on the component (solo informativo)
    componente.requiereSeries = requiereSeries
    componente.productoNombre = producto?.nombre || 'Producto'
    
    // NOTA: Las series NO se seleccionan al crear el kit
    // Las series se seleccionan al momento de VENDER el kit
...
const openSerials = async (componente, index) => {
  try {
    ...
    selectedSeries.value = [] // Reset selection for kit creation
...
const confirmSeries = () => {
  ...
  // For kit creation, we don't store series persistently
  // This is just for validation during creation
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.errors) {
        ...
        notyf.error(data.message || 'Error al crear el kit')
      }
    }
  } catch (error) {
    console.error('Error:', error)
