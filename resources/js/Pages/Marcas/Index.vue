...
const props = defineProps({
    marcas: { type: Object, default: () => ({ data: [] }) },
...
const onSearch = () => {
    router.get(route('marcas.index'), { search: search.value }, { preserveState: true, replace: true })
}

const confirmDelete = async (marca) => {
    const { isConfirmed } = await Swal.fire({ title: '¿Eliminar marca?', text: '¿Eliminar esta marca?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Eliminar', cancelButtonText: 'Cancelar' })
    if (!isConfirmed) return
    router.delete(route('marcas.destroy', marca.id), {
        onSuccess: () => notyf.success('Marca eliminada'),
        onError: () => notyf.error('Error al eliminar'),
    })
}

const toggleStatus = (marca) => {
    router.put(route('marcas.update', marca.id), { activo: !marca.activo }, {
        preserveScroll: true,
...
<template>
    ...
    <div class="min-h-screen">
        <div class="w-full px-4 sm:px-6 py-6">
            <CrudPageHeader title="Marcas" subtitle="Gestión de marcas de productos">
                <template #actions>
            ...
            <IndexTable
                ...
                empty-subtext="Crea la primera marca usando el botón Nueva Marca"
            >
                ...
                <template #pagination>
                    <div v-if="marcas.links" class="flex justify-between items-center">
                        <div class="text-sm text-slate-500">
                            Mostrando {{ marcas.from || 0 }} - {{ marcas.to || 0 }} de {{ marcas.total || 0 }}
                        </div>
                        <div class="flex gap-1.5">
                            <Link v-for="(link, i) in marcas.links" :key="i"
                                :href="link.url || '#'"
                                v-html="link.label"
