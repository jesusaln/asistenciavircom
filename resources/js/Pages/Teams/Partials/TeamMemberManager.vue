            ...
            <FormSection @submitted="addTeamMember">
                ...
                <template #form>
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600 dark:text-gray-300 dark:text-gray-400">
                    ...
                    <div v-if="availableRoles.length > 0" class="col-span-6 lg:col-span-4">
                        ...
                        <div class="relative z-0 mt-1 border border-gray-200 dark:border-slate-800 dark:border-gray-700 rounded-lg cursor-pointer">
                            <button
                                v-for="(role, i) in availableRoles"
                                :key="role.key"
                                type="button"
                                class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-amber-500 dark:focus:border-amber-500 focus:ring-2 focus:ring-amber-500 dark:focus:ring-indigo-600"
                                :class="{'border-t border-gray-200 dark:border-slate-800 dark:border-gray-700 focus:border-none rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(availableRoles).length - 1}"
                                @click="addTeamMemberForm.role = role.key"
                            >
                                <div :class="{'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key}">
                                    <!-- Role Name -->
                                    <div class="flex items-center">
                                        <div class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-400" :class="{'font-semibold': addTeamMemberForm.role == role.key}">
                                            {{ role.name }}
                                        </div>
                                    ...
                                    <!-- Role Description -->
                                    <div class="mt-2 text-xs text-gray-600 dark:text-gray-300 dark:text-gray-400 text-start">
                                        {{ role.description }}
                                    </div>
                                </div>
                            </button>
                ...
                <template #content>
                    <div class="space-y-6">
                        <div v-for="invitation in team.team_invitations" :key="invitation.id" class="flex items-center justify-between">
                            <div class="text-gray-600 dark:text-gray-300 dark:text-gray-400">
                                {{ invitation.email }}
                            </div>
                    ...
                    <div class="relative z-0 mt-1 border border-gray-200 dark:border-slate-800 dark:border-gray-700 rounded-lg cursor-pointer">
                        ...
                        >
                            <div :class="{'opacity-50': updateRoleForm.role && updateRoleForm.role !== role.key}">
                                <!-- Role Name -->
                                <div class="flex items-center">
                                    <div class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-400" :class="{'font-semibold': updateRoleForm.role === role.key}">
                                        {{ role.name }}
                                    </div>
                                ...
                                <!-- Role Description -->
                                <div class="mt-2 text-xs text-gray-600 dark:text-gray-300 dark:text-gray-400">
                                    {{ role.description }}
                                </div>
                            </div>
                        </button>
