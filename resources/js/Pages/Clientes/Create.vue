    ...
    <div class="bg-white dark:bg-slate-900/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl dark:shadow-none rounded-2xl p-8 border border-gray-100 dark:border-gray-700">
      ...
      <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 dark:text-gray-100">Crear Nuevo Cliente</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-0.5">Complete los datos para registrar un nuevo cliente</p>
          </div>
        </div>
      ...
      <div
        ...
        aria-live="polite"
      >
        <p class="text-sm font-medium text-green-800 dark:text-green-300">Cliente creado exitosamente</p>
      </div>
...
const onCpInput = async (val) => {
    // ClientForm ya actualiza form.codigo_postal, pero aquí manejamos API
    // El valor recibido es crudo, pero form.codigo_postal ya debería estar limpio por el input binding?
    // Vue v-model update happens before @input?
    // El evento emitido por ClientForm manda event.target.value.
    
    // Limpieza básica local para lógica
    ...
    if (digits.length === 5) {
        ...
        try {
            ...
            if (availableColonias.value.length === 1) form.colonia = availableColonias.value[0]
            else form.colonia = ''
