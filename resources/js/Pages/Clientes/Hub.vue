...
const getStatusClass = (status) => {
    ...
    return classes[status] || 'bg-gray-200 text-gray-800 dark:text-gray-100';
};
    ...
    <AppLayout title="Hub del Cliente">
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('clientes.index')" class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300 hover:text-gray-900 dark:text-white dark:hover:text-gray-100">
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-slate-900 dark:bg-gray-800 overflow-hidden shadow-xl dark:shadow-none sm:rounded-lg">
                    <!-- Header con información clave -->
                    <div class="p-6 sm:px-20 bg-white dark:bg-slate-900 dark:bg-gray-800 border-b border-gray-200 dark:border-slate-800 dark:border-gray-700">
                        <div class="flex items-start justify-between">
                            <div>
                                <h3 class="text-2xl font-bold text-gray-900 dark:text-white dark:text-gray-100">{{ cliente.nombre_razon_social }}</h3>
                             ...
                             <div class="text-right">
                                <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Saldo Pendiente</p>
                    ...
                    <!-- Pestañas -->
                    <div class="bg-white dark:bg-slate-900 dark:bg-gray-800 px-6">
                        <div class="border-b border-gray-200 dark:border-slate-800 dark:border-gray-700">
                    ...
                    <div class="p-6 bg-gray-50 dark:bg-slate-950 dark:bg-gray-900">
                        <!-- Resumen -->
                        <div v-if="activeTab === 'resumen'" role="tabpanel">
                           <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                               <div class="p-6 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-lg shadow-sm">
                                   <h4 class="font-bold text-gray-800 dark:text-gray-100 dark:text-gray-100">Póliza Activa</h4>
                                    <div v-if="poliza_activa" class="mt-2">
                                        <p class="text-indigo-600 dark:text-indigo-400 font-semibold">{{ poliza_activa.nombre }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Vence: {{ formatDate(poliza_activa.fecha_fin) }}</p>
                                   ...
                                   <div v-else><p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-2">No hay póliza activa.</p></div>
                               </div>
                               <div class="p-6 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-lg shadow-sm">
                                   <h4 class="font-bold text-gray-800 dark:text-gray-100 dark:text-gray-100">Próxima Cita</h4>
                                   <div v-if="cliente.citas && cliente.citas.length > 0" class="mt-2">
                                       <p class="font-semibold text-gray-800 dark:text-gray-100 dark:text-gray-100">{{ formatDate(cliente.citas[0].fecha_hora) }}</p>
                                       <p class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300 truncate">{{ cliente.citas[0].descripcion }}</p>
                                       <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Técnico: {{ cliente.citas[0].tecnico?.name || 'No asignado' }}</p>
                                   </div>
                                   <div v-else><p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-2">No hay citas próximas.</p></div>
                               </div>
                               <div class="p-6 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-lg shadow-sm">
                                   <h4 class="font-bold text-gray-800 dark:text-gray-100 dark:text-gray-100">Último Ticket</h4>
                                   <div v-if="cliente.tickets && cliente.tickets.length > 0" class="mt-2">
                                        <p class="font-semibold text-gray-800 dark:text-gray-100 dark:text-gray-100 truncate">{{ cliente.tickets[0].titulo }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300">Estado: <span :class="['font-semibold px-2 py-0.5 rounded-full text-xs', getStatusClass(cliente.tickets[0].estado)]">{{ cliente.tickets[0].estado }}</span></p>
                                   ...
                                   <div v-else><p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-2">No hay tickets recientes.</p></div>
                               </div>
                           </div>
                        </div>
                        ...
                        <div v-if="activeTab === 'polizas'" role="tabpanel">
                            <h3 class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-100 dark:text-gray-100">Póliza de Servicio Activa</h3>
                            <div v-if="poliza_activa" class="p-6 bg-white dark:bg-slate-900 dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-sm space-y-4">
                               <div class="flex justify-between items-start">
                                    <div>
                                        <p class="text-xl font-bold text-indigo-700 dark:text-indigo-400">{{ poliza_activa.nombre }}</p>
                                        <p class="font-mono text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">{{ poliza_activa.folio }}</p>
                                    </div>
                               ...
                               <div class="grid grid-cols-2 gap-4 text-sm">
                                   <p><strong class="text-gray-800 dark:text-gray-100 dark:text-gray-100">Inicio:</strong> <span class="text-gray-600 dark:text-gray-300 dark:text-gray-300">{{ formatDate(poliza_activa.fecha_inicio) }}</span></p>
                                   <p><strong class="text-gray-800 dark:text-gray-100 dark:text-gray-100">Fin:</strong> <span class="text-gray-600 dark:text-gray-300 dark:text-gray-300">{{ formatDate(poliza_activa.fecha_fin) }}</span></p>
                               </div>
                               <!-- Barras de Consumo -->
                               <div class="space-y-3 pt-4">
                                   <div v-if="poliza_activa.limite_mensual_tickets">
                                       <div class="flex justify-between text-sm mb-1">
                                           ...
                                           <span class="text-gray-600 dark:text-gray-300 dark:text-gray-300">{{ poliza_activa.tickets_soporte_consumidos_mes || 0 }} / {{ poliza_activa.limite_mensual_tickets }}</span>
                                       </div>
                                   ...
                                   <div v-if="poliza_activa.horas_incluidas_mensual">
                                       <div class="flex justify-between text-sm mb-1">
                                           ...
                                           <span class="text-gray-600 dark:text-gray-300 dark:text-gray-300">{{ poliza_activa.horas_consumidas_mes || 0 }} / {{ poliza_activa.horas_incluidas_mensual }}</span>
                                       </div>
                                    ...
                                    <div v-if="poliza_activa.visitas_sitio_mensuales">
                                       <div class="flex justify-between text-sm mb-1">
                                           ...
                                           <span class="text-gray-600 dark:text-gray-300 dark:text-gray-300">{{ poliza_activa.visitas_sitio_consumidas_mes || 0 }} / {{ poliza_activa.visitas_sitio_mensuales }}</span>
                                       </div>
                             ...
                             <div v-else>
                                <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 text-center py-10">El cliente no tiene una póliza de servicio activa.</p>
                            </div>
                        </div>

                        <!-- Tickets -->
                        <div v-if="activeTab === 'tickets'" role="tabpanel" class="space-y-4">
                             <div v-for="ticket in cliente.tickets" :key="ticket.id" class="p-4 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700">
                                <div class="flex justify-between items-start">
                               ...
                               <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-1">
                                   <span>#{{ ticket.folio }}</span> &bull;
                                   <span>Creado: {{ formatDate(ticket.created_at) }}</span> &bull;
                                   <span>Agente: {{ ticket.asignado?.name || 'N/A' }}</span>
                               </div>
                            </div>
                             <div v-if="!cliente.tickets || cliente.tickets.length === 0"><p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 text-center py-10">No hay tickets registrados para este cliente.</p></div>
                        </div>
                        
                        <!-- Citas -->
                        <div v-if="activeTab === 'citas'" role="tabpanel" class="space-y-4">
                             <div v-for="cita in cliente.citas" :key="cita.id" class="p-4 bg-white dark:bg-slate-900 dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700">
                                <div class="flex justify-between items-start">
                              ...
                              <div v-if="!cliente.citas || cliente.citas.length === 0"><p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 text-center py-10">No hay citas registradas para este cliente.</p></div>
                        </div>
                    </div>
                </div>
            </div>
