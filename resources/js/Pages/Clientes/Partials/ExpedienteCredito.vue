...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Expediente de Crédito</h3>
        ...
        <div 
            ...
            :class="[
                ...
                isDragging ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800'
            ]"
        >
            <div class="w-16 h-16 bg-white dark:bg-gray-700 rounded-full shadow-sm flex items-center justify-center text-gray-400 dark:text-gray-500">
            ...
            <div class="text-center">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">INE, Comprobante de Domicilio o Contratos (PDF, JPG, PNG)</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div v-if="cliente.credito_firma" class="bg-gradient-to-br from-emerald-50 dark:from-emerald-900/20 to-white dark:to-gray-800 border border-emerald-200 dark:border-emerald-700 rounded-xl p-4 flex flex-col shadow-sm relative overflow-hidden group">
                ...
                <div class="flex items-center gap-4 mb-4">
                    <div class="w-10 h-10 bg-white dark:bg-gray-700 rounded-lg flex items-center justify-center text-emerald-500 dark:text-emerald-400 shadow-sm">
                        <font-awesome-icon icon="signature" />
                    </div>
                    <div>
                        <p class="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Solicitud Firmada por</p>
                        <p class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ cliente.credito_firmado_nombre }}</p>
                    </div>
                </div>

                <div class="bg-white dark:bg-gray-700 rounded-lg p-2 border border-emerald-100 dark:border-emerald-700 mb-3 select-none">
                    <img :src="cliente.credito_firma" class="max-h-24 mx-auto mix-blend-multiply" alt="Firma Digital">
                </div>

                <div class="grid grid-cols-2 gap-2 mb-3">
                    <div class="bg-white/50 dark:bg-gray-800/50 p-2 rounded-lg border border-emerald-50 dark:border-emerald-700">
                        <p class="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase">Monto Solicitado</p>
                    ...
                    <div class="bg-white/50 dark:bg-gray-800/50 p-2 rounded-lg border border-emerald-50 dark:border-emerald-700">
                        <p class="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase">Plazo Solicitado</p>
                        <p class="text-xs font-black text-blue-700 dark:text-blue-300">{{ cliente.credito_solicitado_dias }} días</p>
                    </div>
                </div>

                <div class="flex items-center justify-between text-[9px] text-gray-400 dark:text-gray-500 font-mono mt-auto pt-2 border-t border-emerald-50 dark:border-emerald-700">
            ...
            <div 
                ...
                class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 flex items-center justify-between group hover:border-blue-200 dark:hover:border-blue-500 transition-all shadow-sm"
            >
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 bg-white dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-400 dark:text-gray-500">
                        <font-awesome-icon icon="file-pdf" v-if="doc.extension === 'pdf'" />
                        <font-awesome-icon icon="file-image" v-else />
                    </div>
                    <div>
                        <p class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ getLabel(doc.tipo) }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ doc.nombre_archivo }}</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    <a :href="doc.url" target="_blank" class="p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                        <font-awesome-icon icon="eye" />
                    </a>
                    <button @click="deleteDocumento(doc.id)" class="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 transition-colors">
        ...
        <div v-if="documentos.length === 0 && !cliente.credito_firma" class="text-center py-10 text-gray-400 dark:text-gray-500 italic">
