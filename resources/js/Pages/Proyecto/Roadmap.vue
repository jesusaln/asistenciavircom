    ...
    <AppLayout :title="`Planeación: ${proyecto.nombre}`">
        ...
        <ConfirmationModal :show="confirmingProjectDeletion" @close="confirmingProjectDeletion = false">
            ...
            <template #footer>
                ...
                <DangerButton class="ml-2" @click="deleteProyecto" :class="{ 'opacity-25': deleteProjectForm.processing }" :disabled="deleteProjectForm.processing">
                    {{ deleteProjectForm.processing ? 'Eliminando...' : 'Eliminar Proyecto' }}
                </DangerButton>
            </template>
        </ConfirmationModal>
...
const confirmDeleteProject = () => {
    ...
    deleteProjectForm.clearErrors();
};
const deleteProyecto = () => {
    deleteProjectForm.delete(route('proyectos.destroy', props.proyecto.id), {
...
const removeGasto = (gastoId) => {
    if(confirm('¿Eliminar este gasto del proyecto?')) {
        router.delete(route('proyectos.gastos.remove', [props.proyecto.id, gastoId]), {
            preserveScroll: true
        });
    }
};
