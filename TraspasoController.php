...
class TraspasoController extends Controller
{
    ...
    {
        // Cargar productos con inventarios para mostrar stock por almacén
        $productos = Producto::select('id', 'nombre', 'codigo', 'requiere_serie', 'stock')
            ->get();

        // Indexar inventarios por producto_id para adjuntarlos a cada producto como array plano
        $inventariosRaw = Inventario::with('almacen:id,nombre')->get()
            ->groupBy('producto_id');

        $productos->transform(function ($producto) use ($inventariosRaw) {
            $invs = $inventariosRaw->get($producto->id, collect());
            $producto->inventarios = $invs->map(function ($inv) {
                return [
                    'almacen_id' => (int) $inv->almacen_id,
                    'cantidad' => (float) $inv->cantidad,
                    'almacen' => $inv->almacen ? ['id' => (int) $inv->almacen->id, 'nombre' => $inv->almacen->nombre] : null,
                ];
            })->values()->all();
            return $producto;
        });
        ...
        return Inertia::render('Traspasos/Create', [
            ...
            'preselected' => [
                ...
                'almacen_origen_id' => $request->integer('almacen_origen_id'),
