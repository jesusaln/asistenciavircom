...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('contabilidad')->name('contabilidad.')->group(function () {
        ...
        Route::get('/reportes/balance-general', [ContabilidadController::class, 'balanceGeneral'])->name('reportes.balance-general');
        Route::get('/reportes/flujo-efectivo', [ContabilidadController::class, 'flujoEfectivo'])->name('reportes.flujo-efectivo');
    ...
    Route::post('/mercadolibre/sugerir-precio', [\App\Http\Controllers\Config\MeliListingController::class, 'sugerirPrecio'])->name('mercadolibre.sugerir-precio');
    Route::post('/mercadolibre/analizar-competencia', [\App\Http\Controllers\Config\MeliListingController::class, 'analizarCompetencia'])->name('mercadolibre.analizar-competencia');
    ...
    Route::get('reportes-inventario/rotacion', [ReportesInventarioController::class, 'rotacionInventario'])->name('reportes-inventario.rotacion');
    ...
    Route::get('/tracking', [\App\Http\Controllers\TrackingController::class, 'index'])->name('tracking.index')->middleware('role:super-admin');
    Route::get('/tracking/data', [\App\Http\Controllers\TrackingController::class, 'data'])->name('tracking.data')->middleware('role:super-admin');
    ...
    Route::get('citas/{cita}/equipos-sin-serializar', [CitaController::class, 'equiposSinSerializar'])->name('citas.equipos-sin-serializar')->middleware('role:ventas|admin|super-admin|tecnico|editor');
    Route::post('citas/{cita}/capturar-seriales', [CitaController::class, 'capturarSeriales'])->name('citas.capturar-seriales')->middleware('role:ventas|admin|super-admin|tecnico|editor');
    ...
    Route::get('citas-por-poliza/{poliza}', [CitaController::class, 'citasPorPoliza'])->name('citas.por-poliza')->middleware('role:ventas|admin|super-admin|tecnico|editor');
    Route::get('citas/historial-equipo/{poliza}', [CitaController::class, 'historialEquipo'])->name('citas.historial-equipo')->middleware('role:ventas|admin|super-admin|tecnico|editor');
    ...
    Route::post('mantenimientos/{mantenimiento}/programar-siguiente', [MantenimientoController::class, 'programarSiguiente'])->name('mantenimientos.programar-siguiente')->middleware('role:admin|editor|super-admin');
    Route::patch('mantenimientos/{mantenimiento}/cancelar', [MantenimientoController::class, 'cancelar'])->name('mantenimientos.cancelar')->middleware('role:admin|editor|super-admin');
    ...
    // Taller
    Route::get('taller/{taller}/reporte', [\App\Http\Controllers\TallerController::class, 'reporte'])->name('taller.reporte')->middleware('role:admin|editor|super-admin|tecnico');
    Route::post('taller/{taller}/facturar', [\App\Http\Controllers\TallerController::class, 'facturar'])->name('taller.facturar')->middleware('role:admin|editor|super-admin');
    Route::resource('taller', \App\Http\Controllers\TallerController::class)->names('taller')->except(['edit'])->middleware('role:admin|editor|super-admin|tecnico');
    ...
    Route::prefix('tecnico/mantenimientos')->name('admin.mantenimientos.tecnico.')->group(function () {
        ...
        Route::post('/{id}/iniciar', [\App\Http\Controllers\Admin\PolizaMantenimientoTecnicoController::class, 'iniciar'])->name('iniciar');
    ...
    // Descarga Masiva SAT (MUST be before /cfdi/{cfdi} wildcard routes)
    ...
    // Generic CFDI routes (wildcard – must be AFTER specific prefixes)
    ...
    Route::get('/monitoreo', [\App\Http\Controllers\Admin\MonitoreoController::class, 'index'])
        ->name('monitoreo')
        ->middleware('role:admin|super-admin');
    Route::post('/monitoreo/refresh', [\App\Http\Controllers\Admin\MonitoreoController::class, 'refresh'])
        ->name('monitoreo.refresh')
        ->middleware('role:admin|super-admin');
    Route::post('/monitoreo/retry-failed', [\App\Http\Controllers\Admin\MonitoreoController::class, 'retryFailed'])
        ->name('monitoreo.retry-failed')
        ->middleware('role:admin|super-admin');
});
