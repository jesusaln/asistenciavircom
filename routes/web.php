...
Route::get('/firmar/{token}', [\App\Http\Controllers\ClientContratoController::class, 'publicSign'])->name('contratos.public.sign');
Route::post('/firmar/{token}', [\App\Http\Controllers\ClientContratoController::class, 'sign'])->name('contratos.public.submit');
Route::get('/firmado/{token}', [\App\Http\Controllers\ClientContratoController::class, 'publicSigned'])->name('contratos.public.signed');
Route::get('/acuse-firma/{token}', [\App\Http\Controllers\ClientContratoController::class, 'downloadAcuse'])->name('contratos.public.acuse');
// -------------------------------------------

// === RUTAS UNICAS ASISTENCIA ===
// Newsletter Tracking
...
Route::get('/crear-poliza-test', function () {
    ...
    $poliza = App\Models\PolizaServicio::create([
        ...
        'plan_poliza_id' => App\Models\PlanPoliza::first()?->id ?? 1,
        'fecha_inicio' => now(),
    ...
    return 'Póliza de prueba #' . $poliza->id . ' creada';
});
