...
class ContabilidadService
{
    ...
    public function generarPolizaDesdeXml(string $xmlContent, int $empresaId, int $userId, ?int $bancoId = null, ?string $clasificacion = null): PolizaContable
    {
        ...
        return DB::transaction(function () use ($data, $empresaId, $userId, $bancoId, $xmlContent, $clasificacion) {
            ...
            $metodoPago = $data['metodo_pago'] ?? 'PUE';
                    ...
                    if ($esVenta) {
                        ...
                        if ($compra) {
                            if ($compra->cuentaPorPagar) {
                                $compra->cuentaPorPagar->marcarPagado($data['metodo_pago'] ?? 'PUE', $bancoId, "Liquidación automática vía integración contable");
                            }
                        }
                    }
                }
    ...
    public function obtenerBalanza(int $empresaId, string $inicio, string $fin): array
    {
        ...
        // 5. Rollup jerárquico (de abajo hacia arriba)
    ...
    public function obtenerEstadoResultados(int $empresaId, string $inicio, string $fin): array
    {
        ...
        // Totales calculados sumando el agrupador mayor (nivel 2 que ya acumula a sus hijas) y cualquier cuenta huérfana de nivel 1
        $sumarSeccion = function($items) {
            $n2 = collect($items)->where('nivel', 2)->sum('monto');
            $n1 = collect($items)->where('nivel', 1)->filter(fn($i) => !in_array($i['codigo'], ['4', '5', '6', '7', '8']))->sum('monto');
            return $n2 + $n1;
        };

        $totalIngresos = collect($ingresos)->where('nivel', 2)->sum('monto');
        if ($totalIngresos == 0) {
            $totalIngresos = collect($ingresos)->where('nivel', 1)->filter(fn($i) => $i['codigo'] !== '4')->sum('monto');
        }
    ...
    public function obtenerIngresosAnuales(int $empresaId, int $anio): float
    {
        ...
        return (float)($pue + $ingresosPagos);
    }
