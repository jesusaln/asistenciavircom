...
class ComisionCalculatorService
{
        ...
        foreach ($ventas as $venta) {
            ...
            if ($esVendedorDirecto || $esTecnicoCita || $esAyudanteCita) {
                foreach ($venta->productos as $p) {
                    ...
                    // Si el usuario es el técnico o ayudante Y el producto es un kit, buscar instalaciones dentro del kit
                    if (($esTecnicoCita || $esAyudanteCita) && $p->tipo_producto === 'kit') {
                        foreach ($p->kitItems as $ki) {
                            if ($ki->item_type === 'servicio' && $ki->item && $ki->item->es_instalacion) {
                                ...
                                if ($tieneAyudante) {
                                    $totalInstalacion = $totalInstalacion * ($esTecnicoCita ? 0.70 : 0.30);
                                } elseif ($esAyudanteCita) {
                                    $totalInstalacion = 0; // Ayudante no gana nada si no está asignado como tal
                                }

                                $itemsBreakdown[] = [
                                    'nombre' => "Instalación (Kit: {$p->nombre})" . ($tieneAyudante ? ($esTecnicoCita ? ' (Líder 70%)' : ' (Ayudante 30%)') : ''),
                                    'tipo' => 'Servicio',
            ...
            if ($esTecnicoCita || $esAyudanteCita || ($esVendedorDirecto && (!$venta->cita || !$venta->cita->tecnico_id))) {
                foreach ($venta->servicios as $s) {
                    ...
                    if ($usuario && $usuario->es_tecnico) {
                        ...
                        if ($tieneAyudante) {
                            $comisionItem = $comisionItem * ($esTecnicoCita ? 0.70 : 0.30);
                        } elseif ($esAyudanteCita) {
                            $comisionItem = 0;
                        }
                    } else {
                    ...
                    $itemsBreakdown[] = [
                        'nombre' => $s->nombre . ($tieneAyudante && ($esTecnicoCita || $esAyudanteCita) ? ($esTecnicoCita ? ' (Líder 70%)' : ' (Ayudante 30%)') : ''),
                        'tipo' => 'Servicio',
                        'cantidad' => $pivot->cantidad,
    ...
    public function calcularComisionVentaParaUsuario(Venta $venta, int $userId): array
    {
        ...
        if ($esTecnicoCita || $esAyudanteCita || ($esVendedorDirecto && (!$venta->cita || !$venta->cita->tecnico_id))) {
            foreach ($venta->servicios as $servicio) {
                ...
                if ($usuario && $usuario->es_tecnico) {
                    ...
                    if ($tieneAyudante) {
                        $comisionServicios += $comisionItem * ($esTecnicoCita ? 0.70 : 0.30);
        ...
        // 3. Comisión por instalaciones dentro de KITS (para técnicos/ayudantes)
        if ($esTecnicoCita || $esAyudanteCita) {
            foreach ($venta->productos as $producto) {
    ...
    public function crearPagoComision(array $data): PagoComision
    {
        ...
        // Solo marcar ventas como pagadas si el pago fue COMPLETO
        if ($pagadoCompleto) {
