...
class CompraTest extends TestCase
{
    ...
    public function puede_crear_compra_basica()
    {
        ...
        $response = $this->from(route('compras.create'))->post(route('compras.store'), $compraData);

        if ($response->isRedirect() && $response->getTargetUrl() === route('compras.create')) {
            dd(session()->all());
        }
    ...
    public function no_puede_cancelar_compra_si_productos_vendidos()
    {
        ...
        $inventario = \App\Models\Inventario::where('producto_id', $this->producto1->id)
            ->where('almacen_id', $this->almacen->id)
            ->first();
        $inventario->increment('cantidad', 10);
        $inventario->decrement('cantidad', 56);
    ...
    public function puede_editar_compra_procesada()
    {
        ...
        $response = $this->from(route('compras.edit', $compra->id))->put(route('compras.update', $compra->id), $updateData);

        if ($response->isRedirect() && $response->getTargetUrl() === route('compras.edit', $compra->id)) {
            dd(session()->all());
        }
    ...
    public function puede_listar_compras_con_filtros()
    {
        ...
        Compra::factory()->create(['estado' => EstadoCompra::Procesada->value]);
        Compra::factory()->create(['estado' => EstadoCompra::Cancelada->value]);
        Compra::factory()->create(['estado' => EstadoCompra::Procesada->value]);
