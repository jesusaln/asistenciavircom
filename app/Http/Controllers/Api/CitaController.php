...
class CitaController extends Controller
{
    ...
    public function checkAvailability(Request $request)
    {
        try {
            ...
            $duracion = $request->input('duracion', 120);
            ...
            if ($citaConflicto) {
                ...
                $finTime = $citaConflicto->fecha_hora_fin ?? $citaConflicto->fecha_hora->copy()->addMinutes(120);
    ...
    private function verificarDisponibilidadTecnico(int $tecnicoId, string $fechaHora, ?int $excludeId = null, int $duracionMin = 120): void
    {
    ...
    public function setPendiente(Request $request, Cita $cita)
    {
        try {
            ...
            if (!$user->hasAnyRole(['admin', 'super-admin'])) {
                return response()->json([
                    'success' => false,
                    'message' => 'Solo los administradores pueden mover citas a pendiente.'
                ], 403);
            }

            $estadoAnterior = $cita->estado;

            // Intentar transición vía máquina de estados; si no es válida, hacer override manual con auditoría
            if (!$cita->cambiarEstado(Cita::ESTADO_PENDIENTE, 'Movida a pendiente por administrador: ' . $user->name)) {
                // Override administrativo: forzar estado y registrar auditoría manual
                $cita->estado = Cita::ESTADO_PENDIENTE;
                $cita->inicio_servicio = null;
                $cita->save();

                \App\Models\CitaHistorial::create([
                    'cita_id' => $cita->id,
                    'user_id' => $user->id,
                    'estado_anterior' => $estadoAnterior,
                    'estado_nuevo' => Cita::ESTADO_PENDIENTE,
                    'comentario' => 'Override administrativo a pendiente por: ' . $user->name,
                    'metadatos' => [
                        'ip' => $request->ip(),
                        'user_agent' => $request->userAgent(),
                        'diff' => ['estado' => ['from' => $estadoAnterior, 'to' => Cita::ESTADO_PENDIENTE]]
                    ],
                ]);
            }
            ...
            Log::error('Error en setPendiente API: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function programarHoy(Request $request, $id)
    {
        try {
            ...
            if (!$user->hasAnyRole(['admin', 'super-admin', 'compras'])) {
                return response()->json([
                    'success' => false,
                    'message' => 'Solo los administradores pueden activar citas para hoy.'
                ], 403);
            }
            ...
            $fechaHora = now();
            $fechaHoraFin = now()->addHours(2);

            // Verificar disponibilidad y límite diario
            $this->verificarDisponibilidadTecnico($tecnicoId, $fechaHora->toDateTimeString(), $cita->id, 120);
            $this->verificarLimiteCitasPorDia($tecnicoId, $fechaHora->toDateTimeString());
