...
class Nom035Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $reEvaluation = [
            ...
            'next_evaluation_date' => $lastClosedPeriod ? $lastClosedPeriod->start_date->copy()->addYears(2) : null,
            'months_overdue' => null,
        ];
        
        if ($lastClosedPeriod) {
            // Usamos start_date como base para que coincida mejor con la fecha real de aplicación de cuestionarios
            $nextDue = $lastClosedPeriod->start_date->copy()->addYears(2);
            $now = now();
            
            if ($nextDue->isPast()) {
                $reEvaluation['due'] = true;
                $reEvaluation['months_overdue'] = round($now->diffInMonths($nextDue));
            } else {
                $reEvaluation['months_remaining'] = (int) round($now->diffInMonths($nextDue));
                $reEvaluation['days_remaining'] = (int) round($now->diffInDays($nextDue));
                $reEvaluation['next_evaluation_date'] = $nextDue;
                
                if ($reEvaluation['months_remaining'] <= 3) {
        ...
        $stepsDone = count(array_filter($wizard));
        $totalSteps = count($wizard);
        $wizard['total_percentage'] = round(($stepsDone / $totalSteps) * 100);
        $wizard['is_complete'] = ($stepsDone === $totalSteps);

        $this->checkComplianceAndNotify($empresa_id, $wizard, $pendingReferrals);
    ...
    public function destroyPeriodo(Nom035EvaluationPeriod $periodo)
    {
        $empresa_id = \App\Support\EmpresaResolver::resolveId();
        ...
        if ($periodo->active) {
            return back()->with('error', 'No puedes eliminar un periodo que está marcado como activo.');
        }

        // Eliminar respuestas y encuestados asociados
        foreach ($periodo->respondents as $respondent) {
            $respondent->answers()->delete();
            $respondent->delete();
        }

        $periodo->delete();

        return back()->with('success', 'El periodo y todos sus registros asociados han sido eliminados permanentemente.');
    }
    ...
    public function certificate(Request $request)
    {
        ...
        $activePeriod = Nom035EvaluationPeriod::where('empresa_id', $empresa_id)
            ->where('active', true)
            ->first();

        if (!$activePeriod) {
            return back()->with('error', 'No hay un periodo activo para generar el certificado.');
        }

        $respondentsCount = Nom035Respondent::where('evaluation_period_id', $activePeriod->id)->count();
        
        // Customization
        $recipient = $request->query('recipient') ?? ($config->responsible_name ?? $empresa->nombre_empresa);

        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('pdf.nom035_certificate', [
            'empresa' => $empresa,
            'period' => $activePeriod,
        ...
        return $pdf->stream("Certificado_Cumplimiento_NOM035_{$empresa->nombre_empresa}.pdf");
    }
    ...
    public function quarterlyReport()
    {
        ...
        // 1. Nuevos ingresos evaluados en el trimestre
        $newRespondents = Nom035Respondent::where('empresa_id', $empresa_id)
            ->whereBetween('created_at', [$quarterStart, $quarterEnd])
            ->count();

        // 2. Denuncias recibidas y resueltas
        $complaints = \App\Models\Nom035Complaint::where('empresa_id', $empresa_id)
            ->whereBetween('created_at', [$quarterStart, $quarterEnd])
            ->get();
        
        // 3. Actividades realizadas
        $activities = \App\Models\Nom035Activity::where('empresa_id', $empresa_id)
            ->whereBetween('activity_date', [$quarterStart, $quarterEnd])
            ->get();

        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('pdf.nom035_quarterly', [
            'empresa' => $empresa,
            'quarter' => $now->quarter,
            'year' => $now->year,
        ...
        return $pdf->stream("Reporte_Trimestral_NOM035_Q{$now->quarter}_{$now->year}.pdf");
    }

    private function checkComplianceAndNotify($empresaId, $wizard, $pendingReferrals)
    {
        // 1. Notificar si falta la política
        if (!$wizard['step1_policy']) {
            \App\Models\UserNotification::createNom035Notification(
                $empresaId,
                'policy_missing',
                'Falta Política NOM-035',
                'No se ha cargado el contenido de la política de prevención de riesgos psicosociales.'
            );
        }

        // 2. Notificar si hay canalizaciones pendientes
        if ($pendingReferrals > 0) {
            \App\Models\UserNotification::createNom035Notification(
                $empresaId,
                'pending_referrals',
                'Canalizaciones Pendientes',
                "Hay {$pendingReferrals} colaboradores que requieren valoración clínica y no han sido atendidos."
            );
        }

        // 3. Notificar si no hay actividades
        if (!$wizard['step6_activities']) {
            \App\Models\UserNotification::createNom035Notification(
                $empresaId,
                'no_activities',
                'Falta Programa de Acción',
                'Debes registrar al menos una actividad de prevención (plática, curso, etc.) para cumplir con la norma.'
            );
        }

        // 4. Notificar denuncias sin revisar
        $unreviewedComplaints = \App\Models\Nom035Complaint::where('empresa_id', $empresaId)
            ->where('status', 'pending')
            ->count();
            
        if ($unreviewedComplaints > 0) {
            \App\Models\UserNotification::createNom035Notification(
                $empresaId,
                'pending_complaints',
                'Denuncias sin Revisar',
                "Tienes {$unreviewedComplaints} denuncias en el buzón esperando revisión.",
                "/nom035/denuncias"
            );
        }
    }
