...
class BancoController extends Controller
{
    ...
    public function indexMovimientos()
    {
        ...
        $movimientos = BancoMovimiento::whereIn('cuenta_bancaria_id', $cuentasIds)
            ->with(['cuentaBancaria', 'poliza.asientos.cuenta'])
            ->orderBy('fecha', 'desc')
            ->get();
    ...
    public function storeMovimiento(Request $request)
    {
        ...
        try {
            ...
            if ($request->tipo === 'traspaso') {
                ...
                $movimientoDestino = BancoMovimiento::create([
            ...
            if ($movimiento) {
                ...
                if ($cuentaBancaria->es_fiscal && !$movimiento->poliza_id) {
                    ...
                    $esTraspaso = $request->tipo === 'traspaso' && isset($cuentaDestino);

                    if ($esTraspaso) {
                        // Traspaso entre cuentas: usar la cuenta contable del destino
                        $cuentaContrapartida = $cuentaDestino->cuentaContable;
                        if (!$cuentaContrapartida) {
                            $cuentaContrapartida = CuentaContable::where('empresa_id', $empresaId)
                                ->where('codigo', 'like', '205%') // Acreedores/TDC
                                ->where('es_detalle', true)
                                ->first();
                        }
                        if (!$cuentaContrapartida) {
                    ...
                    } elseif (str_contains($conceptoUpper, 'COMISION') || str_contains($conceptoUpper, 'MEMBRESIA') || str_contains($conceptoUpper, 'INTERES')) {
                        $cuentaContrapartida = CuentaContable::where('empresa_id', $empresaId)
                    ...
                    // Crear la póliza contable
                    $tipoPoliza = $esTraspaso ? 'diario' : (($request->tipo === 'egreso') ? 'egreso' : 'ingreso');
                    $fecha = $request->fecha;
                    ...
                    $maxNumero = (int) (PolizaContable::where('empresa_id', $empresaId)
                        ...
                        ->orderByDesc('numero')
                        ->lockForUpdate()
                        ->value(DB::raw('CAST(NULLIF(REGEXP_REPLACE(numero, \'[^0-9]\', \'\', \'g\'), \'\') AS INTEGER)')) ?? 0);

                    // Formatear número con prefijo de tipo
                    $prefijo = match($tipoPoliza) {
                        'ingreso' => 'I',
                        'egreso' => 'E',
                        'diario' => 'D',
                        default => ''
                    };
                    $siguienteNumero = $prefijo . str_pad($maxNumero + 1, 5, '0', STR_PAD_LEFT);
                    ...
                    // Crear los asientos
                    if ($esTraspaso) {
                        // Traspaso: DEBE cuenta destino, HABER cuenta origen
                        AsientoContable::create(['poliza_id' => $poliza->id, 'cuenta_id' => $cuentaContrapartida->id, 'debe' => $monto, 'haber' => 0, 'referencia' => 'Traspaso recibido en ' . ($cuentaDestino->alias ?: $cuentaDestino->nombre_banco)]);
                        AsientoContable::create(['poliza_id' => $poliza->id, 'cuenta_id' => $cuentaBanco->id, 'debe' => 0, 'haber' => $monto, 'referencia' => 'Traspaso enviado desde ' . ($cuentaBancaria->alias ?: $cuentaBancaria->nombre_banco)]);
                    } elseif ($request->tipo === 'egreso') {
                    ...
                    if (isset($movimientoDestino)) {
                        $movimientoDestino->update(['poliza_id' => $poliza->id]);
                    }
                }
            }
